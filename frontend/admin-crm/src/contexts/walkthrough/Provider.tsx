// frontend/admin-crm/src/contexts/walkthrough/Provider.tsx
...
import { INITIAL_STATE, INITIAL_PREFERENCES } from './types';
import type { WalkthroughProviderProps } from './types';
import { findTargetElement } from './utils';
...
export const WalkthroughProvider: React.FC<WalkthroughProviderProps> = ({ children }) => {
  ...
  const findTargetElementCb = useCallback(
    async (selector: string, waitFor: boolean = false): Promise<HTMLElement | null> => {
      return findTargetElement(selector, waitFor);
    },
    [],
  );
  ...
  const goToStepInternal = useCallback(
    async (tour: Tour, stepIndex: number, direction: 'forward' | 'backward' = 'forward') => {
      ...
      const targetElement = await findTargetElementCb(step.target, step.waitForElement);
      ...
      if (!targetElement) {
        ...
        while (nextIndex >= 0 && nextIndex < tour.steps.length) {
          ...
          const nextTarget = await findTargetElementCb(nextStep.target, false);
  ...
  const startTour = useCallback(
    async (tourId: TourId) => {
      ...
      for (let i = 0; i < tour.steps.length; i++) {
        const targetElement = await findTargetElementCb(
          tour.steps[i].target,
    ...
    [location.pathname, navigate, goToStepInternal, findTargetElementCb],
  );
