...
describe('venuesApi', () => {
  ...
  describe('getOperatingRules', () => {
    it('calls GET on venue operating_rules endpoint', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/venues/venues/5/operating_rules/');
  ...
  describe('getVenueAvailability', () => {
    it('calls GET with venue id, start_date, and end_date', async () => {
      ...
      const result = await venuesApi.getVenueAvailability(5, '2025-01-01', '2025-01-31');
  ...
  describe('bulkAssignVenues', () => {
    it('calls POST to bulk_assign endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/venues/package-venues/bulk_assign/', data);
      expect(result).toEqual([{ id: 1 }]);
    });
  });
