...
describe('vendorsApi', () => {
  ...
  describe('bulkAssignVendors', () => {
    it('calls POST to bulk_assign endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/vendors/package-vendors/bulk_assign/', data);
      expect(result).toEqual([{ id: 1 }]);
    });
  });
  ...
  describe('getVendorsForPackage', () => {
    it('calls GET with package_id query param', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/vendors/package-vendors/by_package/?package_id=5');
      expect(result).toEqual([{ id: 1 }]);
    });
  });
});
