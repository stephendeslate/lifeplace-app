...
describe('productsApi', () => {
  ...
  describe('getProductsByCategory', () => {
    it('calls GET with category_id query param', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/products/products/by_category/?category_id=7');
      expect(result).toEqual([{ id: 1 }]);
    });
  });
  ...
  describe('createProduct', () => {
    it('transforms event_type_ids to input_event_type_ids for JSON data', async () => {
      ...
      const calledData = mockApi.post.mock.calls[0][1] as Record<string, unknown>;
      expect(calledData.input_event_type_ids).toEqual([1, 2]);
      expect(calledData.event_type_ids).toBeUndefined();
      expect(mockApi.post).toHaveBeenCalledWith('/products/products/', expect.any(Object));
    });
    ...
    it('uses FormData with multipart header when formData is provided', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/products/products/', formData, {
  ...
  describe('validateDiscountForOrder', () => {
    it('calls POST with discount id and validation data', async () => {
      ...
      const result = await productsApi.validateDiscountForOrder(3, validationData as never);
  ...
  describe('incrementDiscountUsage', () => {
    it('calls POST on increment_usage endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/products/discounts/5/increment_usage/');
