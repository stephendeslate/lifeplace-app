...
describe('AvailabilityAPI', () => {
  ...
  describe('checkDateRangeAvailability', () => {
    it('builds params with start_date, end_date, and options', async () => {
      ...
      await AvailabilityAPI.checkDateRangeAvailability('2025-01-01', '2025-01-31', {
        event_type_id: 2,
        booking_flow_id: 1,
      });
  ...
  describe('validateBookingRequest', () => {
    it('calls POST to validate endpoint', async () => {
      ...
      const result = await AvailabilityAPI.validateBookingRequest(request as never);

      expect(mockApi.post).toHaveBeenCalledWith('/events/availability/validate/', request);
  ...
  describe('invalidateCache', () => {
    it('calls POST to cache invalidate endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/events/availability/cache/invalidate/', {
        start_date: '2025-01-01',
        end_date: '2025-01-31',
      });
    });
