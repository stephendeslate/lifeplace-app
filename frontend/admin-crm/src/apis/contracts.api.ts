...
export const contractsApi = {
  ...
  getEventContracts: async (
    ...
    const response = await api.get(
      `/contracts/contracts/?${params.toString()}`,
    );
    const data = response.data as
      | PaginatedResponse<EventContract>
      | EventContract[];
    return Array.isArray(data) ? data : data.results || [];
  },
  ...
  getContractsForEvent: async (eventId: number): Promise<EventContract[]> => {
    const response = await api.get(
      `/contracts/contracts/for_event/?event_id=${eventId}`,
    );
    const data = response.data as
      | PaginatedResponse<EventContract>
      | EventContract[];
  ...
  addContractNote: async (
    id: number,
    data: CreateContractNoteData,
  ): Promise<ContractNote> => {
    const response = await api.post<ContractNote>(
      `/contracts/contracts/${id}/add_note/`,
      data,
    );
    return response.data;
  },
  ...
  getContractNotes: async (contractId: number): Promise<ContractNote[]> => {
    const response = await api.get<ContractNote[]>(
      `/contracts/contracts/${contractId}/notes/`,
    );
    return response.data;
  },
  ...
  getContractSignatures: async (
    ...
    const params = new URLSearchParams();
    if (filters?.contract)
      params.append("contract", filters.contract.toString());
    if (filters?.role) params.append("role", filters.role);

    const response = await api.get(
      `/contracts/signatures/?${params.toString()}`,
    );
    const data = response.data as
      | PaginatedResponse<ContractSignature>
      | ContractSignature[];
    return Array.isArray(data) ? data : data.results || [];
  },

  getContractSignature: async (id: number): Promise<ContractSignature> => {
    const response = await api.get<ContractSignature>(
      `/contracts/signatures/${id}/`,
    );
    return response.data;
  },
  ...
  verifySignature: async (
    ...
    const response = await api.post<ContractSignature>(
      `/contracts/signatures/${id}/verify/`,
      {
  ...
  getAllContractAmendments: async (
    ...
    const response = await api.get(
      `/contracts/amendments/?${params.toString()}`,
    );
    const data = response.data as
      | PaginatedResponse<ContractAmendment>
      | ContractAmendment[];
    return Array.isArray(data) ? data : data.results || [];
  },
  ...
  approveAmendment: async (
    ...
    const response = await api.post<ContractAmendment>(
      `/contracts/amendments/${id}/approve/`,
      {
  ...
  rejectAmendment: async (
    ...
    const response = await api.post<ContractAmendment>(
      `/contracts/amendments/${id}/reject/`,
      {
        review_notes: reviewNotes,
      },
    );
  ...
  createAmendmentContract: async (
    ...
    const response = await api.post<EventContract>(
      `/contracts/amendments/${id}/create_contract/`,
      {
        context_data: contextData,
      },
    );
  ...
  getAllContractDocuments: async (): Promise<ContractDocument[]> => {
    ...
    const data = response.data as
      | PaginatedResponse<ContractDocument>
      | ContractDocument[];
    return Array.isArray(data) ? data : data.results || [];
  },
  ...
  getAllContractNotes: async (): Promise<ContractNote[]> => {
    ...
    const data = response.data as
      | PaginatedResponse<ContractNote>
      | ContractNote[];
    return Array.isArray(data) ? data : data.results || [];
  },
  ...
  getContractsForClient: async (clientId: number): Promise<EventContract[]> => {
    const response = await api.get<EventContract[]>(
      `/contracts/contracts/?client_id=${clientId}`,
    );
    return response.data;
  },
  ...
  downloadContractPdf: async (contractId: number): Promise<Blob> => {
    const response = await api.get(
      `/contracts/contracts/${contractId}/download_pdf/`,
      {
        responseType: "blob",
      },
    );
