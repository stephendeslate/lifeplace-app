...
describe('settingsApi', () => {
  ...
  describe('changePassword', () => {
    it('calls POST to change-password endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/users/me/change-password/', data);
  ...
  describe('acceptInvitation', () => {
    it('calls POST with invitation id and acceptance data', async () => {
      ...
      const result = await settingsApi.acceptInvitation('inv-123', data as never);

      expect(mockApi.post).toHaveBeenCalledWith('/users/invitations/inv-123/accept/', data);
  ...
  describe('updateLegalDocument', () => {
    it('calls PUT with document type and data, extracts nested response', async () => {
      ...
      const result = await settingsApi.updateLegalDocument('terms', updateData as never);

      expect(mockApi.put).toHaveBeenCalledWith('/settings/legal/terms/', updateData);
  ...
  describe('updateCompanySettings', () => {
    it('uses FormData with multipart header when file fields are present', async () => {
      ...
      expect(mockApi.put).toHaveBeenCalledWith('/settings/company/', expect.any(FormData), {
