...
describe('permissionsApi', () => {
  ...
  describe('updateUserPermissions', () => {
    it('calls PATCH with user id and permissions data', async () => {
      ...
      const result = await permissionsApi.updateUserPermissions(5, permissions as never);

      expect(mockApi.patch).toHaveBeenCalledWith('/users/5/permissions/', permissions);
      expect(result).toEqual({
