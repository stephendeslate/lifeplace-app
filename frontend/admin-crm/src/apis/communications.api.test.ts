...
describe('communicationsApi', () => {
  ...
  describe('createTemplate', () => {
    it('calls POST with template data', async () => {
      ...
      const result = await communicationsApi.createTemplate(templateData as never);

      expect(mockApi.post).toHaveBeenCalledWith('/communications/templates/', templateData);
  ...
  describe('updateTemplate', () => {
    it('calls PATCH with id and update data', async () => {
      ...
      const result = await communicationsApi.updateTemplate(3, updateData as never);

      expect(mockApi.patch).toHaveBeenCalledWith('/communications/templates/3/', updateData);
  ...
  describe('deleteTemplate', () => {
    it('calls DELETE with template id', async () => {
      ...
      expect(mockApi.delete).toHaveBeenCalledWith('/communications/templates/7/');
    });
  });
  ...
  describe('sendManual', () => {
    it('calls POST to send_manual endpoint with data', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/communications/records/send_manual/', sendData);
  ...
  describe('duplicateTemplate', () => {
    it('calls POST with new_name in body', async () => {
      ...
      const result = await communicationsApi.duplicateTemplate(5, 'Copy of Test');

      expect(mockApi.post).toHaveBeenCalledWith('/communications/templates/5/duplicate/', {
  ...
  describe('getTemplateHistory', () => {
    it('calls GET for template history', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/communications/templates/5/history/');
  ...
  describe('rollbackTemplate', () => {
    it('calls POST with version in body', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/communications/templates/5/rollback/', {
        version: 2,
      });
  ...
  describe('markAllAsRead', () => {
    it('calls POST with optional filters', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/communications/records/mark_all_as_read/', {
        client_id: 1,
      });
