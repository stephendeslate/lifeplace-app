...
describe('workflowsApi', () => {
  ...
  describe('duplicateWorkflowTemplate', () => {
    it('sends name when provided', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/workflows/templates/5/duplicate/', {
        name: 'Copy',
      });
    });
  ...
  describe('reorderWorkflowStages', () => {
    it('calls POST with reorder data', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/workflows/stages/reorder/', data);
    });
  });
  ...
  describe('getStagesForTemplate', () => {
    it('calls GET on template stages endpoint', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/workflows/templates/5/stages/');
      expect(result).toEqual([{ id: 1 }]);
    });
  });
  ...
  describe('manuallyTriggerStage', () => {
    it('calls POST with event_id in body', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/workflows/stages/3/trigger/', { event_id: 10 });
  ...
  describe('skipStageForEvent', () => {
    it('calls POST to skip_stage with event, stage, and reason', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/workflows/overrides/skip_stage/', {
        event_id: 10,
        stage_id: 5,
        reason: 'Not needed',
      });
  ...
  describe('disableAutomationForEvent', () => {
    it('calls POST to disable_automation endpoint', async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith('/workflows/overrides/disable_automation/', {
        event_id: 10,
        stage_id: 5,
        reason: '',
      });
  ...
  describe('getWebhookEventTypes', () => {
    it('calls GET on event_types endpoint', async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/workflows/webhooks/event_types/');
      expect(result).toEqual([{ value: 'event.created', label: 'Event Created' }]);
    });
  });
});
