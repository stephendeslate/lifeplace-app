...
describe("constants", () => {
  ...
  it("exports admin timezones array", () => {
    ...
    expect(
      ADMIN_TIMEZONES.find((tz) => tz.value === "Asia/Manila"),
    ).toBeDefined();
  });
});
...
describe("isSameDayInManila", () => {
  it("returns true for same day strings", () => {
    expect(
      isSameDayInManila("2025-01-15T01:00:00", "2025-01-15T23:00:00"),
    ).toBe(true);
  });

  it("returns false for different day strings", () => {
    expect(
      isSameDayInManila("2025-01-15T10:00:00", "2025-01-16T10:00:00"),
    ).toBe(false);
  });
  ...
  it("handles cross-midnight UTC but same PHT day", () => {
    ...
    expect(
      isSameDayInManila("2025-01-15T08:00:00", "2025-01-15T16:00:00"),
    ).toBe(true);
  });
});
