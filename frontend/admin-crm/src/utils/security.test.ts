...
import { sanitizeHTML, sanitizeCSS, escapeHTML, sanitizeURL, getSecurityHeaders } from './security';
...
describe('sanitizeHTML', () => {
  describe('strict mode (default)', () => {
    ...
    it('strips disallowed tags like table, a, img', () => {
      const html = '<table><tr><td>data</td></tr></table><a href="#">link</a><img src="x">';
      const result = sanitizeHTML(html, 'strict');
  ...
  describe('email mode', () => {
    it('allows table, a, img tags', () => {
      const html = '<table><tr><td>data</td></tr></table><a href="https://example.com">link</a>';
  ...
  describe('preview mode', () => {
    it('allows mark, del, ins tags', () => {
      const html = '<mark>highlighted</mark><del>deleted</del><ins>inserted</ins>';
...
describe('sanitizeCSS', () => {
  it('removes javascript: from CSS', () => {
    expect(sanitizeCSS('background: javascript:void(0)')).not.toContain('javascript:');
  });
  ...
  it('removes url(data:...)', () => {
    expect(sanitizeCSS('background: url(data:text/html,<script>)')).not.toContain('url');
  });
