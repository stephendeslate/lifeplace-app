  ...
  describe('Mutation Operations', () => {
    ...
    it('sends invitation to client without account', async () => {
      ...
      const clientWithoutAccount =
        result.current.clients.find((c) => !c.has_account) ||
        result.current.clients[0]
  ...
  describe('Client Events', () => {
    it('fetches client events', async () => {
      ...
      const { result: eventsResult } = renderHook(
        () => result.current.useClientEvents(1),
        { wrapper }
      )
