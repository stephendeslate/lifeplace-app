...
describe('getVariableGroupTitle', () => {
  it('returns correct titles for known group keys', () => {
    ...
    expect(getVariableGroupTitle('client_variables')).toBe('Client Information');
    expect(getVariableGroupTitle('signature_variables')).toBe('Signature Placeholders');
  });

  it('converts unknown keys to title case with underscores as spaces', () => {
    expect(getVariableGroupTitle('custom_group_name')).toBe('Custom Group Name');
...
describe('getVariablesForContext', () => {
  ...
  it('returns empty array for schemas without variable_groups', () => {
    ...
    expect(getVariablesForContext(emptySchemas, 'EVENT' as ContextType)).toEqual([]);
  });
...
describe('useTemplateVariables', () => {
  it('calls the communications API for communications domain', async () => {
    ...
    const { result } = renderHook(() => useTemplateVariables('communications'), { wrapper });
