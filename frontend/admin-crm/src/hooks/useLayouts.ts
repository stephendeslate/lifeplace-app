...
export const useLayouts = () => {
  ...
  // Get all layouts
  const useAllLayouts = (filters?: LayoutFilters) => {
    return useQuery({
    ...
    return useMutation({
      ...
      onError: (error: unknown) => {
        const message = (error && typeof error === 'object' && 'response' in error)
          ? String((error as { response?: { data?: { detail?: string; templates?: string[] } } }).response?.data?.detail ||
              (error as { response?: { data?: { templates?: string[] } } }).response?.data?.templates?.[0]) ||
  ...
  const useDeleteLayout = () => {
    return useMutation({
      ...
      onError: (error: unknown) => {
        const message = (error && typeof error === 'object' && 'response' in error)
          ? String((error as { response?: { data?: { error?: string; detail?: string } } }).response?.data?.error ||
  ...
  const useRollbackLayout = () => {
    return useMutation({
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
        queryClient.invalidateQueries({ queryKey: [QUERY_KEY, data.id] });
        queryClient.invalidateQueries({ queryKey: [QUERY_KEY, data.id, 'history'] });
