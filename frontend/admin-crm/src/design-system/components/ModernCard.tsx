...
const getHoverStyles = (variant: CardVariant, clickable: boolean): SxProps<Theme> => {
  if (!clickable) {
    return {
      '&:hover': {
        boxShadow: tokens.shadow.component.cardHover,
...
/**
 * Migrate GlassCard props to ModernCard props
 * Helps with backward compatibility during transition
 */
export const migrateGlassCardProps = (glassProps: {
  variant?: 'light' | 'medium' | 'strong' | 'primary' | 'success' | 'warning' | 'error';
  hover?: boolean;
  clickable?: boolean;
  onClick?: () => void;
  sx?: SxProps<Theme>;
  ...
  // Map glass variants to modern card variants
  ...
  return {
    variant: glassProps.variant ? variantMap[glassProps.variant] || 'elevated' : 'elevated',
    hover: glassProps.hover,
    clickable: glassProps.clickable,
    onClick: glassProps.onClick,
    sx: glassProps.sx,
    children: glassProps.children || null,
    size: 'medium', // Default size for migrated components
  };
};
