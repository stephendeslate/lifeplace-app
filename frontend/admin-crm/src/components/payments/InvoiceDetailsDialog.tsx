...
export const InvoiceDetailsDialog: React.FC<InvoiceDetailsDialogProps> = ({
  ...
  const isOverdue = invoice.status === 'ISSUED' && invoice.due_date && isPast(new Date(invoice.due_date));
        ...
        <Stack spacing={3}>
          ...
          {isOverdue && (
            <Alert severity="error">
              This invoice is overdue. Payment was due on {format(new Date(invoice.due_date!), 'MMM dd, yyyy')}.
            </Alert>
          )}
          ...
          <Box>
            ...
            <Stack spacing={1}>
              ...
              <Box display="flex" justifyContent="space-between">
                ...
                <Typography variant="body2" fontWeight="medium">{invoice.invoice_id}</Typography>
              </Box>
              <Box display="flex" justifyContent="space-between">
              ...
              <Box display="flex" justifyContent="space-between">
                ...
                <Typography variant="body2">{format(new Date(invoice.issue_date), 'MMM dd, yyyy')}</Typography>
              </Box>
              <Box display="flex" justifyContent="space-between">
          ...
          <Box>
            ...
            <Stack spacing={1}>
              <Box display="flex" justifyContent="space-between">
                ...
                <Typography variant="body2">{formatInvoiceAmount(invoice.subtotal, invoice.currency)}</Typography>
              </Box>
              {invoice.tax_amount && parseFloat(invoice.tax_amount) > 0 && (
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">Tax:</Typography>
                  <Typography variant="body2">{formatInvoiceAmount(invoice.tax_amount, invoice.currency)}</Typography>
                </Box>
              )}
              <Divider />
              <Box display="flex" justifyContent="space-between">
              ...
              <Box display="flex" justifyContent="space-between">
                <Typography variant="body2" color={paymentProgress.paid < paymentProgress.total ? 'warning.main' : 'text.secondary'}>
                  Outstanding:
                </Typography>
                <Typography variant="body2" color={paymentProgress.paid < paymentProgress.total ? 'warning.main' : 'text.secondary'} fontWeight="medium">
                  {formatInvoiceAmount(paymentProgress.total - paymentProgress.paid, invoice.currency)}
                </Typography>
              </Box>
            </Stack>
          </Box>
                    ...
                    <TableBody>
                      {invoice.related_payments.map((payment, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{formatInvoiceAmount(payment.amount, payment.currency)}</TableCell>
                          <TableCell>
                            <Chip
                          ...
                          <TableCell>
                            {payment.paid_on ? format(new Date(payment.paid_on), 'MMM dd, yyyy') : '-'}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
