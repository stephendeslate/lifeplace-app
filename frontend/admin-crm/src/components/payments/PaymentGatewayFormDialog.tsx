                ...
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  {getGatewayPaymentMethods('stripe').map((method) => (
                    <Chip
                      key={method.code}
                      label={
                        method.icon.length <= 2 ? `${method.icon} ${method.name}` : method.name
                      }
                      size="small"
                      variant="outlined"
                      sx={{ bgcolor: 'white' }}
        ...
        {isPayMongo && (
          <>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Enter your PayMongo API credentials from your PayMongo Dashboard → Developers → API
                Keys.
              </Typography>
