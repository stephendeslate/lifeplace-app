...
describe('ModernSearch', () => {
  it('renders search input with placeholder text', () => {
    renderWithProviders(<ModernSearch onSearchChange={vi.fn()} placeholder="Search events..." />);
    expect(screen.getByPlaceholderText('Search events...')).toBeInTheDocument();
  });

  it('renders with default placeholder when none provided', () => {
    renderWithProviders(<ModernSearch onSearchChange={vi.fn()} />);
    expect(screen.getByPlaceholderText('Search...')).toBeInTheDocument();
  });

  it('calls onSearchChange when input value changes', () => {
    const handleChange = vi.fn();
    renderWithProviders(<ModernSearch onSearchChange={handleChange} />);
  ...
  it('shows filter button when showFilterButton is true and filters are provided', () => {
    ...
    renderWithProviders(
      <ModernSearch onSearchChange={vi.fn()} showFilterButton={true} filters={filters} />,
    );
  ...
  it('does not show filter button when showFilterButton is false', () => {
    ...
    renderWithProviders(
      <ModernSearch onSearchChange={vi.fn()} showFilterButton={false} filters={filters} />,
    );
    expect(screen.queryByRole('button', { name: /filters/i })).not.toBeInTheDocument();
  });

  it('is disabled when disabled prop is true', () => {
    renderWithProviders(<ModernSearch onSearchChange={vi.fn()} disabled={true} />);
    const input = screen.getByPlaceholderText('Search...');
    expect(input).toBeDisabled();
  });

  it('shows clear button when there is a search value', () => {
    renderWithProviders(<ModernSearch onSearchChange={vi.fn()} searchValue="some query" />);
  ...
  it('calls onSearchChange with empty string when clear button is clicked', () => {
    const handleChange = vi.fn();
    renderWithProviders(<ModernSearch onSearchChange={handleChange} searchValue="some query" />);
