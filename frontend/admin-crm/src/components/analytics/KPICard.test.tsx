...
describe("KPICard", () => {
  ...
  it("shows trend up icon when trend is positive", () => {
    const { container } = renderWithProviders(
      <KPICard title="Events" value="42" trend={15} />
    )
  ...
  it("shows trend down icon when trend is negative", () => {
    const { container } = renderWithProviders(
      <KPICard title="Events" value="42" trend={-8} />
    )
  ...
  it("shows trend flat icon when trend is zero", () => {
    const { container } = renderWithProviders(
      <KPICard title="Events" value="42" trend={0} />
    )
  ...
  it("renders subtitle when provided", () => {
    renderWithProviders(
      <KPICard title="Revenue" value="5000" subtitle="vs last month" />
    )
