...
export const BookingFlowStepsTable: React.FC<BookingFlowStepsTableProps> = ({
  ...
  const hasDisplayConditions = (step: BookingFlowStep) => {
    return (
      step.display_conditions && Object.keys(step.display_conditions).length > 0
    );
  };

  const hasValidationRules = (step: BookingFlowStep) => {
    return (
      step.validation_rules && Object.keys(step.validation_rules).length > 0
    );
  };

  const hasConfiguration = (step: BookingFlowStep) => {
    return (
      step.configuration_data ||
  ...
  return (
    <ModernTable
      columns={
        allColumns as unknown as ModernTableColumn<Record<string, unknown>>[]
      }
