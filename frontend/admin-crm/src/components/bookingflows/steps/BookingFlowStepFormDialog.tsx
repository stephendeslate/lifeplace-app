...
export const BookingFlowStepFormDialog: React.FC<BookingFlowStepFormDialogProps> = ({
  ...
  const [jsonErrors, setJsonErrors] = useState<{ conditions?: string; validation?: string }>({});
  ...
  const handleInputChange = (field: keyof StepFormData) => (
  ...
  const handleSwitchChange = (field: keyof StepFormData) => (
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.step_type === ('availability_check' as StepType)) {
      newErrors.step_type = 'Availability check step type is no longer supported. Use date_time step with availability checking enabled instead.';
    }
    ...
    >
          ...
          <Stack spacing={3}>
                ...
                <FormHelperText>
                  <Box display="flex" alignItems="center" gap={0.5} mt={1}>
                    ...
                    <Typography variant="caption" color="info.main">
                      Some step types have been removed: {stepTypesResponse.removed_types.map(r => r.label).join(', ')}
                    </Typography>
                  </Box>
                </FormHelperText>
              )}
            ...
            {editingStep && isStepTypeRemoved(editingStep.step_type) && (
              <Alert severity="warning" icon={<WarningIcon />}>
                ...
                {(() => {
                  const removedInfo = getRemovedStepInfo(editingStep.step_type);
                  return removedInfo?.migration_available && (
      ...
      <DialogActions sx={{ p: 3 }}>
        ...
        <Button 
          ...
          variant="contained"
          disabled={isLoading || Object.keys(jsonErrors).some(key => jsonErrors[key as keyof typeof jsonErrors])}
