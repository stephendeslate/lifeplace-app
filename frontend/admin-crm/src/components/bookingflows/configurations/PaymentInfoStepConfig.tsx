...
export const PaymentInfoStepConfig: React.FC<PaymentInfoStepConfigProps> = ({
    ...
    if (config) {
      setFormData({
        ...
        quote_request_description:
          config.quote_request_description || 'Get a customized quote for your event',
      });
    }
  }, [config]);

  const handleInputChange =
    (field: keyof PaymentInfoConfigFormData) =>
    (
      event:
  ...
  const handleSwitchChange =
    (field: keyof PaymentInfoConfigFormData) => (event: React.ChangeEvent<HTMLInputElement>) => {
      setFormData((prev) => ({
        ...prev,
  ...
  return (
    <Box>
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        <strong>DRY-Compliant Configuration:</strong> This step only configures UI/UX flags. All
        payment business logic (deposits, refunds, gateways) is managed globally in Payment
        Settings.
      </Alert>
      ...
      <Stack spacing={3}>
        ...
        <Box sx={{ borderRadius: 1, bgcolor: 'background.paper', p: 3 }}>
          ...
          <Stack spacing={2}>
            ...
            {errors.payment_options && <Alert severity="error">{errors.payment_options}</Alert>}
          </Stack>
        </Box>
        ...
        {formData.accept_deposit && (
          <Box sx={{ borderRadius: 1, bgcolor: 'background.paper', p: 3 }}>
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              <strong>Global Setting:</strong> Deposit percentage and balance due days are
              configured globally in Payment Settings.
              {paymentSettings && (
                <>
                  <br />
        ...
        <Box sx={{ borderRadius: 1, bgcolor: 'background.paper', p: 3 }}>
          <Box display="flex" alignItems="center" gap={1} mb={2}>
            ...
            <Typography variant="subtitle1">Refund & Gateway Settings</Typography>
          </Box>

          <Alert severity="info" sx={{ mb: 2 }}>
            <strong>Global Configuration:</strong> Refund policies and payment gateways are
            configured globally in Payment Settings.
            {paymentSettings && (
              <>
                <br />
                ...
                <br />• Refunds:{' '}
                <strong>{paymentSettings.allow_refunds ? 'Enabled' : 'Disabled'}</strong>
                {paymentSettings.allow_refunds && (
                  <>
                    {' '}
                    ({paymentSettings.refund_percentage}% up to{' '}
                    {paymentSettings.refund_deadline_hours}h before event)
                  </>
                )}
                <br />• Default Deposit:{' '}
                <strong>{paymentSettings.default_deposit_percentage}%</strong>
              </>
            )}
          </Alert>
        ...
        <Box sx={{ borderRadius: 1, bgcolor: 'background.paper', p: 3 }}>
          ...
          <Stack spacing={2}>
            ...
            <Typography variant="caption" color="text.secondary">
              Process payment immediately during booking completion (requires payment gateway
              configuration)
            </Typography>
        ...
        <Box sx={{ borderRadius: 1, bgcolor: 'background.paper', p: 3 }}>
          <Box display="flex" alignItems="center" gap={1} mb={2}>
            ...
            <Typography variant="subtitle1">Quote Request Options</Typography>
          </Box>
          ...
          <Stack spacing={2}>
            ...
            <Typography variant="caption" color="text.secondary">
              Allow clients to request quotes for custom requirements. The primary option will
              always be "Secure Your Date" with deposit payment.
            </Typography>
              ...
              <>
                <Alert severity="info" sx={{ my: 2 }}>
                  <Typography variant="body2">
                    <strong>Deposit-First Strategy:</strong> Quote requests appear as a secondary
                    option below the prominent "Secure Your Date" button. This maximizes conversions
                    while providing flexibility for custom bookings.
                  </Typography>
                </Alert>
          ...
          <Stack spacing={1}>
            <Typography variant="body2">
              ...
              {[
                ...
                formData.accept_deposit &&
                  `Deposit (${paymentSettings?.default_deposit_percentage || 50}% from global settings)`,
              ]
                .filter(Boolean)
            ...
            <Alert severity="success" sx={{ mt: 2 }}>
              <Typography variant="caption">
                <strong>DRY Compliance:</strong> Refund policies, payment gateways, and deposit
                amounts are configured globally in Payment Settings - no duplication across booking
                flows!
              </Typography>
            </Alert>
          </Stack>
