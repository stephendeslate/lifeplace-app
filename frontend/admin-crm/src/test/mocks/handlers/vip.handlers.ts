...
import { mockVIPTiers, mockVIPMembers, createMockVIPTier } from '../data/vip.mock';
import type { VIPTier, ClientVIPStatus, VIPSettings } from '../../../types/vip.types';
...
export const vipHandlers = [
  ...
  http.post(`${BASE_URL}/vip/benefits/`, async ({ request }) => {
    ...
    const newBenefit = {
      ...
      application_mode_display: (body.application_mode as string) || 'Automatic',
  ...
  http.post(`${BASE_URL}/vip/client-status/:id/assign_tier/`, async ({ params, request }) => {
    await delay(50);
    const id = Number(params.id);
    const body = (await request.json()) as Record<string, unknown>;
  ...
  http.post(`${BASE_URL}/vip/client-status/:id/award_points/`, async ({ params, request }) => {
    await delay(50);
    const id = Number(params.id);
    const body = (await request.json()) as Record<string, unknown>;
    const idx = membersStore.findIndex((m) => m.id === id);
    ...
    membersStore[idx] = {
      ...
      lifetime_points_earned: membersStore[idx].lifetime_points_earned + points,
      updated_at: new Date().toISOString(),
    };
    return HttpResponse.json({
      transaction: {
  ...
  http.post(`${BASE_URL}/vip/client-status/:id/adjust_points/`, async ({ params, request }) => {
    await delay(50);
    const id = Number(params.id);
    const body = (await request.json()) as Record<string, unknown>;
    const idx = membersStore.findIndex((m) => m.id === id);
  ...
  http.get(`${BASE_URL}/vip/client-status/:id/tier_history/`, async ({ params }) => {
    await delay(30);
    const id = Number(params.id);
    const member = membersStore.find((m) => m.id === id);
    if (!member) {
  ...
  http.get(`${BASE_URL}/vip/client-status/:id/point_transactions/`, async ({ params }) => {
    await delay(30);
    const id = Number(params.id);
    const member = membersStore.find((m) => m.id === id);
    if (!member) {
  ...
  http.get(`${BASE_URL}/vip/point-transactions/`, async ({ request }) => {
    ...
    if (client) {
      transactions = transactions.filter((t) => t.client_vip_status === Number(client));
    }
    if (transactionType) {
