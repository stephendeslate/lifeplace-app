...
export const layoutsHandlers = [
  ...
  http.patch(
    `${BASE_URL}/communications/layouts/:id/`,
  ...
  http.post(
    `${BASE_URL}/communications/layouts/:id/preview/`,
    async ({ params, request }) => {
      await delay(50);
      const id = Number(params.id);
      const layout = layoutsStore.find((l) => l.id === id);
      ...
      const body = (await request.json()) as Record<string, unknown>;
      const sampleContent =
        (body.sample_content as string) || "<p>Sample email content</p>";
  ...
  http.get(
    `${BASE_URL}/communications/layouts/:id/history/`,
    async ({ params }) => {
      await delay(30);
      const id = Number(params.id);
      const layout = layoutsStore.find((l) => l.id === id);
  ...
  http.post(
    `${BASE_URL}/communications/layouts/:id/rollback/`,
    async ({ params }) => {
      await delay(50);
      const id = Number(params.id);
      const layout = layoutsStore.find((l) => l.id === id);
  ...
  http.get(
    `${BASE_URL}/communications/layouts/:id/templates/`,
    async ({ params }) => {
      await delay(30);
      const id = Number(params.id);
      const layout = layoutsStore.find((l) => l.id === id);
  ...
  http.post(
    `${BASE_URL}/communications/layouts/:id/duplicate/`,
    async ({ params, request }) => {
      await delay(50);
      const id = Number(params.id);
      const layout = layoutsStore.find((l) => l.id === id);
