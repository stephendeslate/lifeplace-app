...
import { mockGalleryPhotos, createMockGalleryPhoto } from '../data/gallery.mock';
...
export const galleryHandlers = [
  ...
  http.post(`${BASE_URL}/venues/gallery-photos/`, async ({ request }) => {
    ...
    const newPhoto = createMockGalleryPhoto({
      ...
      event_type: formData.get('event_type') ? Number(formData.get('event_type')) : null,
  ...
  http.patch(`${BASE_URL}/venues/gallery-photos/:id/`, async ({ params, request }) => {
    await delay(50);
    const id = Number(params.id);
  ...
  http.post(`${BASE_URL}/venues/gallery-photos/bulk-create/`, async ({ request }) => {
    await delay(50);
    const formData = await request.formData();
