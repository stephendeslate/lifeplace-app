...
export const notificationsHandlers = [
  ...
  http.get(
    `${BASE_URL}/notifications/notifications/unread/`,
    async ({ request }) => {
      await delay(30);
      const url = new URL(request.url);
      const limit = Number(url.searchParams.get("limit") || 10);
      const unread = notificationsStore
        .filter((n) => !n.is_read)
        .slice(0, limit);
  ...
  http.get(
    `${BASE_URL}/notifications/notifications/recent/`,
    async ({ request }) => {
      await delay(30);
      const url = new URL(request.url);
      const limit = Number(url.searchParams.get("limit") || 10);
  ...
  http.post(
    `${BASE_URL}/notifications/notifications/:id/mark_read/`,
    async ({ params }) => {
      await delay(30);
  ...
  http.post(
    `${BASE_URL}/notifications/notifications/:id/mark_unread/`,
    async ({ params }) => {
      await delay(30);
      const id = Number(params.id);
      const notification = notificationsStore.find((n) => n.id === id);
  ...
  http.post(
    `${BASE_URL}/notifications/notifications/mark_all_read/`,
    async () => {
      await delay(30);
      let count = 0;
  ...
  http.delete(
    `${BASE_URL}/notifications/notifications/:id/`,
    async ({ params }) => {
      await delay(30);
      const id = Number(params.id);
  ...
  http.post(
    `${BASE_URL}/notifications/notifications/create_notification/`,
    async ({ request }) => {
      await delay(50);
  ...
  http.get(
    `${BASE_URL}/notifications/preferences/my_preferences/`,
    async () => {
      await delay(30);
      return HttpResponse.json({
        id: 1,
  ...
  http.put(
    `${BASE_URL}/notifications/preferences/update_preferences/`,
    async ({ request }) => {
      await delay(30);
