...
export const contractsHandlers = [
  ...
  http.patch(
    `${BASE_URL}/contracts/templates/:id/`,
  ...
  http.get(
    `${BASE_URL}/contracts/contracts/for_event/`,
    async ({ request }) => {
      await delay(30);
      const url = new URL(request.url);
  ...
  http.patch(
    `${BASE_URL}/contracts/contracts/:id/`,
    async ({ params, request }) => {
      await delay(50);
      const id = Number(params.id);
      const body = (await request.json()) as Record<string, unknown>;
  ...
  http.post(
    `${BASE_URL}/contracts/contracts/:id/add_signature/`,
    async ({ params }) => {
      await delay(50);
      const id = Number(params.id);
      return HttpResponse.json(
  ...
  http.post(`${BASE_URL}/contracts/contracts/:id/void/`, async ({ params }) => {
    ...
    return HttpResponse.json(
      contractsStore[idx] ||
        createMockEventContract({ id, status: "VOID" as never }),
    );
  }),

  // POST /api/contracts/contracts/:id/request_amendment/ - Request amendment
  http.post(
    `${BASE_URL}/contracts/contracts/:id/request_amendment/`,
    async ({ params }) => {
      await delay(50);
      return HttpResponse.json(
        createMockContractAmendment({
  ...
  http.post(
    `${BASE_URL}/contracts/amendments/:id/approve/`,
    async ({ params }) => {
      await delay(30);
      return HttpResponse.json(
        createMockContractAmendment({
  ...
  http.post(
    `${BASE_URL}/contracts/amendments/:id/reject/`,
    async ({ params }) => {
      await delay(30);
      return HttpResponse.json(
        createMockContractAmendment({
  ...
  http.post(
    `${BASE_URL}/contracts/contracts/:id/send_contract/`,
    async ({ params }) => {
      await delay(50);
      const id = Number(params.id);
      const idx = contractsStore.findIndex((c) => c.id === id);
      ...
      return HttpResponse.json(
        contractsStore[idx] ||
          createMockEventContract({ id, status: "SENT" as never }),
      );
    },
  ),
];
