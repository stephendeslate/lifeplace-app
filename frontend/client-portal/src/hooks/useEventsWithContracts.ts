...
export const useEventsWithContracts = () => {
  ...
  const useEventsListWithContracts = (filters?: EventFilters) => {
    ...
    const eventsWithContracts = useMemo(() => {
      ...
      return eventsQuery.data.map((event): Event => {
        ...
        const eventContracts = contracts.filter(contract => 
          contract.event.id === event.id.toString()
        );
        ...
        const pendingSignatureRequired = eventContracts.some(contract => 
        ...
        const expiryDaysArray = eventContracts
          .map(contract => contract.valid_until ? contractUtils.getDaysUntilExpiry(contract.valid_until) : null)
          .filter((days): days is number => days !== null);
        
        const contractExpiryDays = expiryDaysArray.length > 0 
          ? Math.min(...expiryDaysArray) 
          : null;
  ...
  const useEventWithContracts = (eventId: number) => {
    ...
    const eventWithContracts = useMemo(() => {
      ...
      const eventContracts = contracts.filter(contract => 
        contract.event.id === eventId.toString()
      );
      ...
      const contractSummaries: EventContractSummary[] = eventContracts.map(contract => ({
        ...
        is_urgent: contract.valid_until ? (contractUtils.getDaysUntilExpiry(contract.valid_until) ?? 0) <= 3 : false,
      }));

      // Calculate overall signature progress
      const totalRequired = contractSummaries.reduce((sum, contract) => 
        sum + contract.signature_progress.total_required, 0
      );
      const totalCompleted = contractSummaries.reduce((sum, contract) => 
        sum + contract.signature_progress.signed_count, 0
      );
      ...
      const contractExpiryDays = contractExpiryDaysArray.length > 0 
        ? Math.min(...contractExpiryDaysArray) 
        : null;
      ...
      const enhancedEvent: EventDetail = {
        ...
        pending_signature_required: contractSummaries.some(c => c.can_client_sign && ['SENT', 'PARTIALLY_SIGNED'].includes(c.status)),
        contract_expiry_days: contractExpiryDays,
  ...
  const useEventContracts = (eventId: number) => {
    ...
    return {
      ...
      needsSignature: eventContracts.some(contract => 
        contract.can_client_sign && ['SENT', 'PARTIALLY_SIGNED'].includes(contract.status)
      ),
    };
  };
