...
export const notificationsApi = {
  ...
  getNotifications: async (filters?: NotificationFilters): Promise<Notification[]> => {
    ...
    return (data as PaginatedResponse<Notification>).results || (data as Notification[]);
  },
  ...
  getNotificationTypes: async (): Promise<NotificationType[]> => {
    const response = await api.get<PaginatedResponse<NotificationType> | NotificationType[]>(
      '/notifications/types/?is_active=true',
    );
    const data = response.data;
    return (data as PaginatedResponse<NotificationType>).results || (data as NotificationType[]);
  },
};
