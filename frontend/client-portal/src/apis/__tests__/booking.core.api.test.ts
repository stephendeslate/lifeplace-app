...
describe("BookingCoreApi", () => {
  ...
  describe("startSession", () => {
    it("creates new session for flow", async () => {
      ...
      expect(mockApi.post).toHaveBeenCalledWith(
        "/bookingflow/public/flows/1/start_session/",
        {
          booking_flow: 1,
  ...
  describe("getSession", () => {
    it("retrieves session by UUID", async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith(
        "/bookingflow/public/flows/session/uuid-123/",
      );
      expect(result).toEqual(mockSession);
    });
  });
  ...
  describe("updateSessionData", () => {
    it("updates session step data using PATCH", async () => {
      ...
      const result = await BookingCoreApi.updateSessionData(
        "uuid-123",
        1,
        stepData,
        false,
  ...
  describe("releaseReservation", () => {
    it("releases date reservation", async () => {
      ...
      const result = await BookingCoreApi.releaseReservation(
        "uuid-123",
        "res-token-123",
      );
  ...
  describe("completeBooking", () => {
    ...
    it("includes reservation token when provided", async () => {
      ...
      await BookingCoreApi.completeBooking(
        "uuid-123",
        "payment",
        "res-token-123",
      );
  ...
  describe("getFlowPaymentGateways", () => {
    it("fetches payment gateways for flow", async () => {
      ...
      expect(mockApi.get).toHaveBeenCalledWith(
        "/bookingflow/public/flows/1/payment_gateways/",
      );
  ...
  describe("Session Helpers", () => {
    describe("isSessionExpired", () => {
      ...
      it("returns true for expired session", () => {
        expect(BookingCoreApi.isSessionExpired("2024-06-15T11:00:00Z")).toBe(
          true,
        );
      });

      it("returns false for valid session", () => {
        expect(BookingCoreApi.isSessionExpired("2024-06-15T13:00:00Z")).toBe(
          false,
        );
      });

      it("returns true for exactly current time", () => {
        expect(BookingCoreApi.isSessionExpired("2024-06-15T12:00:00Z")).toBe(
          true,
        );
      });
    });
    ...
    describe("getSessionTimeRemaining", () => {
      ...
      it("returns expired for past time", () => {
        const result = BookingCoreApi.getSessionTimeRemaining(
