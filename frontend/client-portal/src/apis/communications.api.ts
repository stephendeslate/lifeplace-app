...
export const communicationsApi = {
  ...
  getTemplates: async (filters?: {
    ...
    return (
      (data as { results?: CommunicationTemplate[] }).results || (data as CommunicationTemplate[])
    );
  },
  ...
  previewTemplate: async (data: PreviewCommunicationData): Promise<PreviewResponse> => {
    const response = await api.post<PreviewResponse>(
      `/communications/templates/${data.template_id}/preview/`,
      {
  ...
  markAsRead: async (recordId: string): Promise<{ message: string; opened_at?: string }> => {
    const response = await api.post<{ message: string; opened_at?: string }>(
      `/communications/records/${recordId}/mark_as_read/`,
    );
    return response.data;
  },
  ...
  markAsUnread: async (recordId: string): Promise<{ message: string }> => {
    const response = await api.post<{ message: string }>(
      `/communications/records/${recordId}/mark_as_unread/`,
    );
    return response.data;
  },
  ...
  markAllAsRead: async (filters?: {
    channel?: string;
    category?: string;
  }): Promise<{ updated_count: number; opened_at: string }> => {
    const response = await api.post<{ updated_count: number; opened_at: string }>(
  ...
  getUnreadCount: async (): Promise<{ unread_count: number }> => {
    const response = await api.get<{ unread_count: number }>(
      '/communications/records/unread_count/',
    );
    return response.data;
  },
};
