...
export const PaymentStep: React.FC<PaymentStepProps> = ({
  ...
  const hasPackagesSelected = useMemo(() => {
    const packagesFromStepData =
      bookingState.stepData.package_selection?.selected_packages;
    const packagesFromVenueSelection = (
      bookingState.stepData.venue_selection as
        | { selected_packages?: unknown[] }
        | undefined
    )?.selected_packages;
    const packagesFromBookingData = bookingState.currentSession?.booking_data
      ?.selected_packages as unknown[] | undefined;

    const packages =
      packagesFromStepData ||
      packagesFromVenueSelection ||
      packagesFromBookingData ||
      [];
  ...
  const { currentCurrency, formatAmount: currencyFormatAmount } =
    useCurrentCurrency();
  ...
  const { selectedGateway, setSelectedGateway, filteredGateways } =
  ...
  const paymentData: PaymentStepData = useMemo(
    () => ({
      ...
      payment_type:
        stepData.payment_type || (config?.accept_deposit ? "DEPOSIT" : "FULL"),
  ...
  const amounts = useMemo(() => {
    ...
    const depositPercentage =
      effectiveTerms?.deposit_percentage ??
      paymentPlanSettings.default_deposit_percentage;
    const balanceDueDays =
      effectiveTerms?.balance_due_days ?? paymentPlanSettings.balance_due_days;
    const allowRefunds =
      effectiveTerms?.allow_refunds ?? paymentPlanSettings.allow_refunds;
    const refundPercentage =
      effectiveTerms?.refund_percentage ??
      paymentPlanSettings.refund_percentage;
    const refundDeadlineHours =
      effectiveTerms?.refund_deadline_hours ??
      paymentPlanSettings.refund_deadline_hours;
    ...
    const dueNow =
      paymentData.payment_type === "DEPOSIT" ? depositAmount : total;
    const remaining =
      paymentData.payment_type === "DEPOSIT" ? total - depositAmount : 0;
  ...
  React.useEffect(() => {
    if (
      ...
      handleGatewaySelect(
        filteredGateways[0] as unknown as Record<string, unknown>,
      );
    }
  }, [
    isAddingNewMethod,
  ...
  if (gatewaysError) {
    return <Alert severity="error">{gatewaysError}</Alert>;
  }
  ...
  if (paymentSettingsError) {
    return (
      <Alert severity="error">
        ...
        {paymentSettingsError.message || "Unknown error"}
      </Alert>
    );
  }
        ...
        <Box>
          ...
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="body2">
              You haven't selected any packages or add-ons yet. To proceed, you
              can request a custom quote and our team will prepare a
              personalized proposal for your event.
            </Typography>
          </Alert>
    ...
    if (!hasPackagesSelected && amounts.total > 0) {
      return (
        <Box>
          ...
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="body2">
              You've selected add-ons but no event package. To secure your date
              with a payment, please go back and select an event package.
              Alternatively, you can request a custom quote and our team will
              help you build the perfect package for your event.
            </Typography>
          </Alert>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  ...
                  {config?.accept_deposit && (
                    <>
                      • Balance of {amounts.formattedRemaining} due{" "}
                      {amounts.balanceDueDays} days before event
                      <br />
                    </>
                  )}
    ...
    <Box>
      ...
      {completionChoice === "quote" && (
        <>
          ...
          <Alert severity="info" sx={{ mb: 3 }}>
            Tell us about your special requirements and we'll prepare a
            customized quote for you within 24 hours.
          </Alert>
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <Typography
              ...
              sx={{ fontStyle: "italic" }}
            >
              The more details you provide, the more accurate your custom quote
              will be.
            </Typography>
          </Paper>
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
              This amount is an estimate. Your final quote may include
              additional customizations or adjustments based on your specific
              requirements.
            </Typography>
          </Paper>
          ...
          <Box sx={{ textAlign: "center", mb: 2 }}>
            ...
            <Typography
              ...
              sx={{ mt: 2 }}
            >
              We'll review your request and send a detailed quote to your email
              within 24 hours
            </Typography>
        ...
        <>
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
                    ...
                    label={
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                          💰 Pay Deposit ({amounts.formattedDeposit}) -
                          Recommended
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          Balance of {amounts.formattedRemaining} due{" "}
            ...
            {paymentData.payment_type === "DEPOSIT" &&
              amounts.remaining > 0 && (
                <Alert severity="info" sx={{ mb: 2 }}>
                  Remaining balance of {amounts.formattedRemaining} will be due{" "}
                  {amounts.balanceDueDays} days before your event.
                </Alert>
              )}
            ...
            {amounts.allowRefunds && (
              ...
              >
                ...
                <Typography variant="body2">
                  {amounts.refundPercentage}% refund available if cancelled
                  within {amounts.refundDeadlineHours} hours of booking.
                </Typography>
                  ...
                  >
                    ...
                    >
                      ...
                      {selectedPaymentMethod.nickname ||
                        selectedPaymentMethod.type_display}
                      {selectedPaymentMethod.last_four &&
                        ` ending in ${selectedPaymentMethod.last_four}`}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      This payment method will be used when you complete your
                      booking.
                    </Typography>
                  </Box>
                )}
                ...
                <>
                  <Box sx={{ mt: 3 }}>
                    ...
                    <RadioGroup
                      ...
                      onChange={(e) => {
                        const gateway = flowGateways.find(
            ...
            >
              ...
              <Alert severity="success" sx={{ mb: 2 }}>
                ...
                <Typography variant="body2">
                  ...
                  <strong>{amounts.formattedDueNow}</strong> after final
                  confirmation.
                </Typography>
              </Alert>
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary">
                  <strong>Secure Payment:</strong> Your card details are safely
                  stored with Stripe. No payment will be processed until you
          ...
          {process.env.NODE_ENV === "development" && (
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>Debug:</strong> Total Amount:{" "}
                {amounts.dueNow.toFixed(2)} | Payment Data:{" "}
