...
export const InvoicePaymentDialog: React.FC<InvoicePaymentDialogProps> = ({
  ...
  const paymentAmounts = useMemo(() => {
    ...
    const balanceDueDays =
      effectiveTerms?.balance_due_days ??
    ...
    const depositAmount =
      (parseFloat(invoice.total_amount) * depositPercentage) / 100;
  ...
  const handlePaymentMethodSelect = (method: PaymentMethod | null) => {
    ...
    if (method) {
      ...
      if (import.meta.env.DEV)
        console.log(
          "✅ PAYMENT METHOD SELECT - Selected saved method, clearing gateway state",
        );
    } else {
  ...
  const handleFullPayment = async () => {
    ...
    try {
      ...
      if (isAddingNewMethod) {
        ...
        if (import.meta.env.DEV)
          console.log(
            "🔍 PAYMENT DEBUG - New method payment data:",
            paymentData,
          );
      } else {
        ...
        if (import.meta.env.DEV)
          console.log(
            "🔍 PAYMENT DEBUG - Saved method payment data:",
            paymentData,
          );
      }
      ...
      if (import.meta.env.DEV) {
        console.log(
          "🚀 PAYMENT DEBUG - About to call FinancialApi.payInvoice:",
          {
            ...
            <>
              <Stack spacing={3}>
                ...
                {!isLoadingPaymentSettings &&
                  (invoice.effective_payment_terms ||
                    ...
                    <Box>
                      ...
                      {isDepositAlreadyPaid && (
                        <Alert severity="info" sx={{ mb: 2 }}>
                          Deposit of{" "}
                          {formatAmount(
                            paymentAmounts.deposit,
                            invoice.currency,
                          )}{" "}
                          has been paid. You can pay the remaining balance or a
                          custom amount.
                        </Alert>
                      )}
                      ...
                      <RadioGroup
                        ...
                        onChange={(e) =>
                          setPaymentType(
                            e.target.value as "FULL" | "DEPOSIT" | "CUSTOM",
                          )
                        }
                      >
                      ...
                      {paymentType === "CUSTOM" && (
                        <TextField
                          ...
                          InputProps={{
                            startAdornment: (
                              <InputAdornment position="start">
                                {FinancialApi.getCurrencySymbol(
                                  invoice.currency,
                                )}
                              </InputAdornment>
                            ),
                          ...
                          sx={{ mt: 2 }}
                          inputProps={{
                            min: FinancialApi.getMinimumCharge(
                              invoice.currency,
                            ),
                      ...
                      {paymentType === "CUSTOM" && (
                        <Alert severity="info" sx={{ mt: 1 }}>
                          <Typography variant="body2">
                            ...
                            {formatAmount(
                              FinancialApi.getMinimumCharge(invoice.currency),
                              invoice.currency,
                            )}
                            . If you're not paying the full amount, you must
                            leave at least this amount as the remaining balance.
                          </Typography>
                        </Alert>
                      )}
                    </Box>
                  ...
                  selectedPaymentMethod.gateway_details && (
                    ...
                    >
                      ...
                      <Typography variant="caption" color="text.secondary">
                        Click "Pay Now" to complete your payment with this
                        method.
                      </Typography>
                    </Box>
                  )}
          ...
          <DialogActions sx={{ p: 3 }}>
            ...
            <Button
              ...
              disabled={
                ...
                (isAddingNewMethod &&
                  (!selectedGateway || selectedGateway?.code === "stripe")) ||
