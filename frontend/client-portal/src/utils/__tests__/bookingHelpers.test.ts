...
describe("BookingValidationHelpers", () => {
  describe("validateRequiredFields", () => {
    it("returns empty array when all required fields present", () => {
      ...
      const errors = BookingValidationHelpers.validateRequiredFields(
        data,
        requiredFields,
      );
  ...
  describe("validateEmail", () => {
    it("returns true for valid email", () => {
      expect(BookingValidationHelpers.validateEmail("test@example.com")).toBe(
        true,
      );
    });
    ...
    it("returns true for email with subdomain", () => {
      expect(
        BookingValidationHelpers.validateEmail("test@sub.example.com"),
      ).toBe(true);
    });

    it("returns false for invalid email", () => {
      expect(BookingValidationHelpers.validateEmail("invalid-email")).toBe(
        false,
      );
    });
  ...
  describe("validatePhone", () => {
    ...
    it("returns true for Philippine number with country code", () => {
      expect(BookingValidationHelpers.validatePhone("+639123456789")).toBe(
        true,
      );
    });

    it("returns true for phone with spaces/dashes", () => {
      expect(BookingValidationHelpers.validatePhone("0912 345 6789")).toBe(
        true,
      );
      expect(BookingValidationHelpers.validatePhone("0912-345-6789")).toBe(
        true,
  ...
  describe("validateFutureDate", () => {
    ...
    it("returns true for future date", () => {
      expect(BookingValidationHelpers.validateFutureDate("2024-06-20")).toBe(
        true,
      );
    });

    it("returns false for past date", () => {
      expect(BookingValidationHelpers.validateFutureDate("2024-06-10")).toBe(
        false,
      );
    });
    ...
    it("respects minDaysAdvance parameter", () => {
      ...
      expect(BookingValidationHelpers.validateFutureDate("2024-06-22", 7)).toBe(
        true,
      );
      expect(BookingValidationHelpers.validateFutureDate("2024-06-21", 7)).toBe(
        false,
      );
    });
    ...
    it("uses default minDaysAdvance of 1", () => {
      ...
      expect(BookingValidationHelpers.validateFutureDate("2024-06-16", 1)).toBe(
        true,
      );
      expect(BookingValidationHelpers.validateFutureDate("2024-06-15", 1)).toBe(
        false,
      );
    });
  });
  ...
  describe("validateStepData", () => {
    it("validates introduction step acknowledgment", () => {
      ...
      const errorsWithoutAck = BookingValidationHelpers.validateStepData(
        step,
        {},
      );
    ...
    describe("date_time step", () => {
      ...
      it("validates date is in future", () => {
        ...
        expect(
          errors.some(
            (e) => e.field === "start_date" && e.message.includes("future"),
          ),
        ).toBe(true);
      });
    ...
    describe("contact_info step", () => {
      ...
      it("validates email format", () => {
        ...
        expect(
          errors.some(
            (e) => e.field === "email" && e.message.includes("valid email"),
          ),
        ).toBe(true);
      });
      ...
      it("validates phone when require_phone is true", () => {
        ...
        const errors = BookingValidationHelpers.validateStepData(
          stepWithPhone,
          {
            full_name: "John",
    ...
    describe("pricing_summary step", () => {
      ...
      it("validates discount code format if provided", () => {
        ...
        expect(errors.some((e) => e.field === "applied_discount_code")).toBe(
          true,
        );
      });
    });
  ...
  describe("calculateEndTime", () => {
    it("calculates end time correctly", () => {
      expect(BookingValidationHelpers.calculateEndTime("10:00", 2)).toBe(
        "12:00",
      );
    });

    it("handles crossing noon", () => {
      expect(BookingValidationHelpers.calculateEndTime("10:00", 4)).toBe(
        "14:00",
      );
    });

    it("handles crossing midnight", () => {
      expect(BookingValidationHelpers.calculateEndTime("22:00", 4)).toBe(
        "02:00",
      );
    });

    it("preserves minutes", () => {
      expect(BookingValidationHelpers.calculateEndTime("10:30", 2)).toBe(
        "12:30",
      );
    });
  });
  ...
  describe("getSessionRemainingTime", () => {
    ...
    it('returns "Expired" for expired session', () => {
      ...
      expect(BookingValidationHelpers.getSessionRemainingTime(session)).toBe(
        "Expired",
      );
    });
