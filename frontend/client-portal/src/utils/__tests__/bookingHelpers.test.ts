...
describe('BookingValidationHelpers', () => {
  ...
  describe('validateStepData', () => {
    it('validates introduction step acknowledgment', () => {
      ...
      const errorsWithAck = BookingValidationHelpers.validateStepData(step, { acknowledged: true });
    ...
    describe('date_time step', () => {
      ...
      it('validates date is in future', () => {
        ...
        expect(errors.some(e => e.field === 'start_date' && e.message.includes('future'))).toBe(true);
      });
    ...
    describe('contact_info step', () => {
      ...
      it('requires full_name', () => {
        const errors = BookingValidationHelpers.validateStepData(step, { email: 'test@example.com' });
        expect(errors.some(e => e.field === 'full_name')).toBe(true);
      });

      it('requires email', () => {
        const errors = BookingValidationHelpers.validateStepData(step, { full_name: 'John' });
    ...
    describe('payment_info step', () => {
      ...
      it('requires payment_method', () => {
        const errors = BookingValidationHelpers.validateStepData(step, { payment_type: 'FULL' });
        expect(errors.some(e => e.field === 'payment_method')).toBe(true);
      });

      it('requires payment_type', () => {
        const errors = BookingValidationHelpers.validateStepData(step, { payment_method: 'CREDIT_CARD' });
    ...
    describe('review_booking step', () => {
      const step = { step_type: 'review_booking' } as BookingFlowStep;

      it('requires terms_accepted', () => {
        const errors = BookingValidationHelpers.validateStepData(step, {});
        expect(errors.some(e => e.field === 'terms_accepted')).toBe(true);
      });

      it('passes with terms accepted', () => {
        const errors = BookingValidationHelpers.validateStepData(step, { terms_accepted: true });
        expect(errors).toHaveLength(0);
      });
    });
    ...
    describe('package_selection step', () => {
      it('validates minimum selection', () => {
        ...
        const errors = BookingValidationHelpers.validateStepData(step, { selected_packages: [] });
    ...
    describe('addon_selection step', () => {
      it('validates minimum selection', () => {
        ...
        const errors = BookingValidationHelpers.validateStepData(step, { selected_addons: [] });
        expect(errors.some(e => e.message.includes('at least 1'))).toBe(true);
      });
    });
