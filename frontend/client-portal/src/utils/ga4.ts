...
export const GA4Events = {
  ...
  bookingStepViewed: (stepType: string, stepIndex: number) =>
    trackEvent('booking_step_viewed', { step_type: stepType, step_index: stepIndex }),

  bookingStepCompleted: (stepType: string, stepIndex: number) =>
    trackEvent('booking_step_completed', { step_type: stepType, step_index: stepIndex }),
  ...
  bookingAbandoned: (stepType: string, stepIndex: number) =>
    trackEvent('booking_abandoned', { step_type: stepType, step_index: stepIndex }),

  // Payments
  paymentInitiated: (gateway: string, amount?: number) =>
    trackEvent('begin_checkout', { payment_method: gateway, value: amount, currency: 'PHP' }),

  paymentCompleted: (gateway: string, amount?: number) =>
    trackEvent('purchase', { payment_method: gateway, value: amount, currency: 'PHP' }),

  // Engagement
  ctaClicked: (ctaName: string, pageLocation: string) =>
    trackEvent('cta_clicked', { cta_name: ctaName, page_location: pageLocation }),
  ...
  registrationCompleted: (method: string) =>
