...
const nameSchema = z.string().min(1, 'This field is required').max(100, 'Name is too long');
...
export const contactInfoStepSchema = z
  .object({
  ...
  try {
    switch (stepType) {
      case 'introduction':
        return {
          isValid: true,
          data: introductionStepSchema.parse(data),
          errors: {},
        };
      case 'contact_info':
        return {
          isValid: true,
          data: contactInfoStepSchema.parse(data),
          errors: {},
        };
      case 'datetime':
        return {
          isValid: true,
          data: dateTimeStepSchema.parse(data),
          errors: {},
        };
      case 'package_selection':
        return {
          isValid: true,
          data: packageSelectionStepSchema.parse(data),
          errors: {},
        };
      case 'addon_selection':
        return {
          isValid: true,
          data: addonSelectionStepSchema.parse(data),
          errors: {},
        };
      case 'questionnaire':
        return {
          isValid: true,
          data: questionnaireStepSchema.parse(data),
          errors: {},
        };
      case 'payment_info':
        return {
          isValid: true,
          data: paymentInfoStepSchema.parse(data),
          errors: {},
        };
