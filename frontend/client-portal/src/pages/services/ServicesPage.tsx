...
/**
 * Map service IDs to event type names for image lookup.
 * Multiple event type names can map to a single service (e.g. "Retreat" and "Camping"
 * both map to the "camps-retreats" service).
 */
const SERVICE_TO_EVENT_TYPE_NAMES: Record<string, string[]> = {
  "camps-retreats": ["Retreat", "Camping", "Camp"],
...
const ServicesPage: React.FC<ServicesPageProps> = ({ onNavigateToBooking }) => {
  ...
  const { data: eventTypes } = useEventTypeImages();

  /**
   * Build a lookup from service ID to featured image URL.
   * Tries each possible event type name for a service and returns the first
   * one that has a featured_image set.
   */
  const featuredImageMap = useMemo(() => {
    ...
    for (const [serviceId, eventTypeNames] of Object.entries(
      SERVICE_TO_EVENT_TYPE_NAMES,
    )) {
      for (const name of eventTypeNames) {
        const match = eventTypes.find(
          (et) => et.name.toLowerCase() === name.toLowerCase() && et.is_active,
        );
        const featuredImage = match?.featured_image;
        if (featuredImage) {
          map[serviceId] = featuredImage;
          break;
        }
      }
    }
              ...
              <AnimatedElement animation="fadeIn" delay={100}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{ maxWidth: 700 }}
                  >
                    Comprehensive event solutions tailored to your needs, from
              ...
              >
                {services.map((service, index) => (
                  <ServiceCard
                    ...
                    service={service}
                    index={index}
                    featuredImage={featuredImageMap[service.id] ?? null}
