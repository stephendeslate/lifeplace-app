...
describe('WeddingPackages', () => {
  ...
  describe('Venue-Only Options Section', () => {
    ...
    it('displays venue inclusions', () => {
      render(<WeddingPackages {...defaultProps} />);
      expect(screen.getAllByText('• Free prenup venue').length).toBeGreaterThan(0);
      expect(screen.getAllByText('• String lights included').length).toBeGreaterThan(0);
    ...
    it('displays excess hour rates when available', () => {
      ...
      expect(screen.getAllByText('Excess hour: ₱10,000').length).toBeGreaterThan(0);
  ...
  describe('Combination Packages Section', () => {
    it('renders the "Combination Packages (6 Hours)" section heading', () => {
      render(<WeddingPackages {...defaultProps} />);
      expect(screen.getByText('Combination Packages (6 Hours)')).toBeInTheDocument();
    });

    it('renders all 4 combination packages', () => {
      render(<WeddingPackages {...defaultProps} />);
      // These names appear in both combo section and all-in section
      expect(screen.getAllByText('Sanctuary + Open Field').length).toBeGreaterThan(0);
      expect(screen.getAllByText('Sanctuary + Pavilion').length).toBeGreaterThan(0);
      expect(screen.getAllByText('Angelic Field + Open Field').length).toBeGreaterThan(0);
  ...
  describe('All-In Wedding Packages Section', () => {
    ...
    it('displays venue combinations for all-in packages', () => {
      ...
      const angelicOpenField = screen.getAllByText('Angelic Field + Open Field');
    ...
    it('displays all major inclusions for packages', () => {
      render(<WeddingPackages {...defaultProps} />);
      expect(screen.getAllByText('Catering with buffet selections').length).toBe(2);
      expect(screen.getAllByText('Photography and videography').length).toBe(2);
      expect(screen.getAllByText('Professional coordination team').length).toBe(2);
      expect(screen.getAllByText('Lighting and sound equipment').length).toBe(2);
  ...
  describe('Interactivity', () => {
    it('calls onNavigateToBooking when "Inquire Now" button is clicked', async () => {
      ...
      render(<WeddingPackages {...defaultProps} onNavigateToBooking={mockNavigate} />);
