...
describe('ServicesSection', () => {
  ...
  it('renders Weddings service with correct content', () => {
    ...
    const weddingsHeading = screen.getByRole('heading', { name: /weddings/i, level: 3 });
  ...
  it('renders Team Building service with correct content', () => {
    ...
    const teamBuildingHeading = screen.getByRole('heading', { name: /team building/i, level: 3 });
  ...
  it('renders Retreats service with correct content', () => {
    render(<ServicesSection />);

    const retreatsHeading = screen.getByRole('heading', { name: /retreats/i, level: 3 });
    expect(retreatsHeading).toBeInTheDocument();

    const retreatsDescription = screen.getByText(
      /connect with others and find spiritual renewal in our tranquil retreat setting/i
    );
    expect(retreatsDescription).toBeInTheDocument();
  });

  it('renders Camping service with correct content', () => {
    render(<ServicesSection />);

    const campingHeading = screen.getByRole('heading', { name: /camping/i, level: 3 });
    expect(campingHeading).toBeInTheDocument();

    const campingDescription = screen.getByText(
      /experience nature and community in our safe and comfortable camping facilities/i
    );
    expect(campingDescription).toBeInTheDocument();
  });
  ...
  it('renders service cards with correct aria labels', () => {
    ...
    expect(screen.getByLabelText('Retreats service')).toBeInTheDocument();
    expect(screen.getByLabelText('Camping service')).toBeInTheDocument();
  });
  ...
  it('renders icons for all services', () => {
    ...
    // Should have at least 4 icon wrappers
  ...
  it('has proper semantic structure', () => {
    ...
    const teamBuildingHeading = screen.getByRole('heading', { name: /team building/i });
  ...
  it('renders service cards in correct order', () => {
    ...
    expect(headings[2]).toHaveTextContent('Retreats');
    expect(headings[3]).toHaveTextContent('Camping');
  });
  ...
  describe('Accessibility', () => {
    ...
    it('provides descriptive aria-labels for service cards', () => {
      ...
      const retreatsCard = screen.getByLabelText('Retreats service');
      const campingCard = screen.getByLabelText('Camping service');

      expect(weddingsCard).toBeInTheDocument();
      expect(teamBuildingCard).toBeInTheDocument();
      expect(retreatsCard).toBeInTheDocument();
