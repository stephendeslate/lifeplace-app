  ...
  return {
    loginUser: async (email = 'test@example.com', password = 'password123') => {
      ...
      const passwordInput = await waitFor(() =>
        document.querySelector('input[type="password"], input[name="password"]') as HTMLInputElement
      )
      const submitButton = await waitFor(() =>
        document.querySelector('button[type="submit"], button:contains("Login")') as HTMLButtonElement
      )
    ...
    logoutUser: async () => {
      const logoutButton = await waitFor(() =>
        document.querySelector('button:contains("Logout"), [data-testid="logout"]') as HTMLButtonElement
      )
      await user.click(logoutButton)
    },

    mockAuthResponse: (success = true, user = { id: '1', name: 'Test User', email: 'test@example.com' }) => {
      return success 
  ...
  return {
    selectEventType: async (eventType = 'Wedding') => {
      const eventTypeButton = await waitFor(() =>
        document.querySelector(`button:contains("${eventType}"), [data-value="${eventType}"]`) as HTMLButtonElement
      )
      await user.click(eventTypeButton)
    },

    fillContactInfo: async (contactInfo = {
      name: 'John Doe',
      email: 'john@example.com',
      phone: '+1234567890',
    }) => {
      const nameInput = await waitFor(() =>
        document.querySelector('input[name="name"], input[placeholder*="name" i]') as HTMLInputElement
      )
      const emailInput = await waitFor(() =>
    ...
    selectDateTime: async (date = '2024-06-15', time = '15:00') => {
      ...
      const dateInput = await waitFor(() =>
        document.querySelector('input[type="date"], [data-testid="date-picker"]') as HTMLInputElement
      )
      const timeInput = await waitFor(() =>
        document.querySelector('input[type="time"], [data-testid="time-picker"]') as HTMLInputElement
      )
    ...
    proceedToNextStep: async () => {
      const nextButton = await waitFor(() =>
        document.querySelector('button:contains("Next"), [data-testid="next-button"]') as HTMLButtonElement
      )
    ...
    completeBooking: async () => {
      const completeButton = await waitFor(() =>
        document.querySelector('button:contains("Complete"), button:contains("Book Now")') as HTMLButtonElement
      )
  ...
  return {
    ...
    sendMessage: async (content = 'Test message') => {
      ...
      const messageInput = await waitFor(() =>
        document.querySelector('textarea[name="message"], input[placeholder*="message" i]') as HTMLTextAreaElement
      )
      const sendButton = await waitFor(() =>
        document.querySelector('button:contains("Send"), [data-testid="send-button"]') as HTMLButtonElement
      )
