...
- **Facility-wide date blocking** (via events) reserves the entire facility for that date when a permanent block is applied.
- **Venue-level blocked dates** are for administrative blocks (maintenance, closures) on specific venues only -- they do not affect other venues.
...
Each venue can have event-type-specific configurations. For example, "Open Field" might include 3 hours for weddings but 24 hours for camping events. To configure these settings, go to **Settings > Commerce > Products & Packages** and select the venue you want to configure.
