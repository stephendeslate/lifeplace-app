...
class ContractTemplateFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        with_witness = factory.Trait(requires_witness=True, signature_requirements=["CLIENT", "WITNESS", "COMPANY_REP"])

        no_amendments = factory.Trait(allows_amendments=False)

        no_signature_required = factory.Trait(requires_signature=False)
...
class EventContractFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        sent = factory.Trait(
            ...
            valid_until=factory.LazyFunction((timezone.now() + timedelta(days=7)).date),
        )

        partially_signed = factory.Trait(
            status="PARTIALLY_SIGNED",
            sent_at=factory.LazyFunction(lambda: timezone.now() - timedelta(days=2)),
            valid_until=factory.LazyFunction((timezone.now() + timedelta(days=5)).date),
        )
        ...
        expired = factory.Trait(
            status="EXPIRED",
            sent_at=factory.LazyFunction(lambda: timezone.now() - timedelta(days=14)),
            valid_until=factory.LazyFunction((timezone.now() - timedelta(days=7)).date),
        )

        void = factory.Trait(status="VOID")

        amendment = factory.Trait(is_amendment=True, amendment_number=1, status="DRAFT")
...
class ContractSignatureFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        client = factory.Trait(role="CLIENT")

        witness = factory.Trait(role="WITNESS")

        company_rep = factory.Trait(role="COMPANY_REP", signer_title="Event Manager")
...
class ContractAmendmentFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        signed = factory.Trait(
            status="SIGNED",
            reviewed_by=factory.SubFactory("core.factories.users.UserFactory", admin=True),
            reviewed_at=factory.LazyFunction(lambda: timezone.now() - timedelta(days=1)),
        )
...
class ContractDocumentFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        attachment = factory.Trait(document_type="ATTACHMENT")

        addendum = factory.Trait(document_type="ADDENDUM")

        schedule = factory.Trait(document_type="SCHEDULE")

        terms = factory.Trait(document_type="TERMS")

        waiver = factory.Trait(document_type="WAIVER")
...
class ContractNoteFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        general = factory.Trait(category="GENERAL")

        legal = factory.Trait(category="LEGAL")

        negotiation = factory.Trait(category="NEGOTIATION")

        amendment = factory.Trait(category="AMENDMENT")

        issue = factory.Trait(category="ISSUE")

        reminder = factory.Trait(category="REMINDER")

        client_visible = factory.Trait(is_internal=False)
