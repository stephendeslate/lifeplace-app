...
class MessageThreadFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        urgent = factory.Trait(priority="urgent")
        ...
        with_event = factory.Trait(event=factory.SubFactory("core.factories.events.EventFactory"))

        with_admin = factory.Trait(assigned_admin=factory.SubFactory("core.factories.users.UserFactory", admin=True))
...
class MessageFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        internal_note = factory.Trait(
            is_internal_note=True, sender=factory.SubFactory("core.factories.users.UserFactory", admin=True)
        )

        system_message = factory.Trait(message_type="system", content="This is a system notification")

        file_message = factory.Trait(message_type="file")

        event_update = factory.Trait(message_type="event_update", content="Your event details have been updated")

        edited = factory.Trait(edited_at=factory.LazyFunction(timezone.now))
...
class MessageAttachmentFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        image = factory.Trait(filename=factory.Faker("file_name", extension="jpg"), file_type="image/jpeg")

        pdf = factory.Trait(filename=factory.Faker("file_name", extension="pdf"), file_type="application/pdf")
