...
class VendorOperatingRulesFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        with_lead_time = factory.Trait(minimum_lead_days=7)

        with_duration_limits = factory.Trait(minimum_service_hours=Decimal("2.0"), maximum_service_hours=Decimal("8.0"))

        with_setup_teardown = factory.Trait(setup_hours=Decimal("1.0"), teardown_hours=Decimal("0.5"))
...
class PackageVendorFactory(DjangoModelFactory):
    ...
    class Params:
        ...
        with_notes = factory.Trait(notes="Special services included in this package")
