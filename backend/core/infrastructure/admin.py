...
from .models import CircuitBreakerState, Deployment, FailedTask, SystemHealthSnapshot
...
class FailedTaskAdmin(admin.ModelAdmin):
    ...
    def args_formatted(self, obj):
        ...
        return format_html(
            '<pre style="max-height: 200px; overflow: auto;">{}</pre>', json.dumps(obj.args, indent=2, default=str)
        )
    ...
    def kwargs_formatted(self, obj):
        ...
        return format_html(
            '<pre style="max-height: 200px; overflow: auto;">{}</pre>', json.dumps(obj.kwargs, indent=2, default=str)
        )
    ...
    def replay_selected_tasks(self, request, queryset):
        ...
        if success_count:
            self.message_user(request, f"Successfully replayed {success_count} task(s).", messages.SUCCESS)
        if fail_count:
            self.message_user(
                request, f"Failed to replay {fail_count} task(s). Check individual task status.", messages.WARNING
            )
    ...
    def mark_as_ignored(self, request, queryset):
        ...
        count = 0
        for task in queryset:
            task.mark_as_ignored(reason="Bulk ignored via admin", user=request.user)
            count += 1

        self.message_user(request, f"Marked {count} task(s) as ignored.", messages.SUCCESS)

    @admin.action(description="Mark as reviewed")
    def mark_as_reviewed(self, request, queryset):
        """Mark selected tasks as reviewed"""
        count = queryset.update(status="REVIEWED")
        self.message_user(request, f"Marked {count} task(s) as reviewed.", messages.SUCCESS)
...
class CircuitBreakerStateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Service", {"fields": ("service_name", "state")}),
        (
            "Configuration",
            {
                "fields": (
    ...
    def reset_circuit_breaker(self, request, queryset):
        ...
        count = queryset.update(state="CLOSED", failure_count=0, half_open_successes=0, opened_at=None)
        self.message_user(request, f"Reset {count} circuit breaker(s) to CLOSED state.", messages.SUCCESS)
