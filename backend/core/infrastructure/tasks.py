...
def monitor_dlq(self):
    ...
    try:
        ...
        pending_count = FailedTask.objects.filter(status="PENDING_REVIEW").count()

        # Count tasks from last 24 hours
        recent_failures = FailedTask.objects.filter(failed_at__gte=timezone.now() - timedelta(hours=24)).count()

        # Log metrics
        logger.info(f"DLQ Monitor: {pending_count} pending review, {recent_failures} failures in last 24h")

        # Alert thresholds
        if pending_count > 100:
            logger.critical(f"DLQ ALERT: {pending_count} tasks pending review! Immediate attention required.")
            # TODO: Send notification to admin
        elif pending_count > 50:
            logger.warning(f"DLQ WARNING: {pending_count} tasks pending review. Consider reviewing soon.")

        # Check for repeated failures (same task failing multiple times)
        from django.db.models import Count

        repeated_failures = (
            FailedTask.objects.filter(status="PENDING_REVIEW", failed_at__gte=timezone.now() - timedelta(hours=24))
            .values("task_name")
        ...
        for failure in repeated_failures:
            logger.warning(
                f"DLQ: Task '{failure['task_name']}' has failed {failure['count']} times in the last 24 hours"
            )
...
def cleanup_old_failed_tasks(self):
    ...
    try:
        ...
        deleted = FailedTask.objects.filter(
            status__in=["REPLAYED", "RESOLVED_MANUALLY", "IGNORED"], updated_at__lt=resolved_cutoff
        ).delete()
        deleted_counts["resolved"] = deleted[0]

        # Delete old replay failures (90 days)
        replay_failed_cutoff = now - timedelta(days=90)
        deleted = FailedTask.objects.filter(status="REPLAY_FAILED", updated_at__lt=replay_failed_cutoff).delete()
...
def replay_failed_task(self, failed_task_id, user_id=None):
    ...
    try:
        ...
        if result:
            logger.info(f"Successfully replayed task {failed_task_id} as new task {result}")
...
def check_circuit_breaker_health(self):
    ...
    try:
        open_circuits = CircuitBreakerState.objects.filter(state="OPEN")

        half_open_circuits = CircuitBreakerState.objects.filter(state="HALF_OPEN")
...
def snapshot_system_health(self, date_str=None):
    ...
    """
    from .models import CircuitBreakerState, FailedTask, SystemHealthSnapshot
    ...
    try:
        ...
        pending_review_count = FailedTask.objects.filter(status="PENDING_REVIEW").count()
...
def backfill_system_health_snapshots(self, start_date_str, end_date_str):
    ...
    logger.info(
        f"Health snapshot backfill complete: {len(results['success'])} success, {len(results['failed'])} failed"
    )
    return results
