...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["commit_timestamp"]:
            try:
                commit_ts = timezone.datetime.fromisoformat(options["commit_timestamp"].replace("Z", "+00:00"))
            except ValueError:
        ...
        if options["deploy_started_at"]:
            try:
                deploy_started = timezone.datetime.fromisoformat(options["deploy_started_at"].replace("Z", "+00:00"))
            except ValueError:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Recorded deployment: {deployment.service} {deployment.git_sha_short} ({deployment.status})"
            )
        )
