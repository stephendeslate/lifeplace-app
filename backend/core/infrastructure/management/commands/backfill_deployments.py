    ...
    def handle(self, *args, **options):
        ...
        try:
            result = subprocess.run(  # noqa: S603
                ["git", "log", f"--since={options['since']}", "--format=%H|%s|%aI", "--first-parent", "main"],  # noqa: S607
                capture_output=True,
                text=True,
                timeout=30,
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"{action} {created_count} deployment(s), skipped {skipped_count} existing")
        )
