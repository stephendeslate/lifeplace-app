...
class RedisHealthMonitor:
            ...
            try:
                ...
                if ping_time > 10:  # >10ms ping time
                    health_report["alerts"].append(
                        {"level": "warning", "database": db_name, "message": f"High ping time: {ping_time:.2f}ms"}
                    )

                if db_status["hit_ratio"] < 80:  # <80% hit ratio
                    health_report["alerts"].append(
                        {"level": "info", "database": db_name, "message": f"Low hit ratio: {db_status['hit_ratio']}%"}
                    )

                health_report["databases"][db_name] = db_status

            except Exception as e:
                health_report["databases"][db_name] = {"status": "unhealthy", "error": str(e)}
                health_report["overall_status"] = "unhealthy"
                health_report["alerts"].append(
                    {"level": "critical", "database": db_name, "message": f"Connection failed: {e!s}"}
                )
    ...
    def _run_performance_test(self) -> dict:
        ...
        try:
            ...
            return {"error": str(e), "write_time_ms": None, "read_time_ms": None, "data_integrity": False}
    ...
    def get_performance_metrics(self, hours: int = 1) -> dict:
        ...
        for db_name, redis_client in self.redis_clients.items():
            try:
                ...
                if uptime > 0:
                    metrics[db_name]["commands_per_second"] = round(metrics[db_name]["commands_processed"] / uptime, 2)

            except Exception as e:
                metrics[db_name] = {"error": str(e)}

        return {"timestamp": timezone.now().isoformat(), "period_hours": hours, "databases": metrics}
    ...
    def cleanup_expired_keys(self) -> dict:
        ...
        for db_name, redis_client in self.redis_clients.items():
            try:
                ...
                cleanup_results[db_name] = {"error": str(e), "status": "failed"}
                    ...
                    "entries": [
                        {
                            ...
                            "command": " ".join(
                                [arg.decode() if isinstance(arg, bytes) else str(arg) for arg in entry["command"]]
                            ),
                        }
