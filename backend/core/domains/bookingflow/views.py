...
class BookingFlowStepViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            # Get the booking flow to validate it exists
            ...
            if 'order' not in request_data or request_data.get('order') in [None, '', 0]:
                ...
                provided_order = int(request_data['order'])
                ...
                if existing_step:
                    # Get the next available order number instead
                    max_order = BookingFlowStep.objects.filter(
                        booking_flow=booking_flow
                    ).aggregate(models.Max('order'))['order__max']
                    request_data['order'] = (max_order or 0) + 1
            ...
            # If order is not a valid integer, auto-assign
            max_order = BookingFlowStep.objects.filter(
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Pre-process data to handle order if provided
        request_data = request.data.copy()
        
        # If order is provided and is different from current, validate it
        if 'order' in request_data and request_data['order'] != instance.order:
            try:
                new_order = int(request_data['order'])
                # Check if the new order conflicts with existing steps
                existing_step = BookingFlowStep.objects.filter(
                ...
                if existing_step:
                    # Don't allow conflicting orders, keep current order
                    request_data.pop('order')
            except (ValueError, TypeError):
                # Invalid order value, remove it
                request_data.pop('order')
            ...
            if config:
                if step.step_type == 'questionnaire':
                    ...
                    for item in questionnaire_items:
                        questionnaire_fields = item.questionnaire.fields.all().order_by('order')
