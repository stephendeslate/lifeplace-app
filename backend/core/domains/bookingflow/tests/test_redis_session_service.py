...
class BookingFlowSessionServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.cache_patcher = patch(
            'core.domains.bookingflow.redis_session_service.session_cache',
            self.mock_cache
        )
    ...
    def test_create_session(self):
        ...
        session_id = BookingFlowSessionService.create_session(
            booking_flow_id=self.booking_flow.id,
            user_id=123,
            initial_data={'event_type': 'wedding'}
        )
    ...
    def test_get_session_expired(self):
        ...
        session_data = self.mock_cache.get(
            BookingFlowSessionService.SESSION_KEY.format(session_id=session_id)
        )
    ...
    def test_update_session_merges_booking_data(self):
        ...
        BookingFlowSessionService.update_session(
            session_id,
            booking_data={'new_field': 'new_value'}
        )
    ...
    def test_complete_session(self):
        ...
        with patch.object(
            BookingFlowSessionService,
            '_schedule_session_persistence'
        ) as mock_persist:
            result = BookingFlowSessionService.complete_session(
                session_id,
                final_data={'confirmation': 'success'}
            )
    ...
    def test_abandon_session(self):
        ...
        result = BookingFlowSessionService.abandon_session(
            session_id,
            step_name='package_selection'
        )
    ...
    def test_get_user_sessions(self):
        ...
        session1 = BookingFlowSessionService.create_session(
            booking_flow_id=self.booking_flow.id,
            user_id=user_id
        )
        session2 = BookingFlowSessionService.create_session(
            booking_flow_id=self.booking_flow.id,
            user_id=user_id
        )
...
class BookingFlowSessionServiceAnalyticsTestCase(TestCase):
    ...
    def test_get_flow_analytics_summary_empty(self):
        ...
        analytics = BookingFlowSessionService.get_flow_analytics_summary(
            self.booking_flow.id
        )
    ...
    def test_update_flow_analytics_session_started(self):
        ...
        BookingFlowSessionService.create_session(
            booking_flow_id=self.booking_flow.id
        )
...
class BookingFlowSessionServiceIntegrationTestCase(TestCase):
    ...
    def test_complete_booking_flow_lifecycle(self):
        ...
        with patch.object(
            ...
            BookingFlowSessionService.complete_session(
                session_id,
                final_data={'booking_reference': 'ABC123'}
            )
    ...
    def test_abandoned_booking_flow(self):
        ...
        BookingFlowSessionService.abandon_session(
            session_id,
            step_name='contact_info'
        )
    ...
    def test_session_with_validation_errors(self):
        ...
        BookingFlowSessionService.advance_step(
            session_id,
            step_data={'email': 'valid@example.com'},
            step_name='contact_info'
        )
