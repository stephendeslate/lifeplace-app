...
class BookingFlowModelTestCase(TestCase):
    ...
    def test_booking_flow_str_with_event_type(self):
        ...
        booking_flow = BookingFlow.objects.create(name="Wedding Flow", event_type=self.event_type, is_active=True)
    ...
    def test_booking_flow_str_without_event_type(self):
        ...
        booking_flow = BookingFlow.objects.create(name="Generic Flow", event_type=None, is_active=True)
    ...
    def test_event_type_name_property(self):
        ...
        booking_flow_with_type = BookingFlow.objects.create(
            name="Flow With Type", event_type=self.event_type, is_active=True
        )
        booking_flow_without_type = BookingFlow.objects.create(
            name="Flow Without Type", event_type=None, is_active=False
        )
    ...
    def test_unique_active_booking_flow_per_event_type(self):
        ...
        BookingFlow.objects.create(name="First Wedding Flow", event_type=self.event_type, is_active=True)

        # Attempting to create another active flow for the same event type should fail
        with self.assertRaises(ValidationError):
            BookingFlow.objects.create(name="Second Wedding Flow", event_type=self.event_type, is_active=True)
    ...
    def test_unique_active_booking_flow_for_any_event_type(self):
        ...
        BookingFlow.objects.create(name="First Generic Flow", event_type=None, is_active=True)

        # Attempting to create another active flow for null event type should fail
        with self.assertRaises(ValidationError):
            BookingFlow.objects.create(name="Second Generic Flow", event_type=None, is_active=True)

    def test_multiple_inactive_flows_same_event_type(self):
        """Test that multiple inactive flows for same event type are allowed"""
        BookingFlow.objects.create(name="Inactive Flow 1", event_type=self.event_type, is_active=False)
        BookingFlow.objects.create(name="Inactive Flow 2", event_type=self.event_type, is_active=False)
    ...
    def test_enabled_steps_property(self):
        ...
        booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)

        # Create steps with different orders and enabled states
        step1 = BookingFlowStep.objects.create(
            booking_flow=booking_flow, step_type="introduction", order=1, is_enabled=True
        )
        BookingFlowStep.objects.create(
            booking_flow=booking_flow, step_type="contact_info", order=2, is_enabled=False
        )
        step3 = BookingFlowStep.objects.create(
            booking_flow=booking_flow, step_type="confirmation", order=3, is_enabled=True
        )
    ...
    def test_calculate_total_steps(self):
        ...
        BookingFlowStep.objects.create(booking_flow=booking_flow, step_type="introduction", order=1, is_enabled=True)
        BookingFlowStep.objects.create(booking_flow=booking_flow, step_type="contact_info", order=2, is_enabled=True)
        BookingFlowStep.objects.create(booking_flow=booking_flow, step_type="confirmation", order=3, is_enabled=False)
...
class BookingFlowStepModelTestCase(TestCase):
    ...
    def test_step_type_choices(self):
        ...
        for i, step_type in enumerate(valid_types):
            step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type=step_type, order=i + 1)
    ...
    def test_unique_step_type_per_flow(self):
        ...
        with self.assertRaises(Exception):
            BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="introduction", order=2)
    ...
    def test_unique_order_per_flow(self):
        ...
        with self.assertRaises(Exception):
            BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="contact_info", order=1)

    def test_is_visible_for_data_no_conditions(self):
        """Test is_visible_for_data with no display conditions"""
        step = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="introduction", order=1, display_conditions={}
        )
    ...
    def test_is_visible_for_data_with_multiple_conditions(self):
        ...
        self.assertTrue(step.is_visible_for_data({"event_type": "wedding", "has_package": True}))
        self.assertFalse(step.is_visible_for_data({"event_type": "wedding", "has_package": False}))
...
class BookingSessionModelTestCase(TestCase):
    ...
    def test_session_str(self):
        ...
        session = BookingSession.objects.create(
            session_id=self.session_id, booking_flow=self.booking_flow, expires_at=timezone.now() + timedelta(hours=24)
        )
    ...
    def test_mark_step_completed(self):
        ...
        step2 = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="confirmation", order=2, is_enabled=True
        )
    ...
    def test_mark_step_completed_last_step(self):
        ...
        step = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="confirmation", order=1, is_enabled=True
        )
...
class IntroductionStepConfigurationTestCase(TestCase):
    ...
    def test_intro_config_str(self):
        ...
        config = IntroductionStepConfiguration.objects.create(step=self.step, title="Test", content="Test content")
...
class VenueSelectionStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="venue_selection", order=1)
    ...
    def test_venue_config_creation(self):
        ...
        config = VenueSelectionStepConfiguration.objects.create(
            step=self.step, min_venues=1, max_venues=3, show_pricing=True, bundle_discount_percent=Decimal("15.00")
        )
    ...
    def test_venue_config_defaults(self):
        """Test default values"""
        config = VenueSelectionStepConfiguration.objects.create(step=self.step)
...
class DateTimeStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="date_time", order=1)
    ...
    def test_datetime_config_defaults(self):
        """Test default values"""
        config = DateTimeStepConfiguration.objects.create(step=self.step)
...
class PackageSelectionStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.step = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="package_selection", order=1
        )
    ...
    def test_package_config_creation(self):
        ...
        config = PackageSelectionStepConfiguration.objects.create(
            step=self.step, selection_type="SINGLE", min_selection=1, max_selection=1, show_pricing=True
        )
    ...
    def test_package_config_validation_min_max(self):
        ...
        config = PackageSelectionStepConfiguration(step=self.step, min_selection=5, max_selection=3)
    ...
    def test_package_config_multiple_selection(self):
        ...
        config = PackageSelectionStepConfiguration.objects.create(
            step=self.step, selection_type="MULTIPLE", min_selection=1, max_selection=5
        )
...
class AddonSelectionStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="addon_selection", order=1)
    ...
    def test_addon_config_creation(self):
        ...
        config = AddonSelectionStepConfiguration.objects.create(
            step=self.step, min_selection=0, max_selection=10, group_by_category=True, show_recommendations=True
        )
    ...
    def test_addon_config_validation_min_max(self):
        ...
        config = AddonSelectionStepConfiguration(step=self.step, min_selection=5, max_selection=3)
    ...
    def test_addon_config_unlimited_max(self):
        ...
        config = AddonSelectionStepConfiguration.objects.create(step=self.step, min_selection=0, max_selection=0)
...
class PricingSummaryStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="pricing_summary", order=1)
...
class ContactInfoStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="contact_info", order=1)
    ...
    def test_contact_config_creation(self):
        ...
        config = ContactInfoStepConfiguration.objects.create(
            step=self.step, require_full_name=True, require_email=True, require_phone=True, require_address=False
        )
    ...
    def test_contact_config_defaults(self):
        """Test default values"""
        config = ContactInfoStepConfiguration.objects.create(step=self.step)
...
class PaymentInfoStepConfigurationTestCase(TestCase):
    ...
    def test_payment_config_defaults(self):
        """Test default values"""
        config = PaymentInfoStepConfiguration.objects.create(step=self.step)
...
class ConfirmationStepConfigurationTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.booking_flow = BookingFlow.objects.create(name="Test Flow", is_active=True)
        self.step = BookingFlowStep.objects.create(booking_flow=self.booking_flow, step_type="confirmation", order=1)
    ...
    def test_confirmation_config_defaults(self):
        """Test default values"""
        config = ConfirmationStepConfiguration.objects.create(step=self.step, message="Thank you!")
...
class BookingFlowAnalyticsModelTestCase(TestCase):
    ...
    def test_analytics_str(self):
        ...
        analytics = BookingFlowAnalytics.objects.create(booking_flow=self.booking_flow, date=today)
    ...
    def test_analytics_unique_per_day(self):
        ...
        BookingFlowAnalytics.objects.create(booking_flow=self.booking_flow, date=today)
    ...
    def test_analytics_defaults(self):
        """Test default values"""
        analytics = BookingFlowAnalytics.objects.create(booking_flow=self.booking_flow, date=timezone.now().date())
