...
class BookingFlowStepViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not booking_flow_id:
            return Response({"detail": "booking_flow is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"detail": "Booking flow not found"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            with transaction.atomic():
                step = BookingFlowStepService.update_step(instance.id, serializer.validated_data)

            return Response(BookingFlowStepSerializer(step, context=self.get_serializer_context()).data)
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update_configuration(self, request, pk=None):
        ...
        try:
            ...
            with transaction.atomic():
                BookingFlowStepConfigurationService.update_step_configuration(pk, request.data)

            return self.configuration(request, pk)

        except (BookingFlowStepNotFound, InvalidStepConfiguration) as e:
            return Response(
                {"detail": str(e)}, status=e.status_code if hasattr(e, "status_code") else status.HTTP_400_BAD_REQUEST
            )
            ...
            return Response(
                {"detail": f"Validation error: {error_message}", "error_type": "validation_error"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            return Response(
    ...
    def migrate_availability_to_datetime(self, request, pk=None):
        ...
        try:
            ...
            with transaction.atomic():
                BookingFlowStepConfigurationService.migrate_availability_check_to_datetime(pk)
    ...
    def payment_options(self, request, pk=None):
        ...
        try:
            ...
            if not config:
                return Response({"detail": "Payment configuration not found"}, status=status.HTTP_404_NOT_FOUND)
            ...
            for gateway in gateways:
                ...
                if gateway.code == "stripe":
                    gateway_data["supported_methods"] = ["CREDIT_CARD", "DIGITAL_WALLET"]
                    ...
                    gateway_data["supported_methods"] = ["DIGITAL_WALLET"]
                    gateway_data["public_config"] = {
    ...
    def update_payment_terms_configuration(self, request, pk=None):
        ...
        try:
            ...
            with transaction.atomic():
                config = BookingFlowStepConfigurationService.update_payment_terms_configuration(pk, request.data)
    ...
    def availability_settings(self, request, pk=None):
        ...
        try:
            ...
            if step.step_type != "date_time":
                return Response(
                    {"detail": "This action is only available for date_time steps"}, status=status.HTTP_400_BAD_REQUEST
                )

            # Get step configuration
            config = getattr(step, "datetime_config", None)
            if not config:
                return Response({"detail": "DateTime configuration not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def assign_questionnaires(self, request, pk=None):
        ...
        try:
            ...
            with transaction.atomic():
                BookingFlowStepConfigurationService.assign_questionnaires(pk, questionnaire_ids)
    ...
    def available_packages(self, request, pk=None):
        ...
        packages = (
            ProductOption.objects.filter(type="PACKAGE", is_active=True)
    ...
    def available_addons(self, request, pk=None):
        ...
        addons = (
            ProductOption.objects.filter(type="PRODUCT", is_active=True)
            .select_related("category")
            .order_by("category__name", "name")
        )
        ...
        try:
            ...
            validation_rules = {
                ...
                "date_time": {
                    ...
                    "availability_fields": ["venue_preference", "resource_requirements", "staff_requirements"],
                    "note": "Duration fields (start_time, end_time, duration) are deprecated. Use venue_additional_hours instead.",
                },
                "questionnaire": {"required_fields": [], "optional_fields": ["responses"]},
                "package_selection": {"required_fields": ["selected_packages"], "optional_fields": []},
                "addon_selection": {"required_fields": [], "optional_fields": ["selected_addons"]},
                "pricing_summary": {
                    "required_fields": [],
                ...
                "payment_info": {"required_fields": ["payment_method"], "optional_fields": ["billing_address"]},
                "confirmation": {"required_fields": [], "optional_fields": []},
            }
            ...
            return Response(
                {"step_type": step.step_type, "validation_rules": rules, "custom_rules": step.validation_rules}
            )
