...
class BookingSessionService:
    ...
    def update_session_data(session_id, step_data, mark_completed=False):
        ...
        if session.current_step:
            validation_errors = BookingSessionService._validate_step_data(session.current_step, step_data, session)
    ...
    def _apply_marketing_consent(user, marketing_consent: bool):
        ...
        try:
            ...
            if created:
                logger.info(
                    f"Created NotificationPreference for user {user.id} with marketing_consent={marketing_consent}"
                )
            else:
                    ...
                    # CRITICAL FIX: Ensure start_date is datetime even without time
                        ...
                        try:
                            ...
                            for date_format in ["%Y-%m-%d", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%S.%f"]:
                                ...
                                logger.warning(
                                    f"Start date parsing failed (no format matched): {start_date}, using current time as fallback"
                                )
                            ...
                            for date_format in ["%Y-%m-%d", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%S.%f"]:
                                try:
                                    if "T" in date_format:
                                        ...
                                        event_data["end_date"] = datetime.combine(
                                            parsed_date, datetime.max.time().replace(microsecond=0)
                                        )
                                    break
                                except ValueError:
                                    continue
    ...
    def create_quote_from_booking_session(session, event, completion_type="payment"):
        ...
        BookingSessionService._get_event_duration_from_booking_data(session.booking_data)
    ...
    @staticmethod
    def _extract_venue_id_from_booking_data(booking_data: dict[str, Any]) -> int | None:
        """
