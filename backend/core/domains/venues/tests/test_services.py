...
from core.domains.venues.services import VenueAvailabilityService, VenueService
...
class TestVenueServiceGetMethods:
    ...
    def test_get_primary_venue_for_package_no_primary(self, product_option_factory, venue_factory):
        """Test get_primary_venue_for_package falls back to first venue."""
        package = product_option_factory(type="PACKAGE")
        venue = venue_factory()
        PackageVenue.objects.create(package=package, venue=venue, is_primary=False, access_order=1)
...
class TestVenueServiceCalculateEventTimes:
    ...
    def test_calculate_basic_event_times(self, venue_factory, venue_operating_rules_factory):
        ...
        result = VenueService.calculate_event_times(
            venue=venue, program_date=date(2025, 6, 15), program_start_time=time(14, 0), program_hours=Decimal("4.0")
        )
    ...
    def test_calculate_event_times_with_custom_ingress_egress(self, venue_factory, venue_operating_rules_factory):
    ...
    def test_calculate_event_times_overnight_venue(self, venue_factory, venue_operating_rules_factory):
        """Test event times for overnight venue with next day checkout."""
        venue = venue_factory(is_overnight=True)
        venue_operating_rules_factory(
            venue=venue, checkout_next_day=True, default_check_in_time=time(14, 0), default_checkout_time=time(12, 0)
        )

        result = VenueService.calculate_event_times(
            venue=venue, program_date=date(2025, 6, 15), program_start_time=time(14, 0), program_hours=Decimal("6.0")
        )
    ...
    def test_calculate_event_times_with_early_checkin(self, venue_factory, venue_operating_rules_factory):
    ...
    def test_calculate_event_times_with_late_checkout(self, venue_factory, venue_operating_rules_factory):
...
class TestVenueServiceValidateBookingRequest:
    ...
    def test_validate_no_operating_rules(self, venue_factory):
        ...
        result = VenueService.validate_booking_request(
            venue=venue, program_date=future_date, program_start_time=time(14, 0), program_hours=Decimal("4.0")
        )
    ...
    def test_validate_program_too_short(self, venue_factory, venue_operating_rules_factory):
        ...
        venue = venue_factory()
        venue_operating_rules_factory(venue=venue, minimum_program_hours=Decimal("3.0"))
        future_date = timezone.now().date() + timedelta(days=7)

        result = VenueService.validate_booking_request(
            venue=venue, program_date=future_date, program_start_time=time(14, 0), program_hours=Decimal("2.0")
        )
    ...
    def test_validate_program_too_long(self, venue_factory, venue_operating_rules_factory):
        ...
        venue = venue_factory()
        venue_operating_rules_factory(venue=venue, maximum_program_hours=Decimal("6.0"))
        future_date = timezone.now().date() + timedelta(days=7)

        result = VenueService.validate_booking_request(
            venue=venue, program_date=future_date, program_start_time=time(14, 0), program_hours=Decimal("8.0")
        )
    ...
    def test_validate_start_time_too_early(self, venue_factory, venue_operating_rules_factory):
        ...
        venue = venue_factory()
        venue_operating_rules_factory(venue=venue, earliest_start_time=time(10, 0))
        future_date = timezone.now().date() + timedelta(days=7)

        result = VenueService.validate_booking_request(
            venue=venue, program_date=future_date, program_start_time=time(8, 0), program_hours=Decimal("4.0")
        )
    ...
    def test_validate_past_date(self, venue_factory, venue_operating_rules_factory):
        ...
        result = VenueService.validate_booking_request(
            venue=venue, program_date=past_date, program_start_time=time(14, 0), program_hours=Decimal("4.0")
        )
...
class TestVenueServiceGetAvailableTimeSlots:
    ...
    def test_get_available_time_slots_with_rules(self, venue_factory, venue_operating_rules_factory):
        """Test getting available time slots."""
        venue = venue_factory()
        venue_operating_rules_factory(
            venue=venue, earliest_start_time=time(10, 0), minimum_program_hours=Decimal("2.0")
        )
        future_date = timezone.now().date() + timedelta(days=7)

        result = VenueService.get_available_time_slots(
            venue=venue, program_date=future_date, program_hours=Decimal("3.0")
        )
    ...
    def test_get_available_time_slots_no_rules(self, venue_factory):
        ...
        result = VenueService.get_available_time_slots(venue=venue, program_date=future_date)
...
class TestVenueServiceCalculateTotalFees:
    ...
    def test_calculate_fees_not_allowed(self, venue_factory, venue_operating_rules_factory):
        ...
        venue = venue_factory()
        venue_operating_rules_factory(venue=venue, early_checkin_allowed=False, late_checkout_allowed=False)
...
class TestVenueAvailabilityServiceCheckBlockedDate:
    ...
    def test_check_available_date(self, venue_factory):
        ...
        result = VenueAvailabilityService.check_venue_blocked_date(venue=venue, check_date=future_date)
    ...
    def test_check_full_day_blocked(self, venue_factory, venue_blocked_date_factory):
        ...
        venue_blocked_date_factory(venue=venue, date=block_date, reason="Maintenance", is_full_day=True)

        result = VenueAvailabilityService.check_venue_blocked_date(venue=venue, check_date=block_date)
    ...
    def test_check_partial_day_blocked_overlapping(self, venue_factory, venue_blocked_date_factory):
        ...
        result = VenueAvailabilityService.check_venue_blocked_date(
            venue=venue, check_date=block_date, start_time=time(10, 0), end_time=time(14, 0)
        )
    ...
    def test_check_partial_day_blocked_no_overlap(self, venue_factory, venue_blocked_date_factory):
        ...
        result = VenueAvailabilityService.check_venue_blocked_date(
            venue=venue, check_date=block_date, start_time=time(14, 0), end_time=time(18, 0)
        )
...
class TestVenueAvailabilityServiceMultipleVenues:
    ...
    def test_check_multiple_venues_all_available(self, venue_factory):
        ...
        result = VenueAvailabilityService.check_multiple_venues_availability(
            venue_ids=[venue1.id, venue2.id], check_date=future_date
        )
    ...
    def test_check_multiple_venues_one_blocked(self, venue_factory, venue_blocked_date_factory):
        """Test checking multiple venues with one blocked."""
        venue1 = venue_factory()
        venue2 = venue_factory()
        block_date = timezone.now().date() + timedelta(days=7)

        venue_blocked_date_factory(venue=venue1, date=block_date, reason="Maintenance", is_full_day=True)

        result = VenueAvailabilityService.check_multiple_venues_availability(
            venue_ids=[venue1.id, venue2.id], check_date=block_date
        )
...
class TestVenueAvailabilityServicePackageVenues:
    ...
    def test_check_package_venues_all_available(self, package_with_venues):
        ...
        result = VenueAvailabilityService.check_package_venues_availability(
            package_id=package.id, check_date=future_date
        )

        assert result["is_available"] is True
        assert len(result["venues"]) == 2

    def test_check_package_venues_one_blocked(self, package_with_venues, venue_blocked_date_factory):
        """Test package unavailable when one venue is blocked."""
        package = package_with_venues["package"]
        primary_venue = package_with_venues["primary_venue"]
        block_date = timezone.now().date() + timedelta(days=7)

        venue_blocked_date_factory(venue=primary_venue, date=block_date, reason="Blocked", is_full_day=True)

        result = VenueAvailabilityService.check_package_venues_availability(
            package_id=package.id, check_date=block_date
        )
...
class TestVenueAvailabilityServiceBlockedDates:
    ...
    def test_get_blocked_dates_for_venue(self, venue_factory, venue_blocked_date_factory):
        ...
        venue_blocked_date_factory(
            venue=venue, date=today + timedelta(days=5), reason="Block 1", is_full_day=True
        )
        venue_blocked_date_factory(
            venue=venue,
        ...
        result = VenueAvailabilityService.get_blocked_dates_for_venue(
            venue=venue, start_date=today, end_date=today + timedelta(days=30)
        )
    ...
    def test_get_blocked_dates_out_of_range(self, venue_factory, venue_blocked_date_factory):
        ...
        venue = venue_factory()
        today = timezone.now().date()
        venue_blocked_date_factory(venue=venue, date=today + timedelta(days=60), reason="Far Future", is_full_day=True)
    ...
    def test_get_available_dates_for_venue(self, venue_factory, venue_blocked_date_factory):
        ...
        venue_blocked_date_factory(venue=venue, date=today + timedelta(days=3), reason="Blocked", is_full_day=True)

        result = VenueAvailabilityService.get_available_dates_for_venue(
            venue=venue, start_date=today, end_date=today + timedelta(days=5)
        )
    ...
    def test_get_available_dates_with_day_filter(self, venue_factory):
        ...
        result = VenueAvailabilityService.get_available_dates_for_venue(
            venue=venue, start_date=today, end_date=today + timedelta(days=14), days_of_week=[5, 6]
        )
...
class TestVenueAvailabilityServiceBlockUnblock:
    ...
    def test_block_date_full_day(self, venue_factory, user_factory):
        ...
        result = VenueAvailabilityService.block_date(
            venue=venue, block_date=block_date, reason="Maintenance", created_by=user
        )
    ...
    def test_unblock_date(self, venue_factory, venue_blocked_date_factory):
        ...
        venue_blocked_date_factory(venue=venue, date=block_date, reason="Block 1")
        venue_blocked_date_factory(
            venue=venue,
            date=block_date,
            reason="Block 2",
        ...
        deleted_count = VenueAvailabilityService.unblock_date(venue=venue, block_date=block_date)
