...
class Venue(BaseModel):
    ...
    is_overnight = models.BooleanField(
        default=False, help_text="True for overnight stays (e.g., Cabana with checkout next day)"
    )
    ...
    recommended_capacity = models.PositiveIntegerField(
        null=True, blank=True, help_text="Recommended guest count for optimal experience"
    )
    ...
    featured_image = models.ImageField(upload_to="venues/images/", null=True, blank=True)
    gallery_images = models.JSONField(default=list, blank=True, help_text="List of image URLs for venue gallery")
    amenities = models.JSONField(
        default=list, blank=True, help_text="List of amenities (e.g., ['Pool', 'Parking', 'Sound System', 'WiFi'])"
    )
    sort_order = models.PositiveIntegerField(default=0)

    # Standalone rental pricing (for custom package curation)
    is_rentable_standalone = models.BooleanField(
        default=False, help_text="Can this venue be rented without a pre-made package?"
    )
    standalone_base_price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Price when rented as standalone venue"
    )
    standalone_included_hours = models.DecimalField(
        max_digits=4, decimal_places=1, null=True, blank=True, help_text="Hours included when rented standalone"
    )
    standalone_excess_hour_price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, help_text="Excess hour rate when rented standalone"
    )
...
class VenueOperatingRules(BaseModel):
    ...
    venue = models.OneToOneField(Venue, on_delete=models.CASCADE, related_name="venue_operating_rules")
    ...
    checkout_next_day = models.BooleanField(
        default=False, help_text="If True, checkout time is on the day AFTER check-in (overnight stays)"
    )

    # === PROGRAM DURATION ===
    minimum_program_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("1.0"), help_text="Minimum program duration in hours"
    )
    ...
    default_program_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("3.0"), help_text="Default program duration in hours"
    )
    is_fixed_duration = models.BooleanField(
        default=False, help_text="If True, program duration is fixed and cannot be adjusted by client"
    )
    ...
    min_ingress_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("0.0"), help_text="Minimum ingress hours when customizable"
    )
    max_ingress_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("6.0"), help_text="Maximum ingress hours when customizable"
    )
    min_egress_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("0.0"), help_text="Minimum egress hours when customizable"
    )
    max_egress_hours = models.DecimalField(
        max_digits=4, decimal_places=1, default=Decimal("2.0"), help_text="Maximum egress hours when customizable"
    )

    # === TIME CONSTRAINTS ===
    earliest_start_time = models.TimeField(null=True, blank=True, help_text="Earliest time a program can start")
    latest_end_time = models.TimeField(
        null=True, blank=True, help_text="Latest time program can end (e.g., 21:00 for music curfew)"
    )
    hard_cutoff_time = models.TimeField(
        null=True, blank=True, help_text="Absolute latest time all activities must conclude (e.g., 02:00)"
    )
    hard_cutoff_next_day = models.BooleanField(
        default=True, help_text="If True, hard_cutoff_time is on the next day (e.g., 2:00 AM)"
    )
    early_access_minutes = models.PositiveIntegerField(
        default=60, help_text="Minutes before program when early prep access is allowed"
    )
    ...
    earliest_checkin_time = models.TimeField(
        null=True, blank=True, help_text="Absolute earliest check-in time allowed (e.g., 10:00)"
    )
    ...
    late_checkout_max_hours = models.PositiveIntegerField(
        default=4, help_text="Maximum hours for late checkout extension"
    )
    latest_checkout_time = models.TimeField(null=True, blank=True, help_text="Absolute latest checkout time allowed")

    # === CUSTOM RULES (JSON) ===
    custom_rules = models.JSONField(default=dict, blank=True, help_text="Custom venue rules as JSON")
    ...
    def validate_program_duration(self, program_hours: Decimal) -> dict:
        ...
        if program_hours < self.minimum_program_hours:
            errors.append(f"Program must be at least {self.minimum_program_hours} hours")

        # Check maximum duration
        if self.maximum_program_hours and program_hours > self.maximum_program_hours:
            errors.append(f"Program cannot exceed {self.maximum_program_hours} hours")

        # Check fixed duration
        if self.is_fixed_duration and program_hours != self.default_program_hours:
            errors.append(f"This venue requires exactly {self.default_program_hours} hours")
...
class PackageVenue(BaseModel):
    ...
    venue = models.ForeignKey(Venue, on_delete=models.CASCADE, related_name="venue_packages")

    # Primary venue determines datetime rules
    is_primary = models.BooleanField(
        default=False, help_text="Primary venue determines datetime rules (check-in, checkout, duration)"
    )
    ...
    notes = models.TextField(
        blank=True, help_text="Notes about this venue's usage in the package (e.g., 'For ceremony only')"
    )

    # Bonus venue and contribution tracking (for custom package curation)
    is_bonus = models.BooleanField(
        default=False, help_text="True if venue is included free (e.g., cabanas with bundle)"
    )
    hours_contribution = models.DecimalField(
        max_digits=4, decimal_places=1, null=True, blank=True, help_text="Hours this venue contributes to package total"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_primary:
            PackageVenue.objects.filter(package=self.package, is_primary=True).exclude(pk=self.pk).update(
                is_primary=False
            )
...
class VenueEventTypeConfiguration(BaseModel):
    ...
    venue = models.ForeignKey(Venue, on_delete=models.CASCADE, related_name="event_type_configs")
    event_type = models.ForeignKey("events.EventType", on_delete=models.CASCADE, related_name="venue_configs")
    ...
    is_all_day_access = models.BooleanField(
        default=False, help_text="If True, venue has all-day access with no hour limits for this event type"
    )

    # Operating rule overrides (commonly differ between event types)
    default_check_in_time = models.TimeField(
        null=True, blank=True, help_text="Override check-in time for this event type"
    )
    default_checkout_time = models.TimeField(
        null=True, blank=True, help_text="Override checkout time for this event type"
    )
    checkout_next_day = models.BooleanField(
        null=True, blank=True, help_text="Override: checkout is next day (for overnight)"
    )
    maximum_program_hours = models.DecimalField(
        max_digits=4,
        decimal_places=1,
    ...
    is_fixed_duration = models.BooleanField(null=True, blank=True, help_text="Override: duration is fixed")
...
class VenueBlockedDate(BaseModel):
    ...
    blocked_start_time = models.TimeField(null=True, blank=True, help_text="Start time of block (if partial day)")
    blocked_end_time = models.TimeField(null=True, blank=True, help_text="End time of block (if partial day)")

    created_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, related_name="venue_blocks_created"
    )
    ...
    def __str__(self):
        ...
        return (
            f"{self.venue.name} blocked on {self.date} {self.blocked_start_time}-{self.blocked_end_time}: {self.reason}"
        )
...
class GalleryPhoto(BaseModel):
    ...
    venue = models.ForeignKey(Venue, null=True, blank=True, on_delete=models.SET_NULL, related_name="gallery_photos")
    event_type = models.ForeignKey(
        "events.EventType", null=True, blank=True, on_delete=models.SET_NULL, related_name="gallery_photos"
    )
    is_featured = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    sort_order = models.PositiveIntegerField(default=0)
