...
class VenueSerializer(serializers.ModelSerializer):
    ...
    operating_rules = VenueOperatingRulesSerializer(source="venue_operating_rules", read_only=True)
    ...
    def validate(self, data):
        ...
        if max_cap and min_cap > max_cap:
            raise serializers.ValidationError({"minimum_capacity": "Minimum capacity cannot exceed maximum capacity"})
...
class VenueDetailSerializer(VenueSerializer):
    ...
    def get_packages(self, obj):
        ...
        package_venues = (
            obj.venue_packages.filter(package__is_active=True).select_related("package").order_by("access_order")
        )
    ...
    def get_blocked_dates(self, obj):
        ...
        blocked = obj.blocked_dates.filter(date__gte=timezone.now().date()).order_by("date")[:30]
...
class VenueWithRulesSerializer(serializers.ModelSerializer):
    ...
    operating_rules = VenueOperatingRulesSerializer(source="venue_operating_rules", required=False)
    ...
    def validate(self, data):
        ...
        if not is_full_day:
            ...
            if not data.get("blocked_start_time") or not data.get("blocked_end_time"):
                raise serializers.ValidationError("Partial day blocks require both start and end times")

            # Ensure end time is after start time
            if data["blocked_end_time"] <= data["blocked_start_time"]:
                raise serializers.ValidationError({"blocked_end_time": "End time must be after start time"})
...
class PublicVenueSerializer(serializers.ModelSerializer):
    ...
    operating_rules = PublicVenueOperatingRulesSerializer(source="venue_operating_rules", read_only=True)
...
class PublicPackageVenueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PackageVenue
        fields = ["venue", "is_primary", "access_order", "access_duration_hours", "notes", "operating_rules"]

    def get_operating_rules(self, obj):
        """Only return operating rules for primary venue"""
        if obj.is_primary and hasattr(obj.venue, "venue_operating_rules"):
            return PublicVenueOperatingRulesSerializer(obj.venue.venue_operating_rules).data
        return None
...
class RentableVenueWithEventTypeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "effective_included_hours",
            "effective_excess_hour_price",
            "is_all_day_access",
    ...
    def get_event_type_config(self, obj):
        ...
        try:
            return VenueEventTypeConfiguration.objects.get(venue=obj, event_type_id=event_type_id)
