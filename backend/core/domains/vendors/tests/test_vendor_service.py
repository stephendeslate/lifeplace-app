...
class TestGetVendorByCode:
    ...
    def test_returns_vendor_when_exists(self, vendor_factory):
        ...
        vendor_factory(code="VENDOR_ABC")
...
class TestAssignVendorToPackage:
    ...
    def test_creates_package_vendor_assignment(self, vendor_factory):
        ...
        result = VendorService.assign_vendor_to_package(
            package_id=package.id, vendor_id=vendor.id, notes="Test assignment", sort_order=5
        )
    ...
    def test_default_values(self, vendor_factory):
        ...
        result = VendorService.assign_vendor_to_package(package_id=package.id, vendor_id=vendor.id)
...
class TestBulkAssignVendors:
    ...
    def test_uses_update_or_create(self, vendor_factory):
        ...
        PackageVendor.objects.create(vendor=vendor, package=package, notes="Original notes", sort_order=0)

        # Bulk assign with updated values
        assignments = [{"vendor_id": vendor.id, "notes": "Updated notes", "sort_order": 5}]
...
class TestVendorServiceIntegration:
    ...
    def test_full_vendor_lifecycle(self, vendor_factory, operating_rules_factory):
        ...
        vendor = vendor_factory(name="Integration Test Vendor", code="VENDOR_INTEGRATION", service_category="CATERING")
        operating_rules_factory(vendor=vendor, minimum_lead_days=14)
        ...
        VendorService.assign_vendor_to_package(
            package_id=package1.id, vendor_id=vendor.id, notes="Catering for Package A"
        )
        VendorService.assign_vendor_to_package(
            package_id=package2.id, vendor_id=vendor.id, notes="Catering for Package B"
        )
    ...
    def test_vendor_visibility_rules(self, vendor_factory):
        ...
        # Active and bookable
        visible_vendor = vendor_factory(name="Visible", is_active=True, is_bookable=True, service_category="DJ")

        # Inactive
        vendor_factory(name="Inactive", is_active=False, is_bookable=True, service_category="DJ")

        # Not bookable
        vendor_factory(name="Not Bookable", is_active=True, is_bookable=False, service_category="DJ")
