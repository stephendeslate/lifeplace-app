...
class TestVendorModel:
    ...
    def test_vendor_pricing_notes(self, vendor_factory):
        ...
        vendor = vendor_factory(
            pricing_notes='Starting at $50/hour. Package deals available.'
        )
    ...
    def test_vendor_service_description(self, vendor_factory):
        ...
        vendor = vendor_factory(
            service_description='Full-service catering for events of all sizes.'
        )
...
class TestPackageVendorModel:
    ...
    def test_create_package_vendor(self, vendor_factory, package_vendor_factory):
        ...
        pv = package_vendor_factory(
            vendor=vendor,
            package=package,
            notes='Provides catering for the package',
            sort_order=1
        )
    ...
    def test_package_vendor_ordering(self, vendor_factory, package_vendor_factory):
        ...
        pv3 = package_vendor_factory(vendor=vendor3, package=package, sort_order=1)
        pv1 = package_vendor_factory(vendor=vendor1, package=package, sort_order=0)
        pv2 = package_vendor_factory(vendor=vendor2, package=package, sort_order=0)
...
class TestVendorServiceCategoryChoices:
    ...
    def test_all_service_categories(self, vendor_factory):
        ...
        categories = [
            'CATERING', 'PHOTOGRAPHY', 'VIDEOGRAPHY', 'DJ', 'FLORIST',
            'DECORATOR', 'ENTERTAINMENT', 'TRANSPORTATION', 'MAKEUP',
            'RENTALS', 'OFFICIANT', 'COORDINATION', 'OTHER'
        ]

        for i, category in enumerate(categories):
            vendor = vendor_factory(
                name=f'{category} Vendor {i}',
                code=f'VENDOR_{category}_{i}',
                service_category=category
            )
            assert vendor.service_category == category
