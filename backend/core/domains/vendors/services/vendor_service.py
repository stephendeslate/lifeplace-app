...
from ..models import Vendor, PackageVendor
...
class VendorService:
    ...
    @staticmethod
    def get_vendor_by_code(code: str) -> Optional[Vendor]:
    ...
    def get_package_vendors(package_id: int) -> QuerySet[PackageVendor]:
        ...
        return PackageVendor.objects.filter(
            package_id=package_id
        ).select_related(
            'vendor',
            'vendor__vendor_operating_rules'
    ...
    def get_vendors_by_category(category: str) -> QuerySet[Vendor]:
        ...
        return Vendor.objects.filter(
            is_active=True,
            is_bookable=True,
            service_category=category.upper()
        ).order_by('sort_order', 'name')
    ...
    def get_packages_for_vendor(vendor_id: int) -> QuerySet[PackageVendor]:
        ...
        ).select_related('package').order_by('package__name')
