...
from .models import AffectedUser, SecurityBreach
...
class SecurityBreachViewSet(viewsets.ModelViewSet):
    ...
    def notify_npc(self, request, pk=None):
        ...
        if not serializer.validated_data["confirm"]:
            return Response(
                {"error": "Confirmation required to send NPC notification"}, status=status.HTTP_400_BAD_REQUEST
            )

        if breach.npc_notified:
            return Response(
                {"error": "NPC has already been notified for this breach"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def notify_users(self, request, pk=None):
        ...
        if not serializer.validated_data["confirm"]:
            return Response(
                {"error": "Confirmation required to send user notifications"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Get count of users to notify
        pending_count = AffectedUser.objects.filter(breach=breach, notified=False).count()

        if pending_count == 0:
            return Response({"error": "No users pending notification"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def timeline(self, request, pk=None):
        ...
        if breach.detected_at:
            timeline.append(
                {"event": "Breach Detected", "timestamp": breach.detected_at.isoformat(), "hours_elapsed": 0}
            )
            ...
            timeline.append(
                {
                    ...
                    "hours_elapsed": round((breach.confirmed_at - breach.detected_at).total_seconds() / 3600, 1),
                }
            )
            ...
            timeline.append(
                {
                    ...
                    "hours_elapsed": round((breach.npc_notified_at - breach.detected_at).total_seconds() / 3600, 1),
                }
            )
            ...
            timeline.append(
                {
                    ...
                    "hours_elapsed": round((breach.users_notified_at - breach.detected_at).total_seconds() / 3600, 1),
                }
            )
            ...
            timeline.append(
                {
                    ...
                    "hours_elapsed": round((breach.contained_at - breach.detected_at).total_seconds() / 3600, 1),
                }
            )
            ...
            timeline.append(
                {
                    ...
                    "hours_elapsed": round((breach.resolved_at - breach.detected_at).total_seconds() / 3600, 1),
                }
            )
    ...
    def summary(self, request):
        ...
        active = SecurityBreach.objects.exclude(status__in=["RESOLVED", "FALSE_POSITIVE"]).count()

        overdue = sum(
            1
            for b in SecurityBreach.objects.exclude(status__in=["RESOLVED", "FALSE_POSITIVE"])
        ...
        for severity, _ in SecurityBreach.SEVERITY_CHOICES:
            by_severity[severity] = (
                SecurityBreach.objects.filter(severity=severity)
        ...
        for status_code, _ in SecurityBreach.STATUS_CHOICES:
            by_status[status_code] = SecurityBreach.objects.filter(status=status_code).count()
