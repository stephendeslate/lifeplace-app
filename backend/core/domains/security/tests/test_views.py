...
class TestSecurityBreachViewSetPermissions:
    ...
    def test_create_requires_admin(self, client_user_client, mocker):
        ...
        mocker.patch("core.domains.security.services.BreachNotificationService._send_internal_alert")
        data = {
            "title": "New Breach",
    ...
    def test_update_requires_admin(self, client_user_client):
        ...
        response = client_user_client.patch(f"/api/security/breaches/{breach.id}/", {"title": "Updated"})
        assert response.status_code == status.HTTP_403_FORBIDDEN
...
class TestSecurityBreachViewSetUpdate:
    ...
    def test_update_breach(self, admin_client):
        ...
        response = admin_client.patch(
            f"/api/security/breaches/{breach.id}/", {"title": "Updated Title", "status": "INVESTIGATING"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_partial_update_breach(self, admin_client):
        ...
        response = admin_client.patch(
            f"/api/security/breaches/{breach.id}/", {"containment_actions": "Disabled affected accounts"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestSecurityBreachViewSetNotifyNPC:
    ...
    def test_notify_npc_success(self, admin_client, settings, mocker):
        ...
        mock_service = mocker.patch("core.domains.security.views.BreachNotificationService.notify_npc")
        ...
        response = admin_client.post(
            f"/api/security/breaches/{breach.id}/notify-npc/", {"confirm": True}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_notify_npc_requires_confirmation(self, admin_client):
        ...
        response = admin_client.post(
            f"/api/security/breaches/{breach.id}/notify-npc/", {"confirm": False}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_notify_npc_handles_service_error(self, admin_client, mocker):
        ...
        mocker.patch(
            "core.domains.security.views.BreachNotificationService.notify_npc", side_effect=Exception("Service error")
        )
...
class TestSecurityBreachViewSetNotifyUsers:
    ...
    def test_notify_users_success(self, admin_client, user_factory, mocker):
        ...
        mock_service = mocker.patch("core.domains.security.views.BreachNotificationService.notify_affected_users")
        ...
        AffectedUser.objects.create(breach=breach, user=user, data_exposed=["email"], notified=False)

        response = admin_client.post(
            f"/api/security/breaches/{breach.id}/notify-users/", {"confirm": True}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.data["status"] == "success"
    ...
    def test_notify_users_requires_confirmation(self, admin_client, user_factory):
        ...
        response = admin_client.post(
            f"/api/security/breaches/{breach.id}/notify-users/", {"confirm": False}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Confirmation required" in response.data["error"]
    ...
    def test_notify_users_skips_already_notified(self, admin_client, user_factory, mocker):
        ...
        mocker.patch("core.domains.security.views.BreachNotificationService.notify_affected_users")
...
class TestSecurityBreachViewSetAssessImpact:
    ...
    def test_assess_impact_success(self, admin_client, user_factory, mocker):
        ...
        mock_service = mocker.patch("core.domains.security.views.BreachNotificationService.assess_impact")
        ...
        response = admin_client.post(
            ...
            {"user_ids": [user1.id, user2.id], "data_types": ["email", "phone"]},
            format="json",
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.data["status"] == "success"
    ...
    def test_assess_impact_requires_user_ids(self, admin_client):
        ...
        response = admin_client.post(
            f"/api/security/breaches/{breach.id}/assess-impact/", {"data_types": ["email"]}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestSecurityBreachViewSetTimeline:
    ...
    def test_timeline_hours_elapsed_calculation(self, admin_client):
        ...
        detected_event = next(e for e in response.data["timeline"] if e["event"] == "Breach Detected")
        confirmed_event = next(e for e in response.data["timeline"] if e["event"] == "Breach Confirmed")
...
class TestSecurityBreachViewSetDelete:
    ...
    def test_delete_cascades_to_related(self, admin_client, user_factory):
        ...
        BreachNotification.objects.create(
            breach=breach, notification_type="NPC_INITIAL", recipient="npc@gov.ph", content="Content"
        )
