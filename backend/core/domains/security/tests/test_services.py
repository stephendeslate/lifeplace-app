...
class TestBreachNotificationServiceNotifyAffectedUsers:
    ...
    def test_notify_affected_users_sends_emails(self, user_factory, settings):
        ...
        AffectedUser.objects.create(
            breach=breach, user=user1, data_exposed=['email']
        )
        AffectedUser.objects.create(
            breach=breach, user=user2, data_exposed=['email', 'phone']
        )
    ...
    def test_notify_affected_users_creates_notification_records(
        self, user_factory, settings
    ):
        ...
        AffectedUser.objects.create(
            breach=breach, user=user, data_exposed=['email']
        )

        BreachNotificationService.notify_affected_users(breach)

        notification = BreachNotification.objects.get(
            breach=breach, notification_type='USER_EMAIL'
        )
        assert notification.recipient == 'user@example.com'
        assert notification.delivery_status == 'SENT'

    def test_notify_affected_users_marks_users_as_notified(
        self, user_factory, settings
    ):
    ...
    def test_notify_affected_users_updates_breach_when_all_notified(
        self, user_factory, settings
    ):
    ...
    def test_notify_affected_users_skips_already_notified(
        self, user_factory, settings
    ):
        ...
        AffectedUser.objects.create(
            breach=breach, user=user2, data_exposed=['email'],
            notified=False
        )
    ...
    def test_notify_affected_users_content_includes_exposed_data(
        self, user_factory, settings
    ):
        ...
        AffectedUser.objects.create(
            breach=breach, user=user, data_exposed=['email', 'phone', 'address']
        )
    ...
    def test_notify_affected_users_handles_email_failure(
        self, user_factory, settings, mocker
    ):
