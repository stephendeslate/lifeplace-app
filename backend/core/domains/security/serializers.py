...
class BreachNotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'notification_type', 'recipient', 'sent_at',
...
class SecurityBreachListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'breach_id', 'title', 'breach_type', 'severity',
            'status', 'detected_at', 'affected_users_count',
            'involves_spi', 'npc_notified', 'hours_since_detection',
            'is_overdue'
        ]
...
class SecurityBreachDetailSerializer(serializers.ModelSerializer):
    ...
    incident_lead_name = serializers.CharField(
        source='incident_lead.get_full_name', read_only=True
    )

    class Meta:
        model = SecurityBreach
        fields = [
            'id', 'breach_id', 'title', 'description',
            'breach_type', 'severity', 'status',
            'detected_at', 'confirmed_at', 'contained_at', 'resolved_at',
            'affected_users_count', 'affected_records_count',
            'involves_spi', 'data_types_affected',
            'attack_vector', 'vulnerabilities_exploited',
            'containment_actions', 'remediation_steps', 'prevention_measures',
            'npc_notified', 'npc_notified_at', 'npc_reference_number',
            'users_notified', 'users_notified_at',
            'incident_lead', 'incident_lead_name',
            'hours_since_detection', 'is_overdue', 'requires_notification',
            'notifications', 'affected_users',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'breach_id', 'created_at', 'updated_at',
            'npc_notified_at', 'users_notified_at'
        ]
...
class SecurityBreachCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'detected_at', 'data_types_affected', 'involves_spi',
            'attack_vector', 'incident_lead'
        ]
...
class SecurityBreachUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SecurityBreach
        fields = [
            'title', 'description', 'breach_type', 'severity', 'status',
            'confirmed_at', 'contained_at', 'resolved_at',
            'affected_users_count', 'affected_records_count',
            'involves_spi', 'data_types_affected',
            'attack_vector', 'vulnerabilities_exploited',
            'containment_actions', 'remediation_steps', 'prevention_measures',
            'npc_reference_number', 'incident_lead'
        ]
...
class NotifyNPCSerializer(serializers.Serializer):
    ...
    confirm = serializers.BooleanField(
        help_text="Set to true to confirm sending notification"
    )
...
class NotifyUsersSerializer(serializers.Serializer):
    ...
    confirm = serializers.BooleanField(
        help_text="Set to true to confirm sending notifications"
    )
