...
def check_notification_deadlines():
    ...
    breaches = SecurityBreach.objects.filter(status__in=["DETECTED", "INVESTIGATING", "CONFIRMED"], npc_notified=False)
    ...
    for breach in breaches:
        ...
        if hours >= 72:
            ...
            send_deadline_alert.delay(str(breach.id), "OVERDUE: 72-hour notification deadline PASSED!", urgent=True)
        elif hours >= 48:
            # WARNING - 24 hours left
            send_deadline_alert.delay(str(breach.id), "WARNING: Only 24 hours until notification deadline", urgent=True)
        elif hours >= 24:
            # REMINDER - 48 hours left
            send_deadline_alert.delay(str(breach.id), "REMINDER: 48 hours until notification deadline", urgent=False)
...
def send_daily_breach_summary():
    ...
    active_breaches = SecurityBreach.objects.exclude(status__in=["RESOLVED", "FALSE_POSITIVE"]).order_by("-detected_at")
...
def cleanup_security_logs(self):
    ...
    from core.utils.security_logging import SecurityEvent, SecurityEventType, SecuritySeverity, security_logger
    ...
    try:
        ...
        records_to_delete = SecurityEvent.objects.filter(timestamp__lt=cutoff_date).count()
        ...
        while True:
            ...
            batch_ids = list(
                SecurityEvent.objects.filter(timestamp__lt=cutoff_date).values_list("id", flat=True)[:batch_size]
            )
        ...
        logger.error(f"❌ Security log cleanup failed: {e!s}")
...
def cleanup_expired_account_data(self):
    ...
    from core.utils.security_logging import SecurityEventType, SecuritySeverity, security_logger
    ...
    try:
        ...
        expired_users = User.objects.filter(is_active=False, email__startswith="deleted_", last_login__lt=cutoff_date)
        ...
        logger.error(f"❌ Account data cleanup failed: {e!s}")
        raise self.retry(exc=e)
...
def monitor_data_retention_compliance(self):
    ...
    try:
        ...
        overdue_security_logs = SecurityEvent.objects.filter(timestamp__lt=security_cutoff).count()

        overdue_accounts = User.objects.filter(
            is_active=False, email__startswith="deleted_", last_login__lt=account_cutoff
        ).count()
        ...
        approaching_security = SecurityEvent.objects.filter(
            timestamp__lt=(security_cutoff + warning_threshold), timestamp__gte=security_cutoff
        ).count()
        ...
        total_deleted_users = User.objects.filter(is_active=False, email__startswith="deleted_").count()
        ...
        compliance_report = {
            ...
            if (overdue_security_logs == 0 and overdue_accounts == 0)
            else "NON_COMPLIANT",
        }

        # Log compliance check
        severity = (
            SecuritySeverity.LOW if compliance_report["compliance_status"] == "COMPLIANT" else SecuritySeverity.MEDIUM
        )
        ...
        security_logger.log_event(
            ...
            severity=severity,
            details={"task": "monitor_data_retention_compliance", "report": compliance_report},
        )
...
{compliance_report}
        ...
        logger.error(f"❌ Data retention compliance check failed: {e!s}")
