...
class ContractTemplateService:
            ...
            if signature_placeholder in content:
                if role in signatures_by_role:
                    ...
                    if ContractTemplateService._validate_signature_data_uri(signature.signature_data):
                        ...
                        signature_html = '<span class="signature-pending">[ SIGNATURE DATA INVALID ]</span>'
                    content = content.replace(signature_placeholder, signature_html)
                else:
                    # Replace with pending text
                    pending_text = (
                        f'<span class="signature-pending">[ {role_display.upper()} SIGNATURE PENDING ]</span>'
                    )
    ...
    def render_contract_with_signatures(contract_id):
        ...
        try:
            contract = (
                EventContract.objects.select_related("event", "template")
                .prefetch_related("signatures", "event__client")
                .get(id=contract_id)
            )
        ...
        updated_content = ContractTemplateService.render_contract(
            contract.template.id, context_data, contract.signatures.all()
        )
...
class EventContractService:
    ...
    def create_contract_from_template(event_id, template_id, valid_until=None, context_data=None, contract_value=None):
        ...
        max_amendment = EventContract.objects.filter(event_id=event_id).aggregate(max_num=Max("amendment_number"))[
            "max_num"
        ]
    ...
    def update_contract(contract_id, contract_data):
        ...
        if contract.status == "SIGNED" and "content" in contract_data:
            raise ContractAlreadySigned(detail="Cannot update content of a signed contract. Use amendments instead.")
        ...
        with transaction.atomic():
            ...
            if reason:
                ContractNote.objects.create(
                    contract=contract, note=f"Contract voided. Reason: {reason}", category="GENERAL", is_internal=True
                )
...
class ContractSignatureService:
    ...
    def add_signature(contract_id, user_id, signature_data, role="CLIENT", **signature_details):
        ...
        if contract.status not in ["SENT", "PARTIALLY_SIGNED"]:
            raise InvalidContractStatus(detail=f"Contract is in {contract.status} status and cannot be signed")
        ...
        if ContractSignature.objects.filter(contract=contract, role=role).exists():
            raise SignatureAlreadyExists(detail=f"A signature for role '{role}' already exists")

        # Check if role is required for this contract
        required_roles = contract.template.get_signature_requirements()
        if role not in required_roles:
            raise InvalidSignatureRole(detail=f"Role '{role}' is not required for this contract")
    ...
    def remove_signature(signature_id):
        ...
        if contract.status == "SIGNED":
            raise InvalidContractStatus(detail="Cannot remove signature from a fully signed contract")
...
class ContractAmendmentService:
    ...
    def request_amendment(original_contract_id, amendment_data, requested_by):
        ...
        if not original_contract.can_be_amended():
            raise AmendmentNotAllowed(detail="This contract cannot be amended")
    ...
    def create_amendment_contract(amendment_id, context_data=None):
        ...
        with transaction.atomic():
            ...
            rendered_content = ContractTemplateService.render_contract(original_contract.template.id, context_data)

            # Get next amendment number
            max_amendment = (
                EventContract.objects.filter(event=original_contract.event).aggregate(
...
class ContractDocumentService:
    ...
    def add_document(contract_id, document_data, uploaded_by):
        ...
        existing_doc = ContractDocument.objects.filter(
            contract=contract, name=document_data["name"], is_active=True
        ).first()
