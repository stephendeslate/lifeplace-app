...
class ContractTemplate(BaseModel):
    ...
    is_active = models.BooleanField(
        default=True, help_text="Inactive templates are hidden from selection but preserved for historical records"
    )

    # Multi-party signature configuration
    signature_requirements = models.JSONField(
        default=list, help_text="List of required signature roles: ['CLIENT', 'WITNESS', 'COMPANY_REP']"
    )
...
class EventContract(BaseModel):
    ...
    contract_value = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, help_text="Total contract value"
    )
    ...
    original_contract = models.ForeignKey(
        "self", on_delete=models.CASCADE, null=True, blank=True, related_name="amendments"
    )
        ...
        if self.is_fully_signed():
            if self.status != "SIGNED":
                ...
                if not was_signed and hasattr(self.event, "workflow_template") and self.event.workflow_template:
                    ...
                    logger.info(
                        f"Contract {self.id} fully signed - triggering workflow progression for event {self.event.id}"
                    )

                    WorkflowEngine.progress_workflow(
                        event=self.event,
                        trigger_type="CONTRACT_SIGNED",
                        data={"contract_id": self.id, "signed_at": str(self.fully_signed_at)},
                    )
    ...
    def can_be_amended(self):
        ...
        return self.template.allows_amendments and self.status == "SIGNED" and not self.is_amendment
...
class ContractSignature(BaseModel):
    ...
    verification_method = models.CharField(max_length=50, blank=True, help_text="Method used to verify signer identity")

    # Enhanced security fields
    device_fingerprint = models.TextField(blank=True, help_text="Device identification for security tracking")
    signature_metadata = models.JSONField(default=dict, help_text="Additional metadata about the signing process")
    ...
    legal_disclosure_accepted = models.BooleanField(
        default=False, help_text="Whether signer accepted electronic signature disclosure"
    )
    electronic_consent_timestamp = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp when electronic consent was given"
    )
    signature_intent_confirmed = models.BooleanField(
        default=False, help_text="Whether signer confirmed intent to sign electronically"
    )
...
class ContractAmendment(BaseModel):
    ...
    original_contract = models.ForeignKey(EventContract, on_delete=models.CASCADE, related_name="amendment_requests")
    amendment_contract = models.OneToOneField(
        EventContract, on_delete=models.CASCADE, null=True, blank=True, related_name="amendment_details"
    )
    ...
    requested_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, related_name="requested_amendments"
    )
    requested_at = models.DateTimeField(auto_now_add=True)
    reviewed_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_amendments"
    )
