...
class TestContractTemplateSerializer:
    ...
    def test_template_serialization(self, contract_template_factory):
        ...
        template = contract_template_factory(name="Wedding Contract", description="Contract for wedding events")
...
class TestEventContractSerializer:
    ...
    def test_contract_detail_serialization(self, event_contract_factory, contract_signature_factory):
        ...
        """
        contract = event_contract_factory(status="SENT", contract_value=Decimal("50000.00"), currency="PHP")
    ...
    def test_contract_is_expired_calculation(self, event_contract_factory):
        ...
        # Expired contract
        expired_contract = event_contract_factory(status="SENT", valid_until=date.today() - timedelta(days=1))
    ...
    def test_contract_expiry_urgency(self, event_contract_factory):
        ...
        contract = event_contract_factory(status="SENT", valid_until=date.today() + timedelta(days=1))
        serializer = EventContractSerializer(contract)
...
class TestContractSignatureSerializer:
    ...
    def test_signature_create_validation(self, event_contract_factory, contract_template_factory, user_factory):
...
class TestContractDocumentSerializer:
    ...
    def test_document_serialization(self, contract_document_factory, user_factory):
        ...
        document = contract_document_factory(
            name="Schedule A", description="Payment schedule", document_type="SCHEDULE", uploaded_by=user
        )
...
class TestContractNoteSerializer:
    ...
    def test_note_serialization(self, contract_note_factory, user_factory):
        ...
        user = user_factory()
        note = contract_note_factory(note="Important note", category="LEGAL", is_internal=True, created_by=user)
...
class TestContractSerializerMobileCompatibility:
    ...
    def test_contract_signed_at_alias(self, event_contract_factory):
        ...
        contract = event_contract_factory(status="SIGNED", fully_signed_at=signed_time)
        serializer = EventContractSerializer(contract)
