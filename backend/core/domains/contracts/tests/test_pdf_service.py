...
def mock_branding(mocker):
    ...
    mock_service = mocker.patch("core.domains.contracts.pdf_service.PDFBrandingService.get_branding_context")
...
class TestContractPDFService:
    ...
    def test_generate_contract_pdf_with_signatures(
        self, mock_branding, event_contract_factory, contract_template_factory, contract_signature_factory
    ):
        ...
        contract = event_contract_factory(template=template, status="SIGNED", content="Contract with signatures.")
        # Add signatures
        contract_signature_factory(
            contract=contract, role="CLIENT", signer_name="John Doe", signer_email="john@example.com", is_verified=True
        )
        contract_signature_factory(
            contract=contract,
            role="COMPANY_REP",
    ...
    def test_generate_contract_pdf_with_fully_signed_at(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Fully signed contract content.", fully_signed_at=timezone.now()
        )
    ...
    def test_generate_signature_summary_pdf(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Contract with signature summary."
        )
...
class TestPDFGeneration:
    ...
    def test_pdf_generation_with_special_characters(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Contract with special chars: $100,000.00 & more."
        )
    ...
    @patch("core.domains.contracts.pdf_service.PDFBrandingService.get_branding_context")
    def test_pdf_generation_with_long_content(self, mock_branding, event_contract_factory, contract_template_factory):
        ...
        long_content = "\n\n".join(
            [f"Paragraph {i}: " + "Lorem ipsum dolor sit amet, consectetur adipiscing elit. " * 10 for i in range(20)]
        )
        contract = event_contract_factory(template=template, status="SIGNED", content=long_content)
    ...
    def test_pdf_generation_with_no_contract_value(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Contract with no value specified.", contract_value=None
        )
...
class TestPDFErrors:
    ...
    def test_pdf_generation_raises_on_build_error(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Contract content for error test."
        )
    ...
    def test_pdf_generation_logs_error(
        ...
        template = contract_template_factory()
        contract = event_contract_factory(
            template=template, status="SIGNED", content="Contract content for error logging test."
        )
