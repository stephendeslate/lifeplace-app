...
class TestContractTemplateService:
    ...
    def test_get_all_templates_by_event_type(
        self, contract_template_factory, event_type_factory
    ):
    ...
    def test_render_contract(self, contract_template_factory):
        ...
        template = contract_template_factory(
            content='Hello {{ client_name }}, your event is on {{ event_date }}.'
        )
        context_data = {
    ...
    def test_render_contract_with_signatures(
        ...
        rendered = ContractTemplateService.render_contract(
            template.id,
            {},
    ...
    def test_preview_template_with_event(
        self, contract_template_factory, event_factory
    ):
        """Test previewing template with event context."""
        template = contract_template_factory(
            content='Event: {{ event_name }} for {{ client_name }}'
        )
        event = event_factory(name='Wedding Celebration')

        with patch.object(
            ContractTemplateService, 'render_contract',
            return_value='Event: Wedding Celebration for Test Client'
        ):
            preview = ContractTemplateService.preview_template(
                template.id,
                {},
                event_id=event.id
            )
...
class TestEventContractService:
    ...
    def test_create_contract_from_template(
        self, event_factory, contract_template_factory
    ):
        """Test creating a contract from a template."""
        event = event_factory(name='Test Event')
        template = contract_template_factory(
            content='Contract for {{ event_name }}'
        )
    ...
    def test_create_contract_template_not_found(self, event_factory):
        ...
        with pytest.raises(ContractTemplateNotFound):
            EventContractService.create_contract_from_template(
                event_id=event.id,
                template_id=99999
            )
    ...
    def test_update_contract(self, event_contract_factory):
        ...
        updated = EventContractService.update_contract(
            contract.id,
            {'status': 'SENT'}
        )
    ...
    def test_update_signed_contract_content_fails(self, event_contract_factory):
        ...
        with pytest.raises(ContractAlreadySigned):
            EventContractService.update_contract(
                contract.id,
                {'content': 'New content'}
            )
    ...
    def test_void_contract(self, event_contract_factory):
        ...
        voided = EventContractService.void_contract(
            contract.id,
...
class TestContractSignatureService:
    ...
    def test_add_signature_invalid_status(
        self, event_contract_factory, user_factory
    ):
    ...
    def test_add_signature_expired_contract(
        self, event_contract_factory, user_factory
    ):
    ...
    def test_add_signature_without_data_fails(
        self, event_contract_factory, user_factory
    ):
    ...
    def test_add_duplicate_role_signature_fails(
    ...
    def test_add_signature_invalid_role(
        self, event_contract_factory, contract_template_factory, user_factory
    ):
    ...
    def test_get_signatures_for_contract(
    ...
    def test_verify_signature(self, contract_signature_factory):
        ...
        verified = ContractSignatureService.verify_signature(
            signature.id,
    ...
    def test_remove_signature_from_signed_contract_fails(
        self, event_contract_factory, contract_signature_factory
    ):
...
class TestContractAmendmentService:
    ...
    def test_request_amendment_not_allowed(
        self, event_contract_factory, user_factory
    ):
        ...
        with pytest.raises(AmendmentNotAllowed):
            ContractAmendmentService.request_amendment(
                original_contract_id=contract.id,
                amendment_data={'amendment_reason': 'Test'},
                requested_by=user
            )
    ...
    def test_approve_amendment_wrong_status(
        self, contract_amendment_factory, user_factory
    ):
        ...
        with pytest.raises(ValueError):
            ContractAmendmentService.approve_amendment(
                amendment.id,
                reviewed_by=reviewer
            )
    ...
    def test_create_amendment_contract(
        ...
        template = contract_template_factory(
            content='Amendment contract content',
            allows_amendments=True
        )
        original = event_contract_factory(
            status='SIGNED',
            template=template,
            is_amendment=False,
            amendment_number=0
...
class TestContractDocumentService:
    ...
    def test_get_documents_for_contract(
        self, event_contract_factory, contract_document_factory
    ):
    ...
    def test_deactivate_document(self, contract_document_factory, user_factory):
        ...
        deactivated = ContractDocumentService.deactivate_document(
            document.id,
            deactivated_by=user
        )
...
class TestContractNoteService:
    ...
    def test_get_notes_for_contract(
        self, event_contract_factory, contract_note_factory
    ):
        ...
        all_notes = ContractNoteService.get_notes_for_contract(
            contract.id,
            include_internal=True
        )
        external_only = ContractNoteService.get_notes_for_contract(
            contract.id,
    ...
    def test_update_note(self, contract_note_factory, user_factory):
        ...
        updated = ContractNoteService.update_note(
            note.id,
            {'note': 'Updated note', 'category': 'LEGAL'},
            updated_by=user
        )
    ...
    def test_update_note_not_owner_fails(
        self, contract_note_factory, user_factory
    ):
        ...
        with pytest.raises(PermissionError):
            ContractNoteService.update_note(
                note.id,
...
class TestContractReportingService:
    ...
    def test_get_pending_signatures(
        self, event_contract_factory, contract_template_factory
    ):
