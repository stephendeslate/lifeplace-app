    ...
    def handle(self, *args, **options):
        ...
        if contract_id:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Successfully refreshed contract {contract_id}')
                )
            except EventContract.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f'Contract with ID {contract_id} not found')
                )
        elif template_id:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully refreshed {contracts.count()} contracts')
            )
        else:
