...
class ContractContextService:
    ...
    @staticmethod
    def generate_event_context(event: Event, additional_context: dict[str, Any] | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            context = {
                ...
                "payment_status_display": dict(Event.PAYMENT_STATUS_CHOICES).get(
                    event.payment_status, event.payment_status
                ),
    ...
    def generate_full_context(
        event: Event, contract=None, additional_context: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def _get_client_full_name(client: User | None) -> str:
    ...
    @staticmethod
    def _get_client_phone(client: User | None) -> str:
    ...
    @staticmethod
    def _get_client_company(client: User | None) -> str:
    ...
    @staticmethod
    def _get_client_address(client: User | None) -> str:
    ...
    def _get_currency_formatted_amounts(event: Event) -> dict[str, str]:
        ...
        def format_currency(amount):
            ...
            if decimal_places == 0:
                formatted_amount = (
                    f"{amount_float:,.0f}".replace(",", "|").replace(".", decimal_sep).replace("|", thousands_sep)
                )
            else:
                formatted_amount = (
                    f"{amount_float:,.{decimal_places}f}".replace(",", "|")
                    .replace(".", decimal_sep)
            ...
            if display_format == "symbol":  # noqa: SIM116
