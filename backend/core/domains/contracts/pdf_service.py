...
class ContractPDFService:
    ...
    def generate_contract_pdf(contract):
        ...
        try:
            doc.build(
                story, onFirstPage=ContractPDFService._add_watermark, onLaterPages=ContractPDFService._add_watermark
            )
            ...
            logger.error(f"Error generating PDF for contract {contract.id}: {e!s}")
            raise
    ...
    def _add_watermark(canvas, doc):
        ...
        canvas.drawRightString(
            doc.width + doc.rightMargin - 10, doc.bottomMargin - 20, f"Page {canvas.getPageNumber()}"
        )
    ...
    def generate_signature_summary_pdf(contract):
        ...
        title_style = ParagraphStyle(
            "Title", parent=styles["Heading1"], fontSize=16, spaceAfter=30, alignment=TA_CENTER, textColor=primary_color
        )
        ...
        story.append(
            Paragraph(
                f"<b>Event:</b> {contract.event.name if hasattr(contract.event, 'name') else f'Event #{contract.event.id}'}",
                styles["Normal"],
            )
        )
        ...
        if contract.signatures.exists():
            for signature in contract.signatures.all():
                ...
                sig_data = [
                    ...
                    [
                        "User Agent:",
                        signature.user_agent[:50] + "..." if len(signature.user_agent) > 50 else signature.user_agent,
                    ],
                ]
        ...
        try:
            ...
            logger.error(f"Error generating signature summary PDF for contract {contract.id}: {e!s}")
            raise
