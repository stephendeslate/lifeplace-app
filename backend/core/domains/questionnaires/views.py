...
class EventQuestionnaireViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = EventQuestionnaire.objects.select_related(
            'event', 'event__client', 'event__event_type',
            'questionnaire', 'assigned_by', 'sent_by', 'workflow_stage'
        ).prefetch_related(
            'activities', 'questionnaire__fields'
        )
    ...
    def send(self, request, pk=None):
        ...
        if event_questionnaire.status == 'COMPLETE':
            return Response(
                {'detail': 'Cannot send a completed questionnaire.'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def send_reminder(self, request, pk=None):
        ...
        if event_questionnaire.status == 'COMPLETE':
            return Response(
                {'detail': 'Questionnaire is already complete'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def responses(self, request, pk=None):
        ...
        responses = QuestionnaireResponse.objects.filter(
            ...
            field__questionnaire=event_questionnaire.questionnaire
