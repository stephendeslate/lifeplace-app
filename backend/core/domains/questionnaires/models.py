...
class QuestionnaireField(BaseModel):
    ...
    description = models.TextField(blank=True, help_text="Optional helper text shown below the field to guide users")
    placeholder = models.CharField(
        max_length=255, blank=True, help_text="Placeholder text shown inside the input field"
    )
    ...
    max_file_size_mb = models.PositiveIntegerField(
        default=10, help_text="Maximum file size in MB (for file fields only)"
    )
    allowed_file_types = models.JSONField(
        default=list, blank=True, help_text="Allowed file extensions e.g., ['pdf', 'jpg', 'png'] (for file fields only)"
    )
    max_files = models.PositiveIntegerField(
        default=1, help_text="Maximum number of files allowed (for file fields only)"
    )
...
class EventQuestionnaire(BaseModel):
    ...
    event = models.ForeignKey("events.Event", on_delete=models.CASCADE, related_name="event_questionnaires")
    questionnaire = models.ForeignKey("Questionnaire", on_delete=models.PROTECT, related_name="event_assignments")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="PENDING")

    # Assignment tracking
    assigned_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="assigned_questionnaires"
    )

    # Send tracking
    sent_at = models.DateTimeField(null=True, blank=True)
    sent_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="sent_questionnaires"
    )
    ...
    def completion_stats(self):
        ...
        responses = QuestionnaireResponse.objects.filter(
            event=self.event, field__questionnaire=self.questionnaire
        ).exclude(value="")
        ...
        return {
            ...
            "required_completion_percentage": round(
                (required_answered / required_fields * 100) if required_fields > 0 else 100
            ),
        }
    ...
    def update_status_from_responses(self):
        ...
        if old_status != self.status:
            ...
            EventQuestionnaireActivity.objects.create(
                event_questionnaire=self, action=action, notes=f"Status changed from {old_status} to {self.status}"
            )
...
class EventQuestionnaireActivity(BaseModel):
    ...
    event_questionnaire = models.ForeignKey(EventQuestionnaire, on_delete=models.CASCADE, related_name="activities")
    action = models.CharField(max_length=50, choices=ACTION_CHOICES)
    action_by = models.ForeignKey("users.User", on_delete=models.SET_NULL, null=True, blank=True)
    notes = models.TextField(blank=True)
