...
class TestSendQuestionnaireReminder:
    ...
    def test_reminder_no_questionnaire(self, mocker, event_factory, user_factory, event_type_factory):
    ...
    def test_reminder_already_complete(
        ...
        questionnaire_response_factory,
    ):
    ...
    def test_reminder_sent_successfully(
        ...
        mock_notification = mocker.patch("core.domains.notifications.services.NotificationService.create_notification")

        client = user_factory(role="CLIENT")
        event_type = event_type_factory()
        event = event_factory(
            client=client, event_type=event_type, status="CONFIRMED", start_date=timezone.now() + timedelta(days=14)
        )
    ...
    def test_reminder_max_reminders_reached(
        ...
        mocker.patch("core.domains.questionnaires.tasks._get_reminder_count", return_value=MAX_REMINDERS_PER_EVENT)
...
class TestNotifyQuestionnaireCompleted:
    ...
    def test_completion_notification_sent(self, mocker, event_factory, user_factory):
        ...
        client = user_factory(role="CLIENT")
        event = event_factory(client=client, start_date=timezone.now() + timedelta(days=14))
...
class TestNotifyAdminQuestionnaireSubmission:
    ...
    def test_admin_notification_no_admin_users(self, mocker, event_factory, user_factory):
    ...
    def test_admin_notification_sent(self, mocker, event_factory, user_factory, event_type_factory):
        ...
        event = event_factory(client=client, event_type=event_type, start_date=timezone.now() + timedelta(days=14))
    ...
    def test_admin_notification_multiple_admins(self, mocker, event_factory, user_factory):
...
class TestScheduleQuestionnaireReminders:
    ...
    def test_schedule_upcoming_event_incomplete_questionnaire(
        ...
        mock_send_reminder = mocker.patch("core.domains.questionnaires.tasks.send_questionnaire_reminder.delay")

        client = user_factory(role="CLIENT")
        event_type = event_type_factory()
        # Event in 10 days - should trigger reminder (21 days check, reminder_count < 1)
        event = event_factory(
            client=client, event_type=event_type, status="CONFIRMED", start_date=timezone.now() + timedelta(days=10)
        )
    ...
    def test_schedule_event_too_far_away(
        ...
        event_factory(
            client=client, event_type=event_type, status="CONFIRMED", start_date=timezone.now() + timedelta(days=45)
        )
...
class TestCheckQuestionnaireCompletion:
    ...
    def test_check_completion_no_questionnaire(self, mocker, event_factory, event_type_factory):
    ...
    def test_check_completion_no_required_fields(
        self, mocker, event_factory, event_type_factory, questionnaire_factory, questionnaire_field_factory
    ):
    ...
    def test_check_completion_complete_triggers_notifications(
        ...
        mock_client_notify = mocker.patch("core.domains.questionnaires.tasks.notify_questionnaire_completed.delay")
...
class TestTaskRetryBehavior:
    ...
    def test_completion_notification_raises_on_exception(self, mocker, event_factory, user_factory):
