...
class QuestionnaireService:
    ...
    def reorder_questionnaires(order_mapping):
        ...
        logger.info("Reordered questionnaires")
...
class QuestionnaireFieldService:
    ...
    def create_field(questionnaire_id, field_data):
        ...
        if QuestionnaireField.objects.filter(questionnaire=questionnaire, name=field_data["name"]).exists():
        ...
        if "order" not in field_data:
            ...
            max_order = (
                QuestionnaireField.objects.filter(questionnaire=questionnaire).aggregate(models.Max("order"))[
                    "order__max"
                ]
                or 0
            )
        ...
        with transaction.atomic():
            field = QuestionnaireField.objects.create(questionnaire=questionnaire, **field_data)
    ...
    def update_field(field_id, field_data):
        ...
        if "name" in field_data and field_data["name"] != field.name:
            if QuestionnaireField.objects.filter(questionnaire=field.questionnaire, name=field_data["name"]).exists():
                raise DuplicateQuestionnaireField()
        ...
        with transaction.atomic():
            if order_changed:
                ...
                all_fields = (
                    QuestionnaireField.objects.filter(questionnaire=field.questionnaire)
                    .select_for_update()
                    .order_by("order")
                )
                ...
                logger.info("Assigned temporary high orders to all fields")
    ...
    def delete_field(field_id):
        ...
        with transaction.atomic():
            ...
            remaining_fields = (
                QuestionnaireField.objects.filter(questionnaire=questionnaire, order__gt=deleted_order)
                .select_for_update()
                .order_by("order")
            )
    ...
    def reorder_fields(questionnaire_id, order_mapping):
        ...
        # Get all fields for this questionnaire
        fields = QuestionnaireField.objects.filter(questionnaire=questionnaire).select_for_update()
...
class QuestionnaireResponseService:
        ...
        with transaction.atomic():
            ...
            for response_data in responses_data:
                ...
                response = QuestionnaireResponseService.create_response(
                    {"event_id": event_id, "field_id": field_id, "value": str(value)}
                )
    ...
    def sync_event_guest_count(event_id):
        ...
        guest_responses = (
            ...
            .filter(Q(field__type="guests") | Q(field__is_guest_count=True, field__type="number"))
