...
class QuestionnaireSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Questionnaire
        fields = ["id", "name", "event_type", "is_active", "order", "created_at", "updated_at", "fields_count"]
...
class QuestionnaireResponseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "event", "field", "field_name", "field_type", "value", "created_at", "updated_at"]
        read_only_fields = ["id", "created_at", "updated_at"]
...
class EventQuestionnaireResponsesSerializer(serializers.Serializer):
    ...
    responses = serializers.ListField(child=serializers.DictField(child=serializers.CharField()))
...
class EventQuestionnaireActivitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "action", "action_display", "action_by", "action_by_name", "notes", "created_at"]
...
class EventQuestionnaireSerializer(serializers.ModelSerializer):
    ...
    activities = EventQuestionnaireActivitySerializer(many=True, read_only=True)
    # Include questionnaire details for rendering the form
    questionnaire_detail = QuestionnaireDetailSerializer(source="questionnaire", read_only=True)
...
class EventQuestionnaireCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if EventQuestionnaire.objects.filter(event=data["event"], questionnaire=data["questionnaire"]).exists():
            raise serializers.ValidationError("This questionnaire is already assigned to this event.")

        # Validate questionnaire is active
        if not data["questionnaire"].is_active:
            raise serializers.ValidationError("Cannot assign an inactive questionnaire.")
