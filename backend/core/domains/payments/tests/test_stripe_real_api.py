...
class StripeRealAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        has_real_keys = (
            config
            and config.get("secret_key", "").startswith("sk_test_")
            and config.get("publishable_key", "").startswith("pk_test_")
        )
    ...
    def test_real_stripe_payment_processing(self):
        ...
        try:
            ...
            self.assertIn(
                intent.status,
                ["succeeded", "requires_action", "requires_source_action", "requires_payment_method", "processing"],
            )
        ...
        payload = json.dumps(
            {
                ...
                "data": {"object": {"id": "pi_real_test", "status": "succeeded", "amount": 500000, "currency": "php"}},
            }
        )
    ...
    def test_stripe_gateway_configuration_guide(self):
        ...
        try:
            ...
            if not config:
                ...
                print(
                    f"   - Publishable key: {'✅' if config.get('publishable_key', '').startswith('pk_test_') else '❌'}"
                )
