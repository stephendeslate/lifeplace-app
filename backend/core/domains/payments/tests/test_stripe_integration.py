...
class StripePaymentIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="test@example.com", first_name="Test", last_name="Customer", role="CLIENT"
        )
    ...
    def test_failed_stripe_payment_processing(self):
        ...
        try:
            ...
            if "No such payment_method" in str(e):
                ...
                try:
                    intent = stripe.PaymentIntent.create(
                        amount=100000, currency="php", payment_method_types=["card"], metadata={"test": "declined_card"}
                    )
        ...
        try:
            ...
            if intent.status == "succeeded":
                ...
                refund = stripe.Refund.create(
                    ...
                    metadata={"payment_id": str(payment.id), "refund_reason": "Partial service cancellation"},
                )
    ...
    @override_settings(
        STRIPE_SECRET_KEY=STRIPE_TEST_CONFIG["secret_key"], STRIPE_WEBHOOK_SECRET=STRIPE_TEST_CONFIG["webhook_secret"]
    )
    def test_stripe_webhook_processing(self):
        ...
        try:
            ...
            with patch("stripe.Webhook.construct_event") as mock_construct:
                ...
                # Process webhook
                result = PaymentGatewayService.process_webhook(webhook_payload, test_signature)
    ...
    def test_stripe_payment_with_3d_secure(self):
        ...
        try:
            ...
            intent = stripe.PaymentIntent.create(
                ...
                confirm=True,
                return_url="https://example.com/return",
                metadata={"payment_id": str(payment.id), "requires_3ds": "true"},
            )
            ...
            self.assertIn(intent.status, ["requires_action", "requires_source_action", "requires_confirmation"])
    ...
    def test_stripe_multiple_currency_support(self):
        ...
        for currency, stripe_amount, _formatted_amount in currencies_to_test:
    ...
    def test_stripe_api_key_validation(self):
        ...
        invalid_gateway = PaymentGateway.objects.create(
            ...
            code="stripe_invalid",
            is_active=True,
            config={"secret_key": "sk_test_invalid_key_123", "publishable_key": "pk_test_invalid_key_123"},
        )

        Payment.objects.create(
            event=self.event,
            payment_method=PaymentMethod.objects.create(
                gateway=invalid_gateway, user=self.user, token_reference="pm_test_invalid"
            ),
            amount=Decimal("1000.00"),
            currency="PHP",
            due_date=date.today() + timedelta(days=7),
        ...
        try:
            stripe.PaymentIntent.create(amount=100000, currency="php", payment_method="pm_card_visa")
