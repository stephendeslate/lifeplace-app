...
class CompletePaymentFlowTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.client_user = User.objects.create_user(
            email="client@example.com", first_name="Maria", last_name="Santos", role="CLIENT"
        )

        # Create event type and products
        self.event_type = EventType.objects.create(name="Wedding", description="Wedding celebrations")

        self.category = ProductCategory.objects.create(
            name="Wedding Packages", description="Complete wedding photography packages"
        )
        ...
        self.booking_flow = BookingFlow.objects.create(
            name="Premium Wedding Booking", event_type=self.event_type, is_active=True
        )
    ...
    def test_complete_booking_to_payment_flow(self, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
        ...
        event = BookingSessionService.complete_booking(str(session.session_id), completion_type="quote")
    ...
    @patch("stripe.Customer.create")
    @patch("stripe.Customer.list")
    def test_direct_full_payment_flow(self, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
    ...
    @patch("stripe.Customer.create")
    @patch("stripe.Customer.list")
    def test_payment_failure_and_retry(self, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
        ...
        success_transactions = PaymentTransaction.objects.filter(payment=payment, status="COMPLETED")
    ...
    def test_partial_refund_flow(self):
        ...
        refund = RefundService.create_refund(
            ...
            refund_data={"amount": str(refund_amount), "reason": "Partial service cancellation"},
            user=self.client_user,
        )
