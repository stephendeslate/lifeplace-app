...
from core.domains.payments.models import Payment, Refund
...
class TestCreateRefund:
    ...
    def test_create_refund_success(self, completed_payment, admin_user):
        ...
        refund_data = {
            'amount': str(completed_payment.amount / 2),
    ...
    def test_create_refund_exceeds_payment(self, completed_payment, admin_user):
        ...
        with pytest.raises(RefundExceedsPaymentException):
            RefundService.create_refund(
                payment_id=completed_payment.id,
                refund_data={'amount': excess_amount},
                user=admin_user
            )
    ...
    def test_create_partial_refunds_track_total(self, completed_payment, admin_user):
        ...
        RefundService.create_refund(
            payment_id=completed_payment.id,
            refund_data={'amount': first_amount},
            user=admin_user
        )
...
class TestStripeRefundIntegration:
    ...
    @pytest.fixture
    def payment_with_stripe_transaction(
        self, completed_payment, stripe_gateway
    ):
    ...
    @pytest.fixture
    def pending_stripe_refund(
        self, payment_with_stripe_transaction, admin_user
    ):
    ...
    def test_stripe_refund_success(
        ...
        result = RefundService.process_gateway_refund(
            pending_stripe_refund.id,
            'stripe'
        )
    ...
    def test_stripe_refund_failure(
        ...
        with pytest.raises(ValueError) as exc_info:
            RefundService.process_gateway_refund(
                pending_stripe_refund.id,
                'stripe'
            )
    ...
    def test_stripe_refund_no_transaction(
        self, completed_payment, admin_user
    ):
