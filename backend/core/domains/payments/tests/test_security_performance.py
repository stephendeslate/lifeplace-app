...
class PaymentSecurityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="security@test.com", first_name="Security", last_name="Customer", role="CLIENT"
        )
    ...
    def test_payment_method_token_security(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_test_secure_token_123456789", last_four="4242"
        )
    ...
    def test_gateway_config_encryption(self):
        ...
        encrypted_gateway = PaymentGateway.objects.create(
            name="Encrypted Gateway Test", code="encrypted_test", is_active=True, config=sensitive_config
        )
    ...
    def test_payment_amount_validation(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_amount_test"
        )

        # Test valid amounts
        valid_amounts = [Decimal("100.00"), Decimal("9999.99"), Decimal("1.00"), Decimal("50000.00")]
    ...
    def test_sql_injection_prevention(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_sql_test"
        )
    ...
    def test_xss_prevention_in_payment_data(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_xss_test"
        )
    ...
    def test_payment_method_pci_compliance(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference=secure_token, last_four="4242"
        )
    ...
    def test_payment_audit_trail(self):
        ...
        payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_audit_test"
        )
        ...
        with (
            ...
            with patch.object(PaymentTransaction.objects, "create", side_effect=patched_txn_create):
                PaymentGatewayService.process_payment(
                    payment.id, {"gateway_id": str(self.gateway.id), "payment_method": payment_method.id}, self.user
                )

        # Verify transaction audit trail
        txn = PaymentTransaction.objects.get(payment=payment, transaction_id="pi_audit_test")
...
class PaymentPerformanceTestCase(TransactionTestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(
            email="performance@test.com", first_name="Performance", last_name="Customer", role="CLIENT"
        )
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_performance_test"
        )
    ...
    def test_bulk_payment_query_performance(self):
        ...
        results = (
            Payment.objects.select_related("event", "payment_method", "payment_method__gateway")
            .filter(event=self.event, status="COMPLETED", amount__gte=Decimal("1000.00"))
    ...
    def test_payment_gateway_connection_pooling(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(payment_operation) for _ in range(num_operations)]
