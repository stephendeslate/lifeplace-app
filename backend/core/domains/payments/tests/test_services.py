...
class PaymentServiceTestCase(TestCase):
    ...
    def test_update_payment(self):
        ...
        updated = PaymentService.update_payment(
            payment.id,
            {'notes': 'Updated notes'},
            self.user
        )
    ...
    def test_process_gateway_payment(self):
        ...
        with patch('core.domains.payments.services.gateway_service.PaymentGatewayService.process_gateway_payment') as mock_process:
            mock_txn = Mock()
            mock_txn.status = 'COMPLETED'
            mock_process.return_value = mock_txn

            result = PaymentService.process_gateway_payment(
...
class PaymentGatewayServiceTestCase(TestCase):
    ...
    def test_stripe_payment_processing(self, mock_create, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
    ...
    @patch('stripe.Customer.list')
    @patch('stripe.PaymentIntent.create')
    def test_stripe_payment_failure(self, mock_create, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
