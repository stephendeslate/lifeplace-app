...
from core.domains.payments.models import Payment, PaymentGateway, PaymentMethod, PaymentSettings
...
class DepositCalculationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="deposit@test.com", first_name="Deposit", last_name="Customer", role="CLIENT"
        )

        self.event_type = EventType.objects.create(name="Wedding")

        self.booking_flow = BookingFlow.objects.create(
            name="Deposit Test Flow", event_type=self.event_type, is_active=True
        )

        self.payment_step = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="payment_info", order=4, is_enabled=True
        )
    ...
    def test_percentage_deposit_configuration(self):
        ...
        for total_amount, expected_deposit in test_cases:
            with self.subTest(total_amount=total_amount):
                calculated = (total_amount * settings.default_deposit_percentage / Decimal("100.00")).quantize(
                    Decimal("0.01")
                )
...
class DepositPaymentProcessingTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(
            email="deposit_payment@test.com", first_name="Deposit Payment", last_name="Customer", role="CLIENT"
        )
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_deposit_test"
        )
    ...
    def test_multiple_deposit_attempts(
        ...
        mock_create.side_effect = stripe.error.CardError(
            message="Card declined", param="payment_method", code="card_declined"
        )
...
class BookingFlowDepositTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(
            email="booking_deposit@test.com", first_name="Booking", last_name="Customer", role="CLIENT"
        )
        ...
        # Create booking flow
        self.booking_flow = BookingFlow.objects.create(
            name="Deposit Booking Flow", event_type=self.event_type, is_active=True
        )

        self.payment_step = BookingFlowStep.objects.create(
            booking_flow=self.booking_flow, step_type="payment_info", order=4, is_enabled=True, is_required=True
        )
    ...
    def test_deposit_refund_policy_integration(self):
        ...
        deposit_payment = Payment.objects.create(
            ...
            payment_method=PaymentMethod.objects.create(
                gateway=self.gateway, user=self.user, token_reference="pm_refund_test"
            ),
