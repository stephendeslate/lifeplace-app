...
class PaymentRefundTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="refund@test.com", first_name="Refund", last_name="Customer", role="CLIENT"
        )
        ...
        self.gateway, _ = PaymentGateway.objects.get_or_create(
            code="stripe", defaults={"name": "Stripe Test", "is_active": True}
        )
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_test_refund"
        )
    ...
    def test_full_refund_processing(self):
        ...
        refund = RefundService.create_refund(
            ...
            refund_data={"amount": "15000.00", "reason": "Customer cancellation - full refund"},
            user=self.user,
        )
    ...
    def test_partial_refund_processing(self):
        ...
        refund1 = RefundService.create_refund(
            payment_id=payment.id,
            refund_data={"amount": "8000.00", "reason": "Partial service reduction"},
            user=self.user,
        )
        ...
        refund2 = RefundService.create_refund(
            payment_id=payment.id,
            refund_data={"amount": "5000.00", "reason": "Additional refund request"},
            user=self.user,
        )
    ...
    def test_refund_exceeding_payment_amount(self):
        ...
        with self.assertRaises(RefundExceedsPaymentException):
            RefundService.create_refund(
                payment_id=payment.id,
                refund_data={"amount": "15000.00", "reason": "Invalid refund amount"},
                user=self.user,
            )
    ...
    def test_refund_failed_payment(self):
        ...
        with self.assertRaises(InvalidRefundStatusException):
            RefundService.create_refund(
                payment_id=payment.id,
                refund_data={"amount": "5000.00", "reason": "Refund failed payment"},
                user=self.user,
            )
...
class PaymentFailureTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(
            email="failure@test.com", first_name="Failure", last_name="Customer", role="CLIENT"
        )
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_test_failure"
        )
    ...
    def test_insufficient_funds_failure(
        ...
        mock_create.side_effect = stripe.error.CardError(
            message="Your card has insufficient funds.", param="payment_method", code="insufficient_funds"
        )
    ...
    def test_expired_card_failure(
        ...
        mock_create.side_effect = stripe.error.CardError(
            message="Your card has expired.", param="exp_year", code="expired_card"
        )
    ...
    def test_network_error_failure(
        ...
        mock_create.side_effect = stripe.error.APIConnectionError("Network communication with Stripe failed")
    ...
    @patch("stripe.Customer.list")
    @patch("stripe.PaymentIntent.create")
    def test_payment_retry_logic(self, mock_create, mock_cust_list, mock_cust_create, mock_pm_retrieve, mock_pm_attach):
        ...
        mock_create.side_effect = stripe.error.APIConnectionError("Temporary network error")
...
class ConcurrentPaymentTestCase(TransactionTestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(
            email="concurrent@test.com", first_name="Concurrent", last_name="Customer", role="CLIENT"
        )
        ...
        self.gateway = PaymentGateway.objects.create(
            name="Stripe Test", code="stripe", is_active=True, config={"secret_key": "sk_test_123", "test_mode": True}
        )

        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_concurrent_test"
        )
