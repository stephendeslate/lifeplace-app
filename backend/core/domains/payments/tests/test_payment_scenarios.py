...
from core.domains.payments.models import (
    ...
    PaymentPlan, PaymentInstallment, Invoice
)
...
class PaymentInstallmentTestCase(TestCase):
    """Test cases for payment installment plans"""
    ...
    def setUp(self):
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway,
            client=self.user,
            token='pm_installment_test'
        )
    
    def test_monthly_installment_plan_creation(self):
        """Test creating monthly installment plan"""
        plan = PaymentPlan.objects.create(
            event=self.event,
            total_amount=Decimal('60000.00'),  # ₱60,000
            down_payment_amount=Decimal('15000.00'),  # ₱15,000 down
            installment_frequency='MONTHLY',
            number_of_installments=3,  # 3 monthly payments
            currency='PHP'
        )
        ...
        installments = PaymentInstallment.objects.filter(payment_plan=plan).order_by('installment_number')
        self.assertEqual(installments.count(), 3)
        
        # Each installment: (60000 - 15000) / 3 = 15000
        ...
        for i, installment in enumerate(installments, 1):
            ...
            self.assertEqual(installment.amount, expected_amount)
            self.assertEqual(installment.status, 'PENDING')
            
            # Due dates should be monthly intervals
            expected_due = date.today() + timedelta(days=30 * i)
            self.assertEqual(installment.due_date, expected_due)
    
    def test_weekly_installment_plan_creation(self):
        """Test creating weekly installment plan"""
        plan = PaymentPlan.objects.create(
            event=self.event,
            total_amount=Decimal('20000.00'),
            down_payment_amount=Decimal('5000.00'),
            installment_frequency='WEEKLY',
            number_of_installments=5,  # 5 weekly payments
            currency='PHP'
        )
        ...
        # Each installment: (20000 - 5000) / 5 = 3000
        expected_amount = Decimal('3000.00')
        
        for i, installment in enumerate(installments, 1):
            self.assertEqual(installment.amount, expected_amount)
            
            # Due dates should be weekly intervals
            expected_due = date.today() + timedelta(weeks=i)
            self.assertEqual(installment.due_date, expected_due)
    
    def test_installment_payment_processing(self):
        """Test processing individual installment payments"""
        plan = PaymentPlan.objects.create(
            event=self.event,
        ...
        remaining_installments = PaymentInstallment.objects.filter(
            payment_plan=plan, status='PENDING'
        )
        self.assertEqual(remaining_installments.count(), 1)
    
    def test_overdue_installment_detection(self):
        """Test detection of overdue installments"""
        plan = PaymentPlan.objects.create(
            event=self.event,
        ...
        # Create overdue installment (due 10 days ago)
        ...
        # Create upcoming installment
        upcoming_installment = PaymentInstallment.objects.create(
            payment_plan=plan,
        ...
        self.assertEqual(overdue_installment.days_overdue, 10)
        
        self.assertFalse(upcoming_installment.is_overdue)
        self.assertEqual(upcoming_installment.days_overdue, 0)
        ...
        self.assertEqual(overdue_installments.count(), 1)
        self.assertEqual(overdue_installments.first(), overdue_installment)
    
    def test_installment_plan_completion(self):
        """Test completing all installments in payment plan"""
        plan = PaymentPlan.objects.create(
            event=self.event,
            total_amount=Decimal('25000.00'),
            down_payment_amount=Decimal('10000.00'),
            installment_frequency='MONTHLY',
            number_of_installments=1,  # Simple: down + 1 installment
            currency='PHP'
        )
        ...
        self.assertEqual(remaining_installments.count(), 0)
        
        # Verify all installments are paid
        all_installments = PaymentInstallment.objects.filter(payment_plan=plan)
        paid_installments = all_installments.filter(status='PAID')
        
        self.assertEqual(all_installments.count(), paid_installments.count())
    
    def test_installment_plan_cancellation(self):
        """Test cancelling installment plan with outstanding payments"""
        plan = PaymentPlan.objects.create(
            event=self.event,
            total_amount=Decimal('50000.00'),
            down_payment_amount=Decimal('15000.00'),
            installment_frequency='MONTHLY',
            number_of_installments=3,
            currency='PHP'
        )
        ...
        # Verify cancellation
        cancelled_installments = PaymentInstallment.objects.filter(
            payment_plan=plan, status='CANCELLED'
        )
        self.assertEqual(cancelled_installments.count(), 2)
        
        paid_installments = PaymentInstallment.objects.filter(
            payment_plan=plan, status='PAID'
        )
