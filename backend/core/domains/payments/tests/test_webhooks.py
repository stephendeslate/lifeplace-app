...
from core.domains.payments.models import Payment, PaymentGateway, PaymentMethod, PaymentTransaction
...
class StripeWebhookTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="webhook@test.com", first_name="Webhook", last_name="Customer", role="CLIENT"
        )
        ...
        self.payment_method = PaymentMethod.objects.create(
            gateway=self.gateway, user=self.user, token_reference="pm_webhook_test"
        )
    ...
    def test_payment_intent_succeeded_webhook(self):
        ...
        webhook_payload = {
            ...
            "data": {
                "object": {
                    ...
                    "currency": "php",
                    "status": "succeeded",
                    "metadata": {"payment_id": str(payment.id), "event_id": str(self.event.id)},
                    "charges": {
                        "data": [
                            {"id": "ch_webhook_test_123", "amount": 1500000, "currency": "php", "status": "succeeded"}
                        ]
                    },
                }
            },
        ...
        signature = self.create_webhook_signature(payload_json, self.gateway.config["webhook_secret"])
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_event.data.object.metadata = {"payment_id": str(payment.id), "event_id": str(self.event.id)}
            mock_construct.return_value = mock_event

            # Process webhook
            result = PaymentGatewayService.process_webhook(payload_json, signature, self.gateway)
    ...
    def test_charge_dispute_created_webhook(self):
        ...
        dispute_transaction = PaymentTransaction.objects.filter(
            payment=payment, transaction_type="DISPUTE", transaction_id="dp_dispute_test_789"
        ).first()
    ...
    def test_invoice_payment_succeeded_webhook(self):
        ...
        webhook_payload = {
            ...
            "data": {
                "object": {
                    ...
                    "metadata": {"payment_id": str(payment.id), "invoice_id": str(invoice.id)},
                }
            },
        }
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_event.data.object.metadata = {"payment_id": str(payment.id), "invoice_id": str(invoice.id)}
            mock_construct.return_value = mock_event
    ...
    def test_webhook_signature_verification_failure(self):
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_construct.side_effect = stripe.error.SignatureVerificationError(
                "Invalid signature", sig_header=invalid_signature
            )

            result = PaymentGatewayService.process_webhook(payload_json, invalid_signature, self.gateway)
    ...
    def test_webhook_duplicate_event_handling(self):
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            result1 = PaymentGatewayService.process_webhook(payload_json, signature, self.gateway)
            self.assertTrue(result1["success"])

            # Process same webhook again (duplicate)
            result2 = PaymentGatewayService.process_webhook(payload_json, signature, self.gateway)
        ...
        success_transactions = PaymentTransaction.objects.filter(payment=payment, status="SUCCESS")
    ...
    def test_webhook_unknown_event_type(self):
        ...
        webhook_payload = {
            ...
            "data": {"object": {"id": "obj_unknown_222"}},
        }
    ...
    def test_webhook_malformed_payload(self):
        ...
        result = PaymentGatewayService.process_webhook(malformed_payload, signature, self.gateway)
    ...
    def test_webhook_processing_with_missing_payment(self):
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_event.data.object.status = "succeeded"
            mock_event.data.object.metadata = {"payment_id": "99999", "event_id": str(self.event.id)}
            mock_construct.return_value = mock_event
...
class WebhookSecurityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Always update config to ensure test settings are applied
        self.gateway.config = {"webhook_secret": "whsec_security_test_secret_key", "test_mode": True}
        self.gateway.is_active = True
        self.gateway.save()
    ...
    def test_webhook_timestamp_validation(self):
        ...
        signature = hmac.new(
            self.gateway.config["webhook_secret"].encode("utf-8"), signed_payload.encode("utf-8"), hashlib.sha256
        ).hexdigest()
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_construct.side_effect = stripe.error.SignatureVerificationError(
                "Timestamp outside the tolerance zone", sig_header=signature_header
            )

            result = PaymentGatewayService.process_webhook(payload, signature_header, self.gateway)
    ...
    def test_webhook_signature_tampering_detection(self):
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            mock_construct.side_effect = stripe.error.SignatureVerificationError(
                "No signatures found matching the expected signature", sig_header=signature_header
            )
    ...
    def test_webhook_rate_limiting(self):
        ...
        def mock_process_with_rate_limit(*args, **kwargs):
            ...
            recent_attempts = [attempt for attempt in webhook_attempts if time.time() - attempt < 60]
        ...
        with patch(
            "core.domains.payments.services.payment_gateway_service.PaymentGatewayService._process_stripe_webhook",
            side_effect=mock_process_with_rate_limit,
        ):
            results = []

            for _i in range(7):  # Try 7 rapid webhooks
                result = PaymentGatewayService.process_webhook(payload, signature_header, self.gateway)
                results.append(result)

        # First 5 should succeed, last 2 should be rate limited
        successful_results = [r for r in results if r.get("success", False)]
        [r for r in results if "Rate limit exceeded" in r.get("error", "")]
    ...
    def test_webhook_idempotency_key_validation(self):
        ...
        def mock_idempotent_processing(*args, **kwargs):
            ...
            return {"success": True, "message": "Event processed successfully", "duplicate": False}
        ...
        with patch("stripe.Webhook.construct_event") as mock_construct:
            ...
            with patch(
                "core.domains.payments.services.payment_gateway_service.PaymentGatewayService._process_stripe_webhook",
                side_effect=mock_idempotent_processing,
            ):
                # First processing
                result1 = PaymentGatewayService.process_webhook(payload, signature_header, self.gateway)

                # Second processing (duplicate)
                result2 = PaymentGatewayService.process_webhook(payload, signature_header, self.gateway)
