...
class ClientPaymentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def download_receipt(self, request, pk=None):
        ...
        try:
            ...
            if payment.status != "COMPLETED":
                return Response(
                    {"detail": "Receipt is only available for completed payments"}, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def summary(self, request):
        ...
        total_paid = queryset.filter(status="COMPLETED").aggregate(total=models.Sum("amount"))["total"] or Decimal(
            "0.00"
        )
        ...
        unpaid_invoices = invoice_queryset.filter(status__in=["ISSUED", "PARTIALLY_PAID"])
        ...
        paid_subquery = (
            Payment.objects.filter(invoice=OuterRef("pk"), status="COMPLETED")
            .values("invoice")
            .annotate(paid_sum=models.Sum("amount"))
            .values("paid_sum")[:1]
        )
        ...
        total_pending = unpaid_invoices_with_balance.aggregate(total=models.Sum("calculated_remaining"))[
            "total"
        ] or Decimal("0.00")

        # Calculate total overdue (remaining balance on overdue invoices)
        total_overdue = unpaid_invoices_with_balance.filter(due_date__lt=timezone.now().date()).aggregate(
    ...
    def pay(self, request, pk=None):
        ...
        try:
            ...
            try:
                ...
                payment_result = InvoiceService.process_invoice_payment(invoice, payment_data, request.user)
                ...
                return Response(
                    {"success": False, "message": "Payment processing failed", "error": str(e)},
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class ClientPaymentMethodViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            queryset.select_related("user", "gateway").defer("gateway__config").order_by("-is_default", "-created_at")
        )
...
class ClientRefundViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("payment", "payment__event", "payment__event__client", "refunded_by")
