    ...
    def handle(self, *args, **options):
        if options['event_id']:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Successfully updated event {event.id}')
                )
            except Event.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f'Event {options["event_id"]} does not exist')
                )
        else:
            # Update all events
            ...
            for event in events:
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f'Failed to update event {event.id}: {e}')
                    )
            
            self.stdout.write(
                self.style.SUCCESS(f'Successfully updated {updated_count}/{total_events} events')
            )