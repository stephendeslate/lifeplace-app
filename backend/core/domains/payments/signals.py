...
def update_event_financial_totals(event):
    ...
    try:
        ...
        logger.info(
            f"Updated event {event.id} financials: paid={total_paid}, due={event.total_amount_due}, status={event.payment_status}"
        )
...
def _check_and_process_downpayment_received(event, total_paid, total_invoiced):
    ...
    try:
        ...
        if Decimal(str(total_paid)) >= required_amount:
            ...
            latest_payment = Payment.objects.filter(event=event, status="COMPLETED").order_by("-created_at").first()
            ...
            try:
                ...
                if session and session.booking_data:
                    ...
                    if reservation_token:
                        logger.info(
                            f"Found reservation_token {reservation_token} from booking session for event {event.id}"
                        )
            except Exception as e:
            ...
            result = DateBlockingService.atomic_process_downpayment_received(
                event, payment=latest_payment, reservation_token=reservation_token
            )
...
def _trigger_auto_refund_for_race_condition(event, error_message: str):
    ...
    try:
        ...
        logger.warning(f"Race condition detected for event {event.id}: {error_message}. Initiating auto-refund.")

        result = AutoRefundService.initiate_refund_for_race_condition(event)

        if result["success"]:
            logger.info(f"Auto-refund completed for event {event.id}: refunded {result['total_refunded']}")
        else:
            logger.error(f"Auto-refund failed for event {event.id}: {result['error']}")
...
def invalidate_payment_gateway_caches(sender, instance, **kwargs):
    ...
    try:
        ...
        payments_cache_service.invalidate_payment_gateway_caches(gateway_id=instance.id)
    ...
    try:
        ...
        if instance.payment:
            payments_cache_service.invalidate_payment_caches(
                ...
                client_id=getattr(instance.payment.event.client, "id", None)
                if instance.payment.event and hasattr(instance.payment.event, "client")
                else None,
            )
...
def invalidate_refund_caches(sender, instance, **kwargs):
    ...
    try:
        ...
        payments_cache_service.invalidate_refund_caches(
            refund_id=instance.id, payment_id=getattr(instance.payment, "id", None) if instance.payment else None
        )
        # Also invalidate related payment cache
        if instance.payment:
            payments_cache_service.invalidate_payment_caches(
