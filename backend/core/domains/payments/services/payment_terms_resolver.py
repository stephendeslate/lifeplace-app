...
class PaymentTermsResolver:
    ...
    def get_terms_for_flow(booking_flow_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            payment_step = BookingFlowStep.objects.filter(booking_flow=flow, step_type="payment_info").first()
    ...
    def get_terms_for_event(event_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            booking_session = BookingSession.objects.filter(created_event=event).select_related("booking_flow").first()

            if booking_session and booking_session.booking_flow:
                return PaymentTermsResolver.get_terms_for_flow(booking_session.booking_flow.id)
    ...
    def generate_terms_text(terms: dict[str, Any], currency_symbol: str = "₱") -> str:
        ...
        if deposit_type == "FIXED":
            lines.append(
                f"Reservation payment of {deposit_str} has been placed upon signing this agreement and is {refund_status} and {deduct_status}."
            )
        else:
            lines.append(
                f"A deposit of {deposit_str} of the Total Contract Price (TCP) is required upon signing and is {refund_status}."
            )

        # Full payment waiver
        if terms.get("deposit_waived_on_full_payment", True):
            lines.append(
                "If the Client pays the full amount of the TCP upon signing, the reservation fee will be waived."
            )
        ...
        if downpayment_pct and downpayment_pct > 0:
            lines.append(
                f"Settlement of {downpayment_pct:.0f}% of the TCP shall be paid within {downpayment_days} days to officially block the date."
            )
        ...
        if balance_due_type == "DAY_BEFORE":
            lines.append(
                f"Settlement of the remaining {remaining_pct:.0f}% of the TCP shall be paid a day before the event."
            )
        else:
            lines.append(
                f"Settlement of the remaining {remaining_pct:.0f}% of the TCP shall be paid {balance_due_days} days prior to the event."
            )
        ...
        if late_fee_enabled:
            ...
            if late_fee_type == "PERCENTAGE":
                ...
                if late_fee_pct and late_fee_pct > 0:
                    lines.append(
                        f"Late payments are subject to {late_fee_pct:.0f}% of the invoice amount as a penalty charge."
                    )
            else:
        ...
        if cancellation_fee and cancellation_fee > 0:
            lines.append(
                f"In case of Client cancellation, {cancellation_fee:.0f}% of the total payment (excluding the reservation fee) will be deducted as an administrative processing fee."
            )
        ...
        if security_enabled:
            ...
            if security_amount and security_amount > 0:
                if security_desc:
                    lines.append(
                        f"A security deposit of {currency_symbol}{security_amount:,.0f} {security_desc} is required. {refund_text}."
                    )
                else:
                    lines.append(
                        f"A security deposit of {currency_symbol}{security_amount:,.0f} upon check-in is required. {refund_text}."
                    )
