...
class PaymentMethodService:
    ...
    def create_payment_method(data, user):
        ...
        if not method_type or method_type not in [
            choice[0] for choice in PaymentMethod._meta.get_field("type").choices
        ]:
        ...
        if method_type == "CREDIT_CARD":
            ...
            logger.info(
                f"Credit card validation - last_four: '{last_four}', has_stripe_payment_method: '{has_stripe_payment_method}'"
            )
            ...
            if not has_stripe_payment_method and not last_four:
                logger.error("Validation failed - missing stripe_payment_method_id and last_four")
            ...
            if has_stripe_payment_method and not last_four:
                logger.info("Stripe payment method provided but no last_four - this is acceptable")
    ...
    def delete_payment_method(method_id, user):
        ...
        has_pending_payments = Payment.objects.filter(payment_method=payment_method, status="PENDING").exists()
