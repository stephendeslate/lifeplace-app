...
class PaymentEventPublisher:
    ...
    def publish_state_change(cls, payment, transition):
        ...
        try:
            ...
            payment_state_changed.send(sender=cls, payment=payment, transition=transition, event=state_change_event)
            ...
            if transition.to_state.value == "COMPLETED":
                ...
                payment_completed.send(sender=cls, payment=payment, transition=transition, event=completed_event)
                ...
                payment_failed.send(sender=cls, payment=payment, transition=transition, event=failed_event)
                ...
                payment_cancelled.send(sender=cls, payment=payment, transition=transition, event=cancelled_event)
                ...
                payment_refunded.send(sender=cls, payment=payment, transition=transition, event=refunded_event)
            ...
            logger.error(f"Failed to publish domain events for payment {payment.payment_number}: {e}", exc_info=True)
    ...
    def _store_and_queue_event(cls, event):
        ...
        try:
            ...
            logger.error(f"Failed to store and queue event {event.event_id}: {e}", exc_info=True)
...
class PaymentEventHandlers:
    ...
    def handle_state_change(cls, sender, payment, transition, event, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error handling payment state change for {payment.payment_number}: {e}", exc_info=True)
    ...
    def handle_payment_completed(cls, sender, payment, transition, event, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error handling payment completion for {payment.payment_number}: {e}", exc_info=True)
    ...
    def handle_payment_failed(cls, sender, payment, transition, event, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error handling payment failure for {payment.payment_number}: {e}", exc_info=True)
    ...
    def handle_payment_cancelled(cls, sender, payment, transition, event, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error handling payment cancellation for {payment.payment_number}: {e}", exc_info=True)
    ...
    def handle_payment_refunded(cls, sender, payment, transition, event, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error handling payment refund for {payment.payment_number}: {e}", exc_info=True)
        ...
        try:
            ...
            if hasattr(payment.event, "workflow_template") and payment.event.workflow_template:
                ...
                existing_triggers = WorkflowTrigger.objects.filter(
                    event=payment.event, trigger_type="PAYMENT_RECEIVED", processed=False
                )
                ...
                for existing_trigger in existing_triggers:
                    if existing_trigger.result_data.get("payment_id") == payment.id:
                        logger.debug(
                            f"Trigger for payment {payment.id} already exists (trigger {existing_trigger.id}) - skipping duplicate"
                        )
                        return
                ...
                progressed = WorkflowEngine.progress_workflow(
                    event=payment.event,
                    trigger_type="PAYMENT_RECEIVED",
                    data={"trigger_id": trigger.id, "payment_data": trigger.result_data},
                )
