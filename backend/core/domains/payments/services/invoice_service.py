    ...
    def process_invoice_payment(invoice, payment_data, user):
        ...
        try:
            ...
            if invoice.status not in ["ISSUED", "PARTIALLY_PAID"]:
                return {"success": False, "error": f"Cannot pay invoice with status {invoice.get_status_display()}"}
            ...
            if payment_data.get("is_manual", False):
                ...
                return {"success": True, "payment": payment, "message": "Manual payment recorded successfully"}
            ...
            return {"success": False, "error": str(e), "details": "Payment processing encountered an error"}
        ...
        try:
            ...
            if payment_type == "DEPOSIT":
                ...
                if not custom_amount:
                    return {"success": False, "error": "custom_amount is required when payment_type is CUSTOM"}
            ...
            try:
                ...
                return {"success": False, "error": f"Payment gateway {gateway_code} not found or inactive"}
            ...
            with transaction.atomic():
                ...
                existing_payment = Payment.objects.filter(invoice=invoice, status="PENDING").select_for_update().first()

                if existing_payment:
                    logger.info(
                        f"Reusing existing pending payment {existing_payment.payment_number} for invoice {invoice.invoice_id}"
                    )
                    ...
                    payment = Payment.objects.get(id=response.payment_id)
                    logger.info(
                        f"Created new pending payment {payment.payment_number} for invoice {invoice.invoice_id}"
                    )
                ...
                if gateway_code == "stripe":
                    ...
                    if not gateway.config or "secret_key" not in gateway.config:
                        return {"success": False, "error": "Stripe gateway not properly configured"}
                    ...
                    existing_transaction = PaymentTransaction.objects.filter(
                        payment=payment, gateway=gateway, status__in=["PROCESSING", "PENDING"]
                    ).first()

                    if existing_transaction:
                        # Reuse existing payment intent
                        logger.info(
                            f"Reusing existing transaction {existing_transaction.transaction_id} for payment {payment.payment_number}"
                        )
                        ...
                        try:
                            ...
                            intent_data = {
                                ...
                                ),  # Use remaining_amount for partial payment support
            ...
            return {"success": False, "error": str(e), "details": "Payment intent creation encountered an error"}
