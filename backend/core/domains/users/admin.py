...
from .models import AdminInvitation, ConsentRecord, PrivacyRequest, User, UserProfile
...
class PrivacyRequestAdmin(admin.ModelAdmin):
    list_display = ["id", "user_email", "request_type", "status", "created_at", "processed_at", "is_overdue_display"]
    ...
    fieldsets = (
        ("Request Information", {"fields": ("id", "user", "user_email", "request_type", "status")}),
        ("Request Details", {"fields": ("request_data",)}),
        ("Response", {"fields": ("response_data", "rejection_reason", "deletion_summary")}),
        ("Processing", {"fields": ("processed_at", "processed_by")}),
        ("Audit", {"fields": ("ip_address", "user_agent", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
