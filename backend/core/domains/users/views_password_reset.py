...
from core.utils.security_logging import SecurityEventType, security_logger
...
def request_password_reset(request):
    ...
    validation_result = validate_request_data(request.data, required_fields=["email"])
    ...
    try:
        ...
        with transaction.atomic():
            ...
            try:
                ...
                logger.error(f"Failed to send password reset email to {user.email}: {e!s}")
        ...
        logger.error(f"Error processing password reset request from IP {client_ip}: {e!s}")
        return Response(
            {"detail": "An error occurred. Please try again later."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def confirm_password_reset(request, token_id):
    ...
    # Validate request data
    validation_result = validate_request_data(request.data, required_fields=["password", "confirm_password"])
    ...
    try:
        with transaction.atomic():
            ...
            if not reset_token.is_valid():
                if reset_token.is_used:
                    ...
                    return Response(
                        {"detail": "This reset link has already been used"}, status=status.HTTP_400_BAD_REQUEST
                    )
                else:
            ...
            PasswordResetToken.objects.filter(user=user, is_used=False).exclude(id=reset_token.id).update(is_used=True)
        ...
        logger.error(f"Error confirming password reset from IP {client_ip}: {e!s}")
        return Response(
            {"detail": "An error occurred. Please try again later."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def validate_reset_token(request, token_id):
    ...
    try:
        ...
        if not reset_token.is_valid():
            if reset_token.is_used:
                return Response({"valid": False, "reason": "already_used"}, status=status.HTTP_200_OK)
            else:
