...
from core.domains.users.services import AdminInvitationService, UserService
...
class TestUserService:
    ...
    def test_create_user_duplicate_email_raises_error(self, user_factory):
        ...
        with pytest.raises(EmailAlreadyExists):
            UserService.create_user({"email": "existing@example.com", "password": "testpass123"})
    ...
    def test_update_user_profile(self, user_factory):
        ...
        UserService.update_user(user, {"profile": {"phone": "+9876543210", "company": "New Company"}})
...
class TestAdminInvitationService:
    ...
    def test_create_invitation_for_new_user(self, mock_send_email, user_factory):
        ...
        invitation = AdminInvitationService.create_invitation(
            email="newinvite@example.com", first_name="New", last_name="Invite", invited_by=admin
        )
    ...
    def test_create_invitation_for_existing_client(self, mock_send_email, user_factory):
        ...
        invitation = AdminInvitationService.create_invitation(
            email="client@example.com", first_name="Client", last_name="User", invited_by=admin
        )
    ...
    @patch.object(AdminInvitationService, "_send_invitation_email")
    def test_create_invitation_for_existing_admin_raises_error(self, mock_send_email, user_factory):
        """Test creating invitation for existing ADMIN raises UserAlreadyAdmin."""
        admin = user_factory(admin=True)
        user_factory(admin=True, email="existingadmin@example.com")

        with pytest.raises(UserAlreadyAdmin):
            AdminInvitationService.create_invitation(
                email="existingadmin@example.com", first_name="Existing", last_name="Admin", invited_by=admin
            )
    ...
    @patch.object(AdminInvitationService, "_send_invitation_email")
    def test_create_invitation_replaces_existing_pending(self, mock_send_email, user_factory, admin_invitation_factory):
        """Test creating invitation deletes existing pending invitation."""
        admin = user_factory(admin=True)
        # Create existing pending invitation
        old_invitation = admin_invitation_factory(email="invite@example.com", invited_by=admin)
        old_id = old_invitation.id

        # Create new invitation for same email
        new_invitation = AdminInvitationService.create_invitation(
            email="invite@example.com", first_name="New", last_name="Name", invited_by=admin
        )
    ...
    def test_accept_invitation_creates_new_admin(self, admin_invitation_factory):
        ...
        invitation = admin_invitation_factory(email="newinvite@example.com", first_name="New", last_name="Admin")

        user = AdminInvitationService.accept_invitation(invitation_id=invitation.id, password="newpassword123")
    ...
    def test_accept_upgrade_invitation(self, admin_invitation_factory, user_factory):
        ...
        client = user_factory(role="CLIENT", email="client@example.com")
        invitation = admin_invitation_factory(email="client@example.com", is_upgrade=True, user=client)
    ...
    def test_accept_expired_invitation_raises_error(self, admin_invitation_factory):
        ...
        with pytest.raises(InvitationExpired):
            AdminInvitationService.accept_invitation(invitation_id=invitation.id, password="password123")

    def test_accept_already_accepted_invitation_raises_error(self, admin_invitation_factory):
    ...
    def test_accept_nonexistent_invitation_raises_error(self):
        ...
        with pytest.raises(UserNotFound):
            AdminInvitationService.accept_invitation(invitation_id=fake_id, password="password123")
