...
class TestChangePassword:
    ...
    def test_change_password_weak_password(self, authenticated_client, user_factory):
        ...
        url = reverse("users:change_password")
        response = client.post(
            url, {"current_password": "OldPass123!", "new_password": "123", "confirm_password": "123"}, format="json"
        )
    ...
    def test_change_password_missing_fields(self, authenticated_client, user_factory):
        ...
        response = client.post(
            url, {"current_password": "testpass123", "confirm_password": "NewStrongPass123!"}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST

        # Missing confirm_password
        response = client.post(
            url, {"current_password": "testpass123", "new_password": "NewStrongPass123!"}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
