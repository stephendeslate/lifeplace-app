...
def google_login(request):
    ...
    if not settings.GOOGLE_OAUTH_CLIENT_ID:
        ...
        return Response({"error": "Google sign-in is not configured"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to log Google auth event: {log_error!s}")
        ...
        return Response(
            {"error": "An error occurred during Google sign-in"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
