...
class MessagingConsumer(AsyncWebsocketConsumer):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "connection_established", "thread_id": self.thread_id, "user_id": user.id}
                )
            )
    ...
    async def handle_send_message(self, data):
        ...
        try:
            ...
            message = await self.create_message(
                thread_id=self.thread_id, sender=self.user, content=content, is_internal_note=is_internal_note
            )
            ...
            await self.channel_layer.group_send(
                self.thread_group_name, {"type": "new_message", "message": message_data, "sender_id": self.user.id}
            )

        except Exception as e:
            await self.send_error(f"Error sending message: {e!s}")
    ...
    async def handle_mark_as_read(self, data):
        ...
        try:
            ...
            await self.send_error(f"Error marking message as read: {e!s}")
    ...
    async def new_message(self, event):
        ...
        if message_data.get("is_internal_note") and self.user.role != "ADMIN":
            return  # Don't send internal notes to non-admin users

        await self.send(
            text_data=json.dumps(
                {"type": "new_message", "message": message_data, "is_own_message": sender_id == self.user.id}
            )
        )
    ...
    async def thread_updated(self, event):
        ...
        await self.send(text_data=json.dumps({"type": "thread_updated", "thread_data": event["thread_data"]}))
    ...
    def create_message(self, thread_id, sender, content, is_internal_note=False):
        ...
        message = Message.objects.create(
            thread=thread, sender=sender, content=content, message_type="text", is_internal_note=is_internal_note
        )
...
class GlobalMessagingConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            await self.send(text_data=json.dumps({"type": "global_connection_established", "user_id": user.id}))
    ...
    async def unread_count_update(self, event):
        ...
        await self.send(text_data=json.dumps({"type": "unread_count_update", "unread_count": event["unread_count"]}))
