...
class MessageThreadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = MessageThread.objects.select_related("client", "event", "assigned_admin").prefetch_related(
    ...
    def messages(self, request, pk=None):
        ...
        if not permission.has_object_permission(request, self, thread):
            return Response(
                {"error": "You do not have permission to access this thread"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def assign(self, request, pk=None):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only admin users can assign threads"}, status=status.HTTP_403_FORBIDDEN)
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Message.objects.select_related("thread", "sender").prefetch_related("attachments")

        # Filter based on thread access
        if user.role == "CLIENT":
            # Clients can only see messages from their threads and non-internal notes
            queryset = queryset.filter(thread__client=user, is_internal_note=False)
        elif user.role == "ADMIN":
    ...
    def mark_as_read(self, request, pk=None):
        ...
        if not permission.has_object_permission(request, self, message):
            return Response(
                {"error": "You do not have permission to access this message"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def bulk_mark_as_read(self, request):
        ...
        return Response({"status": "success", "marked_read": marked_count, "total_requested": len(message_ids)})
...
class MessageThreadAdminViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return MessageThread.objects.select_related("client", "event", "assigned_admin").prefetch_related(
            "messages__sender"
        )
    ...
    def bulk_assign(self, request):
        ...
        if not thread_ids:
            return Response({"error": "thread_ids is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # Update threads
        updated_count = MessageThread.objects.filter(id__in=thread_ids).update(assigned_admin=admin_user)
    ...
    def bulk_update_status(self, request):
        ...
        if not thread_ids or not new_status:
            return Response({"error": "thread_ids and status are required"}, status=status.HTTP_400_BAD_REQUEST)

        # Validate status
        valid_statuses = [choice[0] for choice in MessageThread.STATUS_CHOICES]
        if new_status not in valid_statuses:
            return Response(
                {"error": f"Invalid status. Must be one of: {valid_statuses}"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Update threads
        updated_count = MessageThread.objects.filter(id__in=thread_ids).update(status=new_status)

        return Response({"status": "success", "updated_count": updated_count, "new_status": new_status})
    ...
    def stats(self, request):
        ...
        status_counts = dict(
            MessageThread.objects.values("status").annotate(count=Count("status")).values_list("status", "count")
        )
        stats["status_breakdown"] = status_counts

        # Priority breakdown
        priority_counts = dict(
            MessageThread.objects.values("priority").annotate(count=Count("priority")).values_list("priority", "count")
        )
...
class SupportInquiryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            MessageThread.objects.filter(client=self.request.user, thread_type="support")
            .select_related("event", "assigned_admin")
            .order_by("-created_at")
        )
    ...
    def add_reply(self, request, pk=None):
        ...
        message = Message.objects.create(
            thread=thread, sender=request.user, content=content, message_type="text", is_internal_note=False
        )
...
class AdminSupportInquiryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = MessageThread.objects.filter(thread_type="support").select_related(
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            "resolved_today": queryset.filter(status="resolved", updated_at__date=today).count(),
    ...
    def add_reply(self, request, pk=None):
        ...
        message = Message.objects.create(
            thread=thread, sender=request.user, content=content, message_type="text", is_internal_note=is_internal
        )
