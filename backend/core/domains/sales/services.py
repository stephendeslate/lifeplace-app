...
def get_default_tax_rate():
    """Get tax rate from system default — delegates to selectors."""
    from .selectors import get_default_tax_rate
    return get_default_tax_rate()


def get_tax_rate_for_product(product):
    """Get appropriate tax rate for a product — delegates to selectors."""
    from .selectors import get_tax_rate_for_product
    return get_tax_rate_for_product(product=product)
...
class QuoteTemplateService:
    ...
    def create_template(data, user):
        ...
        with transaction.atomic():
            ...
            for product_data in products:
                QuoteTemplateProduct.objects.create(template=template, **product_data)
    ...
    def add_product_to_template(template_id, product_data):
        ...
        if QuoteTemplateProduct.objects.filter(template=template, product_id=product_data["product"]).exists():
        ...
        product = QuoteTemplateProduct.objects.create(template=template, **product_data_copy)
        return product
...
class QuoteService:
    ...
    def create_quote(data, user):
        ...
        with transaction.atomic():
            ...
            max_version = (
                EventQuote.objects.filter(event=event).aggregate(max_version=models.Max("version"))["max_version"] or 0
            )
            ...
            # Record activity
            QuoteActivity.objects.create(quote=quote, action="CREATED", action_by=user, notes="Quote created")
            ...
            logger.info(
                f"Created quote {quote.id} for event {event.id}: subtotal=₱{quote.subtotal}, tax=₱{quote.tax_amount}, total=₱{quote.total_amount}"
            )
            return quote
    ...
    def update_quote(quote_id, data, user):
        ...
        if quote.status in ["ACCEPTED", "REJECTED"]:
            raise InvalidQuoteStatusTransition(f"Cannot update a quote with status {quote.status}")
        ...
        with transaction.atomic():
                ...
                if key == "status" and value != quote.status:
                    ...
                    if value == "SENT":
                        ...
                        QuoteActivity.objects.create(
                            quote=quote, action="SENT", action_by=user, notes="Quote sent to client"
                        )
                        ...
                        QuoteActivity.objects.create(
                            quote=quote, action="ACCEPTED", action_by=user, notes="Quote accepted"
                        )

                    elif value == "REJECTED":
                        quote.rejected_at = timezone.now()
                        QuoteActivity.objects.create(
                            quote=quote, action="REJECTED", action_by=user, notes="Quote rejected"
                        )
                ...
                for item_data in line_items_data:
                    ...
                    if item_id:
                        ...
                        try:
                            ...
                            should_recalculate = (product_id and product_id != line_item.product_id) or (
                                "quantity" in item_data
                                and item_data.get("quantity") != line_item.quantity
                                and (product_id or line_item.product_id)
                            )
                            ...
                            if should_recalculate:
                                ...
                                try:
                                    ...
                                    pricing_item = PricingCalculationService._create_package_line_item(
                                        package_data,
                                        None,  # No venue additional hours for admin quotes
                                    )

                                    if pricing_item:
                                        item_type = (
                                            "ADDON" if getattr(product, "type", "PACKAGE") == "ADDON" else "PACKAGE"
                                        )
            ...
            if changes and not any(change.startswith("Status changed") for change in changes):
                QuoteActivity.objects.create(
                    quote=quote, action="UPDATED", action_by=user, notes=f"Quote updated: {', '.join(changes)}"
                )
            ...
            logger.info(
                f"Updated quote {quote.id}: subtotal=₱{quote.subtotal}, tax=₱{quote.tax_amount}, total=₱{quote.total_amount}"
            )
            return quote
        ...
        with transaction.atomic():
            ...
            if product_id:
                ...
                try:
                    ...
                    pricing_item = PricingCalculationService._create_package_line_item(
                        package_data, venue_additional_hours if venue_additional_hours else None
                    )
                ...
                if "total" not in line_item_data:
                    line_item_data["total"] = Decimal(str(line_item_data["unit_price"])) * int(
                        line_item_data["quantity"]
                    )

                line_item = QuoteLineItem.objects.create(quote=quote, **line_item_data)

            # Record activity
            QuoteActivity.objects.create(
                quote=quote, action="UPDATED", action_by=user, notes=f"Added line item: {line_item.description}"
            )
    ...
    def update_line_item(line_item_id, line_item_data, user):
        ...
        with transaction.atomic():
            ...
            should_recalculate = (new_product_id and new_product_id != current_product_id) or (
            ...
            should_recalculate = should_recalculate or venue_hours_changed
            ...
            # Record activity
            QuoteActivity.objects.create(
                quote=quote, action="UPDATED", action_by=user, notes=f"Updated line item: {description}"
            )
    ...
    def remove_line_item(line_item_id, user):
        ...
        with transaction.atomic():
            ...
            breakdown = PricingCalculationService.calculate_from_booking_data(
                booking_data, client=None, venue_additional_hours=None, event_type_id=event_type_id
            )
            ...
            # Record activity
            QuoteActivity.objects.create(
                quote=quote, action="UPDATED", action_by=user, notes=f"Removed line item: {description}"
            )
