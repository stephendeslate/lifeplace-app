...
class EventQuoteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Quote Information", {"fields": ("event", "template", "version", "status", "created_by")}),
        ("Financial Details", {"fields": ("subtotal", "discount_amount", "tax_amount", "total_amount", "discount")}),
        ("Validity", {"fields": ("valid_until",)}),
        (
            "Status Tracking",
            {"fields": ("sent_at", "accepted_at", "rejected_at", "rejection_reason"), "classes": ("collapse",)},
        ),
        ("Content", {"fields": ("notes", "terms_and_conditions", "client_message"), "classes": ("collapse",)}),
        ("Files & Signature", {"fields": ("pdf_file", "signature_data"), "classes": ("collapse",)}),
    )
...
class QuoteTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Template Information", {"fields": ("name", "event_type", "is_active")}),
        ("Content", {"fields": ("introduction", "terms_and_conditions")}),
        ("Configuration", {"fields": ("default_validity_days", "has_multiple_options", "default_tax_rate")}),
        (
            "Related Templates",
            {"fields": ("contract_templates", "questionnaires", "workflow_template"), "classes": ("collapse",)},
        ),
    )
