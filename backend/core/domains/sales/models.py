...
class EventQuote(BaseModel):
    ...
    discount = models.ForeignKey(
        "products.Discount", on_delete=models.SET_NULL, null=True, blank=True, related_name="quotes"
    )
    ...
    def accept(self, signature_data=None):
        ...
        with transaction.atomic():
            ...
            if locked_quote.status != "SENT":
                raise ValueError(
                    f"Cannot accept quote with status '{locked_quote.status}'. Quote must be in SENT status."
                )

            # Check if quote has expired
            if locked_quote.valid_until and locked_quote.valid_until < timezone.now().date():
                raise ValueError(f"Cannot accept expired quote. This quote expired on {locked_quote.valid_until}.")
    ...
    def reject(self, reason=None):
        ...
        with transaction.atomic():
            ...
            if locked_quote.status != "SENT":
                raise ValueError(
                    f"Cannot reject quote with status '{locked_quote.status}'. Quote must be in SENT status."
                )
    ...
    def send_to_client(self, user=None):
        ...
        # Record activity
        QuoteActivity.objects.create(
            quote=self, action="SENT", action_by=user, notes=f"Quote sent to client {self.event.client}"
        )
        ...
        if self.valid_until:
            ...
            if reminder_date.date() < self.valid_until:
                QuoteReminder.objects.create(
                    quote=self, scheduled_date=reminder_date, message="Follow up on quote sent 3 days ago"
                )
...
class QuoteTemplate(BaseModel):
    ...
    workflow_template = models.ForeignKey(
        "workflows.WorkflowTemplate", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    def apply_to_event(self, event, created_by=None):
        ...
        # Record activity
        QuoteActivity.objects.create(
            quote=quote, action="CREATED", action_by=created_by, notes=f"Quote created from template {self.name}"
        )
...
class QuoteLineItem(BaseModel):
    ...
    metadata = models.JSONField(
        null=True, blank=True, help_text="Additional metadata: attendee_breakdown for per-person packages, etc."
    )
