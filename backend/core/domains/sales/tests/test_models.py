...
class TestQuoteTemplateModel:
    ...
    def test_template_apply_to_event(self, db, event_factory, user_factory, product_option, default_tax_rate):
        """Test applying a template to an event creates a quote."""
        event = event_factory()
        admin_user = user_factory(admin=True)

        template = QuoteTemplate.objects.create(
            name="Test Template", terms_and_conditions="Template T&C", is_active=True, default_validity_days=14
        )
    ...
    def test_template_apply_creates_line_items(self, db, event_factory, user_factory, product_option, default_tax_rate):
    ...
    def test_template_apply_calculates_totals(self, db, event_factory, user_factory, product_option, default_tax_rate):
        """Test applying template calculates quote totals."""
        event = event_factory()
        admin_user = user_factory(admin=True)

        template = QuoteTemplate.objects.create(
            name="Test Template", terms_and_conditions="", is_active=True, default_tax_rate=default_tax_rate
        )
    ...
    def test_template_apply_bounds_validity_by_event_date(self, db, event_factory, user_factory):
        """Test that quote validity is bounded by event date."""
        # Create event 10 days from now
        event = event_factory(start_date=timezone.now() + timedelta(days=10))
        admin_user = user_factory(admin=True)
...
class TestQuoteLineItemModel:
    ...
    def test_line_item_string_representation(self, quote):
        ...
        line_item = QuoteLineItem.objects.create(
            quote=quote, description="Venue Rental", quantity=1, unit_price=Decimal("5000.00"), total=Decimal("5000.00")
        )
    ...
    def test_line_item_auto_calculates_total(self, quote):
        ...
        line_item = QuoteLineItem(quote=quote, description="Test Item", quantity=3, unit_price=Decimal("1000.00"))
        line_item.save()
    ...
    def test_line_item_item_type_default(self, quote):
        ...
        line_item = QuoteLineItem.objects.create(
            quote=quote, description="Test Item", quantity=1, unit_price=Decimal("1000.00"), total=Decimal("1000.00")
        )
...
class TestQuoteOptionModel:
    ...
    def test_option_string_representation(self, quote):
        ...
        option = QuoteOption.objects.create(
            quote=quote, name="Basic Package", description="", total_price=Decimal("5000.00")
        )
    ...
    def test_option_calculate_total(self, quote, product_option):
        ...
        option = QuoteOption.objects.create(
            quote=quote, name="Test Option", description="", total_price=Decimal("0.00")
        )
...
class TestQuoteOptionItemModel:
    ...
    def test_option_item_auto_calculates_total(self, quote):
        ...
        item = QuoteOptionItem(option=option, description="Test Service", quantity=3, unit_price=Decimal("1500.00"))
        item.save()
    ...
    def test_option_item_save_updates_option_total(self, quote):
        ...
        QuoteOptionItem.objects.create(
            option=option, description="Item 1", quantity=1, unit_price=Decimal("1000.00"), total=Decimal("1000.00")
        )
...
class TestQuoteActivityModel:
    ...
    def test_create_activity(self, quote, user_factory):
        ...
        activity = QuoteActivity.objects.create(
            quote=quote, action="CREATED", action_by=user, notes="Quote created for client"
        )
    ...
    def test_activity_string_representation(self, quote):
        ...
        activity = QuoteActivity.objects.create(quote=quote, action="SENT", notes="Quote sent to client")
    ...
    def test_activity_ordering(self, quote, user_factory):
        ...
        act1 = QuoteActivity.objects.create(quote=quote, action="CREATED", action_by=user)
        act2 = QuoteActivity.objects.create(quote=quote, action="SENT", action_by=user)
...
class TestQuoteReminderModel:
    ...
    def test_create_reminder(self, quote):
        ...
        reminder = QuoteReminder.objects.create(
            quote=quote, scheduled_date=timezone.now() + timedelta(days=3), message="Follow up on quote"
        )
    ...
    def test_reminder_string_representation(self, quote):
        ...
        reminder = QuoteReminder.objects.create(quote=quote, scheduled_date=scheduled, message="Follow up")
    ...
    def test_reminder_ordering(self, quote):
        ...
        reminder1 = QuoteReminder.objects.create(
            quote=quote, scheduled_date=timezone.now() + timedelta(days=7), message="Later follow up"
        )
        reminder2 = QuoteReminder.objects.create(
            quote=quote, scheduled_date=timezone.now() + timedelta(days=3), message="Earlier follow up"
        )
...
class TestQuoteTemplateProductModel:
    ...
    def test_create_template_product(self, db, product_option):
        ...
        template_product = QuoteTemplateProduct.objects.create(
            template=template, product=product_option, quantity=2, is_required=True
        )
    ...
    def test_template_product_string_representation(self, db, product_option):
        ...
        template = QuoteTemplate.objects.create(name="Wedding Template", terms_and_conditions="", is_active=True)
    ...
    def test_template_product_unique_constraint(self, db, product_option):
        ...
        QuoteTemplateProduct.objects.create(template=template, product=product_option, quantity=1)
