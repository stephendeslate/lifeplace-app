...
from .models import EventWorkflowOverride, WorkflowStage, WorkflowTrigger
...
class WorkflowTemplateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            WorkflowTemplateService.get_all_templates(event_type_id=event_type_id, is_active=is_active)
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(WorkflowTemplateDetailSerializer(template).data, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            template = WorkflowTemplateService.update_template(instance.id, serializer.validated_data)
    ...
    def duplicate(self, request, pk=None):
        ...
        return Response(WorkflowTemplateDetailSerializer(new_template).data, status=status.HTTP_201_CREATED)
...
class WorkflowStageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return WorkflowStage.objects.select_related("template", "template__event_type", "email_template").order_by(
            "template", "stage", "order"
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            stage = WorkflowStageService.create_stage(template_id, validated_data)
    ...
    def update(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            stage = WorkflowStageService.update_stage(instance.id, validated_data)
    ...
    def trigger(self, request, pk=None):
        ...
        try:
            ...
            if event.workflow_template_id != stage.template_id:
                return Response(
                    {"detail": "Event does not use this workflow template"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            return Response(
                {"message": f"Successfully triggered automation for stage '{stage.name}'", "trigger_id": trigger.id}
            )
...
class WorkflowTriggerViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = WorkflowTrigger.objects.select_related("event", "stage", "stage__template")
...
class EventWorkflowOverrideViewSet(viewsets.ModelViewSet):
    ...
    def skip_stage(self, request):
        ...
        if not event_id or not stage_id:
            return Response({"detail": "event_id and stage_id are required"}, status=status.HTTP_400_BAD_REQUEST)
...
class WorkflowWebhookViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        template_id = self.request.query_params.get("workflow_template") or self.request.query_params.get(
    ...
    def get_serializer_class(self):
        from .serializers import WorkflowWebhookDetailSerializer, WorkflowWebhookSerializer
    ...
    def deliveries(self, request, pk=None):
        ...
        deliveries = WorkflowWebhookDelivery.objects.filter(webhook=webhook).order_by("-created_at")[:limit]
    ...
    def event_types(self, request):
        ...
        return Response([{"value": choice[0], "label": choice[1]} for choice in WorkflowWebhook.WEBHOOK_EVENT_CHOICES])
