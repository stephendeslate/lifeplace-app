...
class WorkflowTemplateService:
    ...
    def duplicate_template(template_id, new_name=None):
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Duplicated workflow template '{original.name}' to '{new_template.name}' with {original.stages.count()} stages"
            )
            return new_template
    ...
    def _create_stage(template, stage_data):
        ...
        same_stage_order = WorkflowStage.objects.filter(
            template=template, stage=stage_data["stage"], order=stage_data["order"]
        ).exists()
...
class WorkflowStageService:
    ...
    def create_stage(template_id, stage_data):
        ...
        if "order" not in stage_data:
            ...
            max_order = (
                WorkflowStage.objects.filter(template=template, stage=stage_data["stage"]).aggregate(Max("order"))[
                    "order__max"
                ]
                or 0
            )
        ...
        with transaction.atomic():
            ...
            if order_changed or stage_type_changed:
                ...
                all_stages = WorkflowStage.objects.filter(
                    template=template, stage=working_stage_type
                ).select_for_update()
                ...
                if stage_type_changed:
                    ...
                    all_stages = (
                        WorkflowStage.objects.filter(template=template, stage=working_stage_type)
                        .select_for_update()
                        .order_by("order")
                    )
    ...
    def reorder_stages(template_id, stage_type, order_mapping):
        ...
        with transaction.atomic():
            ...
            stages = (
                WorkflowStage.objects.filter(template=template, stage=stage_type).select_for_update().order_by("order")
            )
    ...
    def delete_stage(stage_id):
        ...
        with transaction.atomic():
            ...
            remaining_stages = (
                WorkflowStage.objects.filter(template=template, stage=stage_type, order__gt=deleted_order)
                .select_for_update()
                .order_by("order")
            )
