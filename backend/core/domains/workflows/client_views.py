...
from .client_serializers import ClientWorkflowStageSerializer
...
class ClientWorkflowViewSet(viewsets.ViewSet):
    ...
    def event_progress(self, request, event_id=None):
        ...
        try:
            ...
            event = Event.objects.select_related("workflow_template", "current_stage").get(
                id=event_id, client=request.user
            )

            if not event.workflow_template:
                return Response({"detail": "No workflow assigned to this event"}, status=status.HTTP_404_NOT_FOUND)
            ...
            logger.error(f"Error getting workflow progress for event {event_id}: {e!s}")
            return Response(
