...
class TestWorkflowStageViewSetActions:
    ...
    def test_trigger_stage_automation(self, api_client, user_factory, event_type_factory, event_factory):
        ...
        assert WorkflowTrigger.objects.filter(
            event=event,
            stage=stage,
    ...
    def test_trigger_stage_wrong_workflow_template(
        self, api_client, user_factory, event_type_factory, event_factory
    ):
        """Test trigger action with event using different workflow template."""
        admin_user = user_factory(admin=True)
        event_type = event_type_factory()

        template1 = WorkflowTemplate.objects.create(
            name='Template 1',
            event_type=event_type
        )
...
class TestWorkflowTriggerViewSet:
    ...
    def test_list_triggers_filter_by_event(
        self, api_client, user_factory, event_type_factory, event_factory
    ):
    ...
    def test_list_triggers_filter_by_trigger_type(
        self, api_client, user_factory, event_type_factory, event_factory
    ):
    ...
    def test_list_triggers_filter_by_processed(
        self, api_client, user_factory, event_type_factory, event_factory
    ):
    ...
    def test_trigger_viewset_is_read_only(
        self, api_client, user_factory, event_type_factory, event_factory
    ):
