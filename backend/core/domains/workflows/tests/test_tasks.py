...
class TestExecuteDelayedStageAction:
    ...
    def test_execute_action_event_moved_to_different_stage(self, event_factory, event_type_factory):
        ...
        stage1 = WorkflowStage.objects.create(
            template=template, name="Stage 1", stage="LEAD", order=1, is_automated=True, automation_type="NOTIFICATION"
        )
...
class TestScheduleBeforeEventAction:
    ...
    def test_schedule_before_event_future_date(self, event_factory, event_type_factory):
        ...
        event = event_factory(
            event_type=event_type, workflow_template=template, current_stage=stage, start_date=event_start
        )
    ...
    def test_schedule_before_event_past_date_executes_immediately(self, event_factory, event_type_factory):
    ...
    def test_schedule_before_event_stage_not_found(self, event_factory, event_type_factory):
        """Test handling when stage doesn't exist."""
        event_type = event_type_factory()
        event = event_factory(event_type=event_type, start_date=timezone.now() + timedelta(days=30))
    ...
    def test_schedule_before_event_various_day_formats(self, event_factory, event_type_factory):
...
class TestTasksIntegration:
    ...
    def test_before_event_action_with_notification(self, event_factory, event_type_factory):
