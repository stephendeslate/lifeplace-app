...
from .models import EventWorkflowOverride, WorkflowStage, WorkflowTemplate, WorkflowTrigger
...
class WorkflowStageDetailSerializer(WorkflowStageSerializer):
    ...
    questionnaire_template_name = serializers.CharField(
        source="questionnaire_template.name", read_only=True, allow_null=True
    )

    class Meta(WorkflowStageSerializer.Meta):
        fields = [*WorkflowStageSerializer.Meta.fields, "email_template_name", "contract_template_name", "questionnaire_template_name"]
...
class EventWorkflowOverrideSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # For other types, require a stage reference
            ...
            raise serializers.ValidationError({"stage": "Stage is required for this override type"})
    ...
    def validate_events(self, value):
        ...
        for event in value:
            if event not in valid_events:
                raise serializers.ValidationError(f"Invalid event type: {event}. Valid types: {valid_events}")
        return value
