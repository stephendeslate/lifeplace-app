...
class TestAppSettingsService:
    ...
    def test_get_setting_returns_value(self):
        ...
        AppSettings.objects.create(category="SYSTEM", key="test_key", value={"test": "value"})
    ...
    def test_get_setting_returns_default_when_not_found(self):
        ...
        result = AppSettingsService.get_setting("SYSTEM", "nonexistent", default={"default": True})
    ...
    def test_get_setting_user_specific(self, user_factory):
        ...
        user = user_factory()
        AppSettings.objects.create(category="NOTIFICATION", key="email_enabled", value={"enabled": True}, user=user)
    ...
    def test_set_setting_creates_new(self):
        ...
        setting = AppSettingsService.set_setting(
            category="SYSTEM", key="new_setting", value={"new": True}, description="A new setting"
        )
    ...
    def test_set_setting_updates_existing(self):
        ...
        setting = AppSettingsService.set_setting("SYSTEM", "update_me", {"new": True}, description="Updated")
    ...
    def test_set_setting_with_encryption(self):
        ...
        setting = AppSettingsService.set_setting(
            category="PAYMENT", key="api_key", value={"secret": "my_secret"}, encrypt=True
        )
    ...
    def test_set_setting_user_specific(self, user_factory):
        ...
        user = user_factory()
        setting = AppSettingsService.set_setting(
            category="NOTIFICATION", key="dark_mode", value={"enabled": True}, user=user
        )
    ...
    def test_set_setting_raises_validation_error_on_failure(self, mocker):
        ...
        mocker.patch.object(AppSettings.objects, "get_or_create", side_effect=Exception("Database error"))
    ...
    def test_bulk_update_settings_raises_on_failure(self, mocker):
        ...
        mocker.patch.object(AppSettingsService, "set_setting", side_effect=Exception("Test error"))

        with pytest.raises(ValidationError) as exc_info:
            AppSettingsService.bulk_update_settings([{"category": "SYSTEM", "key": "test", "value": {"v": 1}}])
...
class TestCurrencySettingsService:
    ...
    def test_get_system_settings_raises_on_error(self, mocker):
        ...
        mocker.patch.object(CurrencySettings, "get_system_settings", side_effect=Exception("Database error"))
    ...
    def test_get_user_settings_raises_on_error(self, user_factory, mocker):
        ...
        user = user_factory()
        mocker.patch.object(CurrencySettings, "get_user_settings", side_effect=Exception("Database error"))
    ...
    def test_create_currency_settings_raises_on_error(self, user_factory, mocker):
        ...
        user = user_factory()
        mocker.patch.object(
            CurrencySettings.objects, "filter", return_value=MagicMock(exists=MagicMock(return_value=False))
        )
        mocker.patch.object(CurrencySettings.objects, "create", side_effect=Exception("Database error"))
    ...
    def test_update_currency_settings(self, user_factory):
        ...
        user = user_factory()
        settings = CurrencySettings.objects.create(user=user, default_currency="PHP", enabled_currencies=["PHP"])
        data = {
            "default_currency": "USD",
            "enabled_currencies": ["USD", "PHP"],
        }

        updated = CurrencySettingsService.update_currency_settings(settings_id=settings.id, data=data, user=user)
    ...
    def test_update_currency_settings_not_found(self, user_factory):
        ...
        with pytest.raises(ValidationError) as exc_info:
            CurrencySettingsService.update_currency_settings(
                settings_id=999, data={"default_currency": "USD"}, user=user
            )
    ...
    def test_update_currency_settings_wrong_user(self, user_factory):
        ...
        user1 = user_factory()
        user2 = user_factory()
        settings = CurrencySettings.objects.create(user=user1, default_currency="PHP")

        with pytest.raises(ValidationError) as exc_info:
            CurrencySettingsService.update_currency_settings(
                settings_id=settings.id, data={"default_currency": "USD"}, user=user2
            )
    ...
    def test_delete_currency_settings(self, user_factory):
        ...
        user = user_factory()
        settings = CurrencySettings.objects.create(user=user, default_currency="EUR")
    ...
    def test_delete_system_settings_not_allowed(self, user_factory):
        ...
        settings = CurrencySettings.objects.create(default_currency="PHP", enabled_currencies=["PHP"])
    ...
    def test_reset_to_defaults_system(self):
        ...
        CurrencySettings.objects.create(default_currency="USD", enabled_currencies=["USD", "EUR"], decimal_places=2)
    ...
    def test_reset_to_defaults_user(self, user_factory):
        ...
        user = user_factory()
        CurrencySettings.objects.create(user=user, default_currency="EUR")
    ...
    def test_get_currency_format_settings_system(self):
        ...
        CurrencySettings.objects.create(
            default_currency="USD", enabled_currencies=["USD", "PHP"], display_format="code", decimal_places=2
        )
    ...
    def test_get_currency_format_settings_user(self, user_factory):
        ...
        user = user_factory()
        CurrencySettings.objects.create(
            user=user, default_currency="EUR", enabled_currencies=["EUR"], display_format="both"
        )
