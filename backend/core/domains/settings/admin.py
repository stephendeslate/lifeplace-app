...
class AppSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('category', 'key', 'description', 'user')
        }),
        ('Value', {
            'fields': ('value', 'is_encrypted', 'encrypted_value')
        }),
        ('Metadata', {
...
class CurrencySettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Currency Configuration', {
            'fields': ('default_currency', 'enabled_currencies', 'user')
        }),
        ('Display Settings', {
            'fields': ('display_format', 'decimal_places', 'thousands_separator', 'decimal_separator')
        }),
        ('Behavior Settings', {
            'fields': ('auto_format', 'compact_format')
        }),
        ('Metadata', {
            'fields': ('id', 'created_at', 'updated_at'),
            'classes': ('collapse',)
...
class LegalDocumentAdmin(admin.ModelAdmin):
    list_display = ('document_type', 'title', 'version', 'effective_date', 'is_published', 'last_updated_by', 'updated_at')
    ...
    fieldsets = (
        ('Document Information', {
            'fields': ('document_type', 'title', 'version', 'effective_date')
        }),
        ('Content', {
            'fields': ('content',),
            'classes': ('wide',)
