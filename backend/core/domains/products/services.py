...
from .exceptions import CategoryNotFound, DiscountCodeExists, DiscountNotFound, ProductNotFound
        ...
        if is_valid is not None:
            ...
            if is_valid:
                ...
                queryset = (
                    ...
                    .filter(Q(max_uses__isnull=True) | Q(current_uses__lt=models.F("max_uses")))
                )
            else:
...
class CustomPackageService:
    ...
    def find_matching_packages(cls, venue_ids, bundle_discount_percent=None, event_type_id=None):
        ...
        discount_percent = (
            Decimal(str(bundle_discount_percent)) if bundle_discount_percent else cls.BUNDLE_DISCOUNT_PERCENT
        )
