    ...
    def validate(self, data):
        ...
        if max_booking_days and advance_days > max_booking_days:
            raise serializers.ValidationError(
                {"maximum_booking_days": "Maximum booking days must be greater than advance booking days"}
            )

        # Validate maximum_quantity constraints
        allow_multiple = data.get(
            "allow_multiple", getattr(self.instance, "allow_multiple", False) if self.instance else False
        )
        maximum_quantity = data.get("maximum_quantity")
        if maximum_quantity is not None:
            if not allow_multiple:
                raise serializers.ValidationError(
                    {"maximum_quantity": 'Maximum quantity is only applicable when "Allow Multiple" is enabled'}
                )
