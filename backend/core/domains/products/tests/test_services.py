...
class TestProductCategoryService:
    ...
    def test_get_all_categories_filter_root(self, product_category_factory):
        ...
        root2 = product_category_factory(name='Root 2')
...
class TestProductService:
    ...
    def test_update_product(self, product_option_factory):
        ...
        updated = ProductService.update_product(
            product.id,
            {'name': 'Updated', 'base_price': Decimal('2000.00')}
        )
...
class TestDiscountService:
    ...
    def test_update_discount(self, discount_factory):
        ...
        updated = DiscountService.update_discount(
            discount.id,
            {'name': 'Updated', 'value': Decimal('20.00')}
        )
...
class TestCustomPackageService:
    ...
    def test_cleanup_abandoned_packages(self, product_option_factory):
        ...
        old_package = product_option_factory(
            custom_package=True,
            name='Old Custom Package'
        )
        # Manually set created_at to simulate old package
        ProductOption.objects.filter(id=old_package.id).update(
            created_at=timezone.now() - timedelta(hours=25)
        )

        # Create recent custom package
        recent_package = product_option_factory(
            custom_package=True,
