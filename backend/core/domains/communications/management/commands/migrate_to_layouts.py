...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--layout-id", type=int, help="Specific layout ID to assign (defaults to default layout)")
        parser.add_argument("--force", action="store_true", help="Override existing layout assignments")
        parser.add_argument(
            "--category", type=str, choices=["SYSTEM", "MANUAL", "AUTO"], help="Only migrate templates of this category"
        )
    ...
    def handle(self, *args, **options):
        ...
        if layout_id:
            ...
            if not layout:
                ...
                self.stdout.write(self.style.WARNING("No default layout found. Creating default layout..."))
                if not dry_run:
                    layout = self._create_default_layout()
                else:
                    self.stdout.write(self.style.NOTICE("[DRY RUN] Would create default layout"))
                    return
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS("No templates to migrate."))
            return
        ...
        with transaction.atomic():
            ...
            for template in templates:
                ...
                updated += 1
                self.stdout.write(
                    f"  Updated: {template.name} (layout: {old_layout.name if old_layout else 'None'} -> {layout.name})"
                )
