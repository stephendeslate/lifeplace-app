...
def sanitize_webhook_data(data: dict, depth: int = 0) -> dict:
    ...
    for key, value in data.items():
        ...
        if value is None:
            ...
            sanitized[safe_key] = [
                sanitize_webhook_data(v, depth + 1)
        ...
        try:
            if event_type == "opened":
                ...
                updated = communication_service.update_delivery_status(
                    external_message_id=str(message_id), status=new_status, opened_at=occurred_at
                )
            else:
                # Handle other delivery events
                updated = communication_service.update_delivery_status(
                    external_message_id=str(message_id), status=new_status
                )
            ...
            logger.error(f"Error updating communication record: {e!s}")
            return HttpResponse(status=500)

    except Exception as e:
        logger.error(f"Unexpected error processing Brevo webhook from IP {client_ip}: {e!s}")
        return HttpResponse(status=500)
