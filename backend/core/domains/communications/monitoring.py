...
class CommunicationMetrics:
    ...
    def record_communication_sent(
        self, template_name: str, channel: str, provider: str, success: bool, response_time_ms: float = None
    ):
    ...
    def get_database_metrics(self, hours: int = 24) -> dict[str, Any]:
        ...
        # Template breakdown
        template_stats = queryset.values("template_name").annotate(count=Count("id")).order_by("-count")[:10]

        # Provider breakdown from context_data
        provider_stats = {}
        for record in queryset.filter(context_data__has_key="provider_used").values("context_data", "delivery_status"):
...
class HealthChecker:
    ...
    def check_all_systems(self) -> dict[str, Any]:
        ...
        health_report = {"timestamp": timezone.now().isoformat(), "overall_status": "healthy", "checks": {}}
    ...
    def _check_providers(self) -> dict[str, Any]:
        ...
        try:
            ...
            healthy_providers = [name for name, status in provider_health.items() if status.get("healthy", False)]
            ...
            return {"healthy": False, "error": str(e), "check_time": timezone.now().isoformat()}
    ...
    def _check_database(self) -> dict[str, Any]:
        ...
        try:
            ...
            old_pending = CommunicationRecord.objects.filter(
                delivery_status="PENDING", created_at__lt=timezone.now() - timedelta(hours=24)
            ).count()
...
class AlertManager:
    ...
    def check_and_alert(self, health_report: dict[str, Any]) -> list[dict]:
        ...
        if recent_metrics["summary"]["total_sent"] > 10:  # Only alert if significant volume
            failure_rate = recent_metrics["summary"]["total_failure"] / recent_metrics["summary"]["total_sent"] * 100
