...
def send_communication_async(
    ...
    try:
        ...
        logger.error(f"Async communication task failed: {e!s}")
...
def send_bulk_communications_async(
    self, template_id: int, recipients: list[dict], sent_by_id: int = None, batch_size: int = 10
):
    """
    ...
    try:
        ...
        if sent_by_id:
            try:
                User.objects.get(id=sent_by_id)
            except User.DoesNotExist:
        ...
        for batch_start in range(0, total_recipients, batch_size):
            ...
            for recipient_data in batch_recipients:
                try:
                    ...
                    logger.error(f"Failed to queue communication for {recipient_data['recipient']}: {e!s}")
                    failed_sends += 1
        ...
        logger.error(f"Bulk communication task failed: {e!s}")
...
def process_retry_queue_async(self):
    ...
    try:
        ...
        return {"success": True, "results": results, "timestamp": timezone.now().isoformat()}

    except Exception as e:
        logger.error(f"Retry queue processing failed: {e!s}")
        return {"success": False, "error": str(e), "timestamp": timezone.now().isoformat()}
...
def cleanup_old_records_async(self, days: int = None):
    ...
    try:
        ...
        deleted_count, _ = CommunicationRecord.objects.filter(
            created_at__lt=cutoff_date, delivery_status__in=["DELIVERED", "SENT"], is_opened=True
        ).delete()
...
def warm_cache_async(self, template_ids: list[int] = None):
    ...
    try:
        ...
        return {"success": True, "template_ids": template_ids, "timestamp": timezone.now().isoformat()}
        ...
        for provider_name, status in health_status.items():
            if not status["healthy"] and status["failures"] >= 10:
                ...
                try:
                    ...
                    logger.error(f"Failed to auto-reset provider {provider_name}: {e!s}")
                    unhealthy_providers.append(provider_name)

        return {
            "success": True,
            "health_status": health_status,
            "auto_reset_providers": [p for p in health_status if p not in unhealthy_providers],
            "unhealthy_providers": unhealthy_providers,
            "timestamp": timezone.now().isoformat(),
        }

    except Exception as e:
        logger.error(f"Health check task failed: {e!s}")
        return {"success": False, "error": str(e), "timestamp": timezone.now().isoformat()}
