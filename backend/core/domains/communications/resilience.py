...
class CircuitBreaker:
    ...
    def __init__(
        self, provider_name: str, failure_threshold: int = 5, recovery_timeout: int = 60, test_timeout: int = 30
    ):
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if not self.can_execute():
            raise CommunicationProviderError(f"Circuit breaker for {self.provider_name} is open")
...
class RetryStrategy:
    ...
    def __init__(
        self, max_retries: int = 3, base_delay: float = 1.0, max_delay: float = 60.0, backoff_factor: float = 2.0
    ):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    logger.warning(f"Attempt {attempt + 1} failed, retrying in {delay}s: {e!s}")
                    time.sleep(delay)
                else:
...
class ProviderManager:
    ...
    def __init__(self):
        ...
        self.retry_strategy = RetryStrategy(max_retries=communication_config.get_rate_limit("MAX_RETRIES") or 3)
        self.load_providers()

    def load_providers(self):
        """Load available providers based on configuration"""
        from .providers import BrevoProvider, MockProvider
    ...
    def get_ordered_providers(self) -> list[str]:
        ...
        return sorted(self.providers.keys(), key=lambda x: self.providers[x]["fallback_order"])
    ...
    def send_with_fallback(self, method: str, *args, **kwargs) -> tuple[str, str]:
        ...
        for provider_name in ordered_providers:
            ...
            try:
                ...
                # Use circuit breaker
                message_id = circuit_breaker.call(self.retry_strategy.execute, execute_send)
                ...
                last_exception = e
                logger.warning(f"Provider {provider_name} failed: {e!s}")
                continue

        # All providers failed
        if last_exception:
            raise CommunicationProviderError(f"All communication providers failed. Last error: {last_exception!s}")
        else:
...
class DeliveryQueue:
    ...
    def add_failed_delivery(self, delivery_data: dict):
        ...
        delivery_data["next_retry"] = self._calculate_next_retry(delivery_data["retry_count"]).isoformat()
