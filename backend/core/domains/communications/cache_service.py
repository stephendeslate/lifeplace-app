...
class CommunicationsCacheService(VersionedCacheService):
    ...
    def cache_template_list(self, templates_data: list[dict], query_params: dict = None) -> str:
    ...
    def get_cached_template_list(self, query_params: dict = None) -> list[dict] | None:
    ...
    def get_cached_template_detail(self, template_id: int) -> dict | None:
    ...
    def get_cached_template_by_name(self, template_name: str) -> dict | None:
        """Get cached template by name"""
        key = self.TEMPLATE_BY_NAME_KEY.format(template_name=template_name)
        return self.cache.get(key)

    def cache_template_preview(self, template_id: int, context_data: dict, preview_data: dict) -> str:
        """Cache template preview result (versioned)"""
        context_hash = self._generate_query_hash(context_data)
        key = self._versioned_key(
            "templates", self.TEMPLATE_PREVIEW_KEY.format(template_id=template_id, context_hash=context_hash)
        )
        self.cache.set(key, preview_data, self.TIMEOUT_LONG)
        logger.debug(f"Cached template preview: {key}")
        return key

    def get_cached_template_preview(self, template_id: int, context_data: dict) -> dict | None:
    ...
    def get_cached_variable_schemas(self) -> dict | None:
    ...
    def get_cached_templates_by_channel(self, channel: str) -> list[dict] | None:
    ...
    def get_cached_templates_by_category(self, category: str) -> list[dict] | None:
    ...
    def cache_record_list(self, records_data: list[dict], query_params: dict = None) -> str:
    ...
    def get_cached_record_list(self, query_params: dict = None) -> list[dict] | None:
    ...
    def get_cached_record_detail(self, record_id: str) -> dict | None:
    ...
    def get_cached_records_by_client(self, client_id: int) -> list[dict] | None:
    ...
    def get_cached_records_by_template(self, template_name: str) -> list[dict] | None:
    ...
    def cache_template_analytics(self, template_name: str, days: int, analytics_data: dict) -> str:
        """Cache template analytics statistics (versioned)"""
        key = self._versioned_key(
            "analytics", self.ANALYTICS_TEMPLATE_STATS_KEY.format(template_name=template_name, days=days)
        )
        self.analytics.set(key, analytics_data, self.TIMEOUT_SHORT)
        logger.debug(f"Cached template analytics: {key}")
        return key

    def get_cached_template_analytics(self, template_name: str, days: int) -> dict | None:
    ...
    def cache_client_analytics(self, client_id: int, days: int, analytics_data: dict) -> str:
        """Cache client-specific communication analytics (versioned)"""
        key = self._versioned_key("analytics", self.ANALYTICS_CLIENT_STATS_KEY.format(client_id=client_id, days=days))
        self.analytics.set(key, analytics_data, self.TIMEOUT_SHORT)
        logger.debug(f"Cached client analytics: {key}")
        return key

    def get_cached_client_analytics(self, client_id: int, days: int) -> dict | None:
    ...
    def get_cached_global_analytics(self, days: int) -> dict | None:
        """Get cached global analytics"""
        key = self._versioned_key("analytics", self.ANALYTICS_GLOBAL_STATS_KEY.format(days=days))
        return self.analytics.get(key)

    def cache_channel_analytics(self, channel: str, days: int, analytics_data: dict) -> str:
        """Cache channel-specific communication analytics (versioned)"""
        key = self._versioned_key("analytics", self.ANALYTICS_CHANNEL_STATS_KEY.format(channel=channel, days=days))
        self.analytics.set(key, analytics_data, self.TIMEOUT_SHORT)
        logger.debug(f"Cached channel analytics: {key}")
        return key

    def get_cached_channel_analytics(self, channel: str, days: int) -> dict | None:
    ...
    def cache_rendered_content(self, template_id: int, context_data: dict, rendered_content: dict) -> str:
        """Cache rendered template content (versioned)"""
        context_hash = self._generate_query_hash(context_data)
        key = self._versioned_key(
            "templates", self.RENDERED_CONTENT_KEY.format(template_id=template_id, context_hash=context_hash)
        )
        self.cache.set(key, rendered_content, self.TIMEOUT_LONG)
        logger.debug(f"Cached rendered content: {key}")
        return key

    def get_cached_rendered_content(self, template_id: int, context_data: dict) -> dict | None:
    ...
    def invalidate_template_caches(self, template_id: int = None, template_name: str = None):
        ...
        if template_id:
            self._delete_specific_key(self.TEMPLATE_DETAIL_KEY.format(template_id=template_id))
    ...
    def invalidate_record_caches(self, record_id: str = None, client_id: int = None, template_name: str = None):
        ...
        # Delete specific keys
        if record_id:
            self._delete_specific_key(self.RECORD_DETAIL_KEY.format(record_id=record_id))

        if client_id:
            self._delete_specific_key(self.RECORD_BY_CLIENT_KEY.format(client_id=client_id))

        if template_name:
            self._delete_specific_key(self.RECORD_BY_TEMPLATE_KEY.format(template_name=template_name))
