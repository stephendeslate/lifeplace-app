...
class BrevoProvider(CommunicationProvider):
    ...
    def _make_request(self, endpoint: str, method: str = "POST", data: dict = None):
        ...
        try:
            ...
            logger.error(f"Brevo API request failed: {e!s}")
            logger.error(f"Request URL: {url}")
            logger.error(f"Request data: {data}")
            logger.error(f"Response text: {getattr(e.response, 'text', 'No response text')}")
            raise CommunicationProviderError(f"Brevo API error: {e!s}")
    ...
    def send_email(self, recipient: str, subject: str, body: str, **kwargs) -> str:
        ...
        if not recipient_name or recipient_name.strip() == "":
            ...
            recipient_name = recipient.split("@", maxsplit=1)[0].title()
        ...
        try:
            ...
            logger.error(f"Failed to send email via Brevo to {recipient}: {e!s}")
            raise CommunicationProviderError(f"Email sending failed: {e!s}")
    ...
    def send_sms(self, recipient: str, body: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to send SMS via Brevo to {recipient}: {e!s}")
            raise CommunicationProviderError(f"SMS sending failed: {e!s}")
    ...
    def get_delivery_status(self, message_id: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to get delivery status from Brevo for {message_id}: {e!s}")
            return "PENDING"
    ...
    def verify_domain(self, domain: str) -> dict:
        ...
        try:
            ...
            logger.error(f"Failed to verify domain {domain}: {e!s}")
