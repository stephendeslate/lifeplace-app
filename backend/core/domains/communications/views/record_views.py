...
from ..monitoring import alert_manager, communication_metrics, health_checker
from ..serializers import BulkSendSerializer, CommunicationRecordSerializer, SendCommunicationSerializer
from ..services import AnalyticsService, CommunicationService, CommunicationTemplateService
from ..throttling import BulkSendThrottle, CommunicationRateLimiter, ManualSendThrottle
...
class CommunicationRecordViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = CommunicationRecord.objects.select_related("client", "sent_by", "event").order_by("-created_at")
    ...
    def send_manual(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response(
                {"error": "Only administrators can send manual communications"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        if use_async:
            ...
            return Response(
                {"message": "Communication queued for async processing", "async": True}, status=status.HTTP_202_ACCEPTED
            )
        else:
            # Synchronous sending
            try:
                ...
                if record:
                    ...
                    return Response(
                        {"error": "Failed to send communication: Unknown error"}, status=status.HTTP_400_BAD_REQUEST
                    )
            except Exception as e:
                logger.error(f"send_manual failed: {e!s}")
                return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def send_bulk(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response(
                {"error": "Only administrators can send bulk communications"}, status=status.HTTP_403_FORBIDDEN
            )

        # Check daily recipient limit
        recipients = request.data.get("recipients", [])
        is_allowed, message = CommunicationRateLimiter.check_daily_bulk_limit(request.user.id, len(recipients))
        if not is_allowed:
        ...
        records = communication_service.send_bulk_communications(
            template=template, recipients=recipients, sent_by=request.user, use_async=use_async
        )
    ...
    def mark_as_read(self, request, pk=None):
        ...
        try:
            ...
            if request.user.role == "CLIENT" and record.client != request.user:
                return Response(
                    {"error": "You can only mark your own messages as read"}, status=status.HTTP_403_FORBIDDEN
                )
            ...
            if not record.is_opened:
                ...
                return Response(
                    {"message": "Message marked as read", "opened_at": record.opened_at}, status=status.HTTP_200_OK
                )
            else:
                return Response({"message": "Message was already marked as read"}, status=status.HTTP_200_OK)

        except Exception as e:
            return Response({"error": f"Failed to mark message as read: {e!s}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def mark_as_unread(self, request, pk=None):
        ...
        try:
            ...
            if request.user.role == "CLIENT" and record.client != request.user:
                return Response(
                    {"error": "You can only mark your own messages as unread"}, status=status.HTTP_403_FORBIDDEN
                )
    ...
    def mark_all_as_read(self, request):
        ...
        try:
            ...
            if request.user.role == "CLIENT":
                ...
                queryset = CommunicationRecord.objects.filter(client=request.user, is_opened=False, is_deleted=False)
            else:
                # Admins can filter by client_id
                client_id = request.data.get("client_id")
                if client_id:
                    queryset = CommunicationRecord.objects.filter(
                        client_id=client_id, is_opened=False, is_deleted=False
                    )
                else:
                    return Response({"error": "Admins must specify a client_id"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            now = timezone.now()
            updated_count = queryset.update(is_opened=True, opened_at=now)

            return Response(
                {"message": f"Marked {updated_count} messages as read", "count": updated_count, "opened_at": now},
                status=status.HTTP_200_OK,
            )

        except Exception as e:
            return Response({"error": f"Failed to mark messages as read: {e!s}"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if request.user.role == "CLIENT":
                count = CommunicationRecord.objects.filter(
                    client=request.user, is_opened=False, is_deleted=False
                ).count()
            else:
                ...
                if client_id:
                    ...
                    count = CommunicationRecord.objects.filter(is_opened=False, is_deleted=False).count()

            return Response({"unread_count": count}, status=status.HTTP_200_OK)

        except Exception as e:
            return Response({"error": f"Failed to get unread count: {e!s}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def health_check(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can access health status"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            unhealthy_providers = [
                name for name, data in health_data["providers"].items() if not data.get("healthy", False)
            ]
            ...
            return Response(
                {"system_status": "error", "error": str(e), "timestamp": timezone.now().isoformat()},
    ...
    def reset_provider(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can reset providers"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            return Response(
                {
                    "error": f"Failed to reset provider {provider_name}: {e!s}",
                    "timestamp": timezone.now().isoformat(),
                },
    ...
    def process_retry_queue(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can process retry queue"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to process retry queue: {e!s}", "timestamp": timezone.now().isoformat()},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def system_health(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can access system health"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            return Response(
                {"overall_status": "error", "error": str(e), "timestamp": timezone.now().isoformat()},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def metrics(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can access metrics"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            logger.error(f"Metrics retrieval failed: {e!s}")
            return Response(
    ...
    def alerts(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can access alerts"}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            logger.error(f"Alerts retrieval failed: {e!s}")
            return Response(
                {"error": str(e), "timestamp": timezone.now().isoformat()}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def clear_alerts(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response({"error": "Only administrators can clear alerts"}, status=status.HTTP_403_FORBIDDEN)

        try:
            alert_manager.clear_alerts()

            return Response({"message": "Active alerts cleared successfully", "timestamp": timezone.now().isoformat()})

        except Exception as e:
            logger.error(f"Clear alerts failed: {e!s}")
            return Response(
                {"error": str(e), "timestamp": timezone.now().isoformat()}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
