...
class TestCommunicationTemplateService:
    ...
    def test_get_all_templates_filter_by_both(self):
        ...
        templates = CommunicationTemplateService.get_all_templates(category="SYSTEM", channel="EMAIL")
    ...
    def test_preview_template_renders_correctly(self, mock_engine):
        ...
        result = CommunicationTemplateService.preview_template(template.id, {"name": "John"})
    ...
    def test_preview_template_with_custom_body_for_manual(self, mock_engine):
        ...
        result = CommunicationTemplateService.preview_template(
            template.id, {"custom_subject": "Custom Subject", "custom_body": "Custom body content"}
        )
...
class TestCommunicationService:
    ...
    def test_send_communication_failure_records_error(self, mock_queue, mock_metrics, mock_provider_manager):
    ...
    @patch("core.domains.communications.services.provider_manager")
    def test_send_communication_respects_user_preferences(self, mock_provider_manager, user_factory, mocker):
        ...
        mocker.patch(
            "core.domains.notifications.models.NotificationPreference.objects.get", return_value=mock_preferences
        )
...
class TestAnalyticsService:
    ...
    def test_get_template_stats_filters_by_date_range(self):
        ...
        CommunicationRecord.objects.filter(id=old_record.id).update(created_at=timezone.now() - timedelta(days=60))
