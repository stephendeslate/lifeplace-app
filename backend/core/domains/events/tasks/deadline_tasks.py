...
@shared_task(
    name="core.domains.events.tasks.check_downpayment_deadline",
    bind=True,
    max_retries=3,
...
@shared_task(
    name="core.domains.events.tasks.daily_deadline_sweep",
    bind=True,
    max_retries=1,
)
def daily_deadline_sweep(self):
    ...
    expired_events = Event.objects.filter(
        downpayment_deadline__lte=now, payment_status="UNPAID", status="CONFIRMED", date_blocked=False
    ...
    logger.info(
        f"Daily deadline sweep completed: {cancelled_count} cancelled, {error_count} errors, {total_count} total"
    )
...
@shared_task(
    name="core.domains.events.tasks.send_deadline_reminder",
    bind=True,
    max_retries=3,
    default_retry_delay=60,
)
...
@shared_task(
    name="core.domains.events.tasks.schedule_deadline_reminders",
    bind=True,
    max_retries=1,
)
def schedule_deadline_reminders(self):
    ...
    for days in reminder_days:
        ...
        events = Event.objects.filter(
            downpayment_deadline__date=target_date, payment_status="UNPAID", status="CONFIRMED", date_blocked=False
        ).exclude(status="CANCELLED")
