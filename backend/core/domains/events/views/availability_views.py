        ...
        try:
            ...
            if event_type_id:
                try:
                    ...
                    return Response({"error": "Invalid event_type_id"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if booking_flow_id:
                try:
                    ...
                    return Response({"error": "Invalid booking_flow_id"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        ...
        try:
            ...
            if start_date > end_date:
                return Response(
                    {"error": "start_date must be before or equal to end_date"}, status=status.HTTP_400_BAD_REQUEST
                )

            # Limit range to prevent abuse
            date_diff = (end_date - start_date).days
            if date_diff > 365:
                return Response({"error": "Date range cannot exceed 365 days"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            availability_results = availability_service.check_multiple_dates(
                start_date=start_date, end_date=end_date, event_type_id=event_type_id, booking_flow_id=booking_flow_id
            )
            ...
            response_data = {
                ...
                "availability": [
                    DateAvailabilityAPIView()._serialize_availability_info(info) for info in availability_results
                ],
...
class NextAvailableDateAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            max_days_ahead = min(max_days_ahead, 365)  # Limit to prevent abuse

            # Find next available date
            next_available = availability_service.get_next_available_date(
                start_date=start_date, event_type_id=event_type_id, max_days_ahead=max_days_ahead
            )
...
def invalidate_availability_cache(request):
    ...
    try:
        ...
        return Response({"message": "Cache invalidated successfully"}, status=status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        try:
            ...
            all_blocked_events = Event.objects.filter(
                start_date__date__gte=start_date, start_date__date__lte=end_date, status="CONFIRMED", date_blocked=True
            ...
            events = (
                Event.objects.filter(
                    start_date__date__gte=start_date, start_date__date__lte=end_date, status="CONFIRMED"
                )
                .exclude(status="CANCELLED")
            ...
            blocked_dates = sorted(global_blocked_dates)
