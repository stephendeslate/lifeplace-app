...
from ..models import Event, EventFeedback, EventFile, EventTask
...
class ClientEventViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            logger.info(
                f"Client {self.request.user.id} requesting events with status={status_filter}, upcoming_only={upcoming_only}"
            )

            return ClientEventService.get_client_events(
                client_id=self.request.user.id, status=status_filter, upcoming_only=upcoming_only
            )
        except Exception as e:
            logger.error(f"Error getting events for client {self.request.user.id}: {e!s}")
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            event = ClientEventService.get_client_event_detail(event_id=pk, client_id=request.user.id)
            serializer = self.get_serializer(event)
            return Response(serializer.data)
        except EventNotFound:
            logger.warning(f"Event {pk} not found for client {request.user.id}")
            return Response(
                {"detail": "Event not found or you don't have permission to view it."}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            logger.error(f"Error retrieving event {pk} for client {request.user.id}: {e!s}")
            return Response(
    ...
    def list(self, request):
        ...
        try:
            ...
            logger.error(f"Error listing events for client {request.user.id}: {e!s}")
            return Response(
    ...
    def timeline(self, request, pk=None):
        ...
        timeline_entries = ClientEventService.get_event_timeline(event_id=pk, client_id=request.user.id)
    ...
    def documents(self, request, pk=None):
        ...
        documents = ClientEventService.get_client_accessible_documents(event_id=pk, client_id=request.user.id)
    ...
    def update_preferences(self, request, pk=None):
        ...
        event = ClientEventService.update_client_preferences(
            event_id=pk, client_id=request.user.id, preferences_data=serializer.validated_data
        )

        return Response(ClientEventDetailSerializer(event).data, status=status.HTTP_200_OK)
    ...
    def tasks(self, request, pk=None):
        ...
        tasks = EventTask.objects.filter(event=event, is_visible_to_client=True).order_by("due_date", "priority")
    ...
    def update_task(self, request, pk=None, task_id=None):
        ...
        try:
            task = EventTask.objects.get(id=task_id, event=event, is_visible_to_client=True, requires_client_input=True)
        except EventTask.DoesNotExist:
            return Response({"detail": "Task not found or cannot be updated"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response(ClientEventTaskSerializer(task).data, status=status.HTTP_200_OK)
    ...
    def upload_file(self, request, pk=None):
        ...
        return Response(
            ClientEventFileSerializer(file_obj, context={"request": request}).data, status=status.HTTP_201_CREATED
        )
    ...
    def feedback(self, request, pk=None):
        ...
        if request.method == "GET":
            ...
            try:
                feedback = EventFeedback.objects.get(event=event, submitted_by=request.user)
            ...
            if EventFeedback.objects.filter(event=event, submitted_by=request.user).exists():
                return Response(
                    {"detail": "Feedback already submitted for this event"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            # Create feedback
            feedback = serializer.save(event=event, submitted_by=request.user)
            ...
            return Response(ClientEventFeedbackSerializer(feedback).data, status=status.HTTP_201_CREATED)
    ...
    def update_feedback(self, request, pk=None, feedback_id=None):
        ...
        try:
            feedback = EventFeedback.objects.get(id=feedback_id, event=event, submitted_by=request.user)
        except EventFeedback.DoesNotExist:
        ...
        if feedback.response:
            return Response(
                {"detail": "Cannot update feedback after admin response"}, status=status.HTTP_400_BAD_REQUEST
            )

        serializer = ClientEventFeedbackSerializer(feedback, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        feedback = serializer.save()

        logger.info(f"Client {request.user.id} updated feedback {feedback_id} for event {pk}")

        return Response(ClientEventFeedbackSerializer(feedback).data, status=status.HTTP_200_OK)
    ...
    def rebook(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f"Client {request.user.id} created rebook session {session.session_id} for cancelled event {pk}"
            )
    ...
    def download_document(self, request, pk=None, file_id=None):
        ...
        try:
            ...
            event_file = EventFile.objects.get(id=file_id, event=event, is_public=True)
        except EventFile.DoesNotExist:
            return Response({"detail": "Document not found or not accessible"}, status=status.HTTP_404_NOT_FOUND)

        if not event_file.file:
            return Response({"detail": "File not found"}, status=status.HTTP_404_NOT_FOUND)

        response = FileResponse(
            event_file.file.open("rb"), content_type=event_file.mime_type or "application/octet-stream"
        )
    ...
    def self_check_in(self, request, pk=None):
        ...
        if event.status != "CONFIRMED":
            return Response({"detail": "Event must be confirmed to check in."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not event_date or now.date() != event_date.date():
            return Response(
                {"detail": "Check-in is only available on the event day."}, status=status.HTTP_400_BAD_REQUEST
            )
