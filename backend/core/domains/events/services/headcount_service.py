...
from ..models import Event, EventProductOption, EventTimeline
...
class HeadcountUpdateService:
    ...
    def update_headcount(
        ...
        from core.domains.sales.models import EventQuote, QuoteLineItem, QuoteActivity
        ...
        with transaction.atomic():
            ...
            if create_quote_revision:
                ...
                latest_quote = (
                    EventQuote.objects
                    .filter(event=event, status__in=['ACCEPTED', 'SENT'])
                    .order_by('-version')
                    .first()
                )
                ...
                if latest_quote:
                    ...
                    per_person_product_ids = [
                        epo.product_option_id for epo in per_person_epos
                    ]

                    for line_item in new_quote.line_items.filter(
                        product_id__in=per_person_product_ids
                    ):
                    ...
                    tax_amount = sum(
                        item.total * (item.tax_rate / Decimal('100'))
                    ...
                    new_quote.notes = (
                        f"{new_quote.notes}\n"
                        f"Headcount updated from {old_count} to {new_count}."
                    ).strip()
    ...
    def _create_supplementary_invoice(
        ...
        try:
            invoice_id = (
                f"INV-SUPP-{timezone.now().strftime('%Y%m%d')}-"
                f"{event.id}-HC{new_count}"
            )
            ...
            existing_invoice = (
                Invoice.objects
                .filter(event=event, status__in=['ISSUED', 'PARTIALLY_PAID'])
                .order_by('-created_at')
                .first()
            )
            due_date = (
                existing_invoice.due_date
                if existing_invoice
                else (timezone.now().date() + timezone.timedelta(days=7))
            )
            ...
            invoice = Invoice.objects.create(
                ...
                status='DRAFT',
                notes=(
                    f"Supplementary invoice for headcount change: "
                    f"{old_count} -> {new_count} guests."
                ),
                quote=new_quote,
            )
            ...
            InvoiceLineItem.objects.create(
                invoice=invoice,
                description=(
                    f"Headcount adjustment: {old_count} -> {new_count} guests"
                ),
                quantity=1,
                unit_price=delta,
            ...
            logger.info(
                f"Created supplementary invoice {invoice_id} for event {event.id}, "
                f"amount={delta}"
            )
