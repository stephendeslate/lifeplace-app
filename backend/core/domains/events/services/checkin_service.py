...
class CheckInService:
    ...
    def check_in(event: Event, staff_user=None, notes: str = "") -> dict:
        ...
        event.save(update_fields=["check_in_status", "actual_check_in_time", "checked_in_by", "check_in_notes"])
    ...
    @staticmethod
    def checkout(event: Event, staff_user=None, notes: str = "", calculate_late_fee: bool = True) -> dict:
        """
        ...
        if event.check_in_status != "CHECKED_IN":
            result["error"] = (
                f"Cannot checkout event with check-in status '{event.check_in_status}'. Event must be CHECKED_IN."
            )
            return result
        ...
        event.save(update_fields=["check_in_status", "actual_checkout_time", "checked_out_by", "checkout_notes"])
    ...
    def get_events_pending_checkin(start_date=None, end_date=None):
        ...
        return Event.objects.filter(
            status="CONFIRMED", check_in_status="PENDING", start_date__gte=start_date, start_date__lte=end_date
        ).order_by("start_date")
    ...
    def get_events_currently_checked_in():
        ...
        """
        return Event.objects.filter(status="CONFIRMED", check_in_status="CHECKED_IN").order_by("start_date")
