...
from ..models import DateReservation, Event
...
class AtomicAvailabilityService:
    ...
    def validate_and_reserve_date(
        ...
        try:
            ...
            if existing_own_reservation:
                ...
                logger.info(
                    f"Returning existing reservation {existing_own_reservation.token} for session {booking_session_id}"
                )
            ...
            blocking_events = (
                ...
                .filter(start_date__date=check_date, date_blocked=True)
            ...
            if blocking_events.exists():
                ...
                logger.info(f"Date {check_date} is blocked by event {blocking_event.id} (session {booking_session_id})")
                return result

            # Step 3: Check for active reservations from OTHER sessions
            active_reservations = (
                DateReservation.objects.select_for_update(nowait=False)
                .filter(target_date=check_date, status="PENDING", expires_at__gt=timezone.now())
    ...
    def release_reservation(reservation_token: str) -> dict:
        ...
        try:
            ...
            reservation = DateReservation.objects.select_for_update().filter(token=token_uuid).first()
    ...
    @staticmethod
    def get_reservation_by_token(reservation_token: str) -> DateReservation | None:
        """
    ...
    @staticmethod
    def validate_reservation_for_completion(reservation_token: str, booking_session_id: str) -> dict:
        """
    ...
    def cleanup_expired_reservations() -> int:
        ...
        """
        expired_count = DateReservation.objects.filter(status="PENDING", expires_at__lt=timezone.now()).update(
            status="EXPIRED"
        )
