...
from ..models import Event, EventFeedback, EventFile, EventTask, EventTimeline
...
class ClientEventSerializer(serializers.ModelSerializer):
    ...
    def get_contracts(self, obj):
        ...
        contracts = (
            obj.contracts.filter(status__in=["SENT", "PARTIALLY_SIGNED", "SIGNED"])
    ...
    def get_pending_signature_required(self, obj):
        ...
        contracts = (
            obj.contracts.filter(status__in=["SENT", "PARTIALLY_SIGNED"])
            .select_related("template")
            .prefetch_related("signatures")
        )
    ...
    def get_venue_image_url(self, obj):
        ...
        event_products = (
            obj.event_products.select_related("product_option")
            .filter(product_option__type="PACKAGE", product_option__is_active=True)
            .order_by("id")
        )
...
class ClientEventDetailSerializer(ClientEventSerializer):
    ...
    class Meta(ClientEventSerializer.Meta):
        fields = ClientEventSerializer.Meta.fields + [  # noqa: RUF005
            "current_stage",
            "total_price",
            "preferences",
            "upcoming_tasks",
            "recent_updates",
            "accessible_documents_count",
            "has_notes",
            # Check-in fields
            "check_in_status",
    ...
    def get_upcoming_tasks(self, obj):
        ...
        tasks = obj.tasks.filter(is_visible_to_client=True, status__in=["PENDING", "IN_PROGRESS"]).order_by("due_date")[
            :5
        ]
        return [
            {
...
class ClientEventTimelineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EventTimeline
        fields = ["id", "action_type", "description", "created_at", "actor_name"]
...
class ClientEventFileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EventFile
        fields = ["id", "name", "category", "size", "created_at", "download_url", "file_type"]
...
class ClientEventTaskUpdateSerializer(serializers.Serializer):
    ...
    completion_notes = serializers.CharField(required=False, allow_blank=True, max_length=1000)
...
class ClientEventFeedbackSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["id", "response", "created_at", "submitted_by_name", "response_by_name", "has_response"]
        ...
        if value:
            for category, rating in value.items():
                if not isinstance(rating, (int, float)) or rating < 1 or rating > 5:
                    raise serializers.ValidationError(f"Category '{category}' rating must be between 1 and 5")
        return value
...
class ClientPackageInfoSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        if hasattr(product, "package_venues"):
            ...
            included_venues = ClientPackageVenueSerializer(package_venues, many=True, context=self.context).data
...
class ClientEventInfoSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        packages_data = []
        event_products = (
            instance.event_products.select_related("product_option__category")
            .prefetch_related("product_option__package_venues__venue")
            .filter(product_option__type="PACKAGE")
            .order_by("id")
        )

        for ep in event_products:
            packages_data.append(ClientPackageInfoSerializer(ep, context=context).data)
