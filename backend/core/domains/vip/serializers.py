    ...
    def validate(self, data):
        ...
        if benefit_type in value_required_types and not data.get('value'):
            raise serializers.ValidationError({
                'value': f'Value is required for {benefit_type} benefits.'
            })
...
class ClientVIPStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'client_email', 'client_name', 'current_tier_name',
            'points_balance', 'lifetime_points_earned', 'lifetime_points_spent',
            'total_spent', 'completed_bookings_count', 'last_activity_at',
...
class ClientVIPStatusPublicSerializer(serializers.ModelSerializer):
    ...
    def get_next_tier(self, obj):
        ...
        if not obj.current_tier:
            ...
            next_tier = VIPTier.objects.filter(
                is_active=True,
