...
def expire_vip_points():
    ...
    from .models import ClientVIPStatus, VIPPointTransaction, VIPSettings
    ...
    for client_status in client_statuses:
        ...
        old_earnings = (
            VIPPointTransaction.objects.filter(
        ...
        logger.info(
            f"Expired {points_to_expire} points for client {client_status.client.email} (new balance: {new_balance})"
        )
