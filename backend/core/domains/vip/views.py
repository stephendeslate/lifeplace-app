...
from core.utils.permissions import IsAdmin, IsClient
...
class VIPBenefitViewSet(viewsets.ModelViewSet):
    ...
    def benefit_types(self, request):
        ...
        return Response([{"value": choice[0], "label": choice[1]} for choice in VIPBenefit.BENEFIT_TYPE_CHOICES])
...
class ClientVIPStatusViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = (
                queryset.filter(client__email__icontains=search)
                | queryset.filter(client__first_name__icontains=search)
                | queryset.filter(client__last_name__icontains=search)
            )
    ...
    def assign_tier(self, request, pk=None):
        ...
        updated_status = VIPService.assign_tier_manually(
            client=client_status.client, tier=tier, assigned_by=request.user, reason=reason
        )
    ...
    def tier_history(self, request, pk=None):
        ...
        client_status = self.get_object()
        history = (
            VIPTierHistory.objects.filter(client_vip_status=client_status)
            .select_related("from_tier", "to_tier", "changed_by")
            .order_by("-created_at")
        )
    ...
    def point_transactions(self, request, pk=None):
        ...
        client_status = self.get_object()
        transactions = (
            VIPPointTransaction.objects.filter(client_vip_status=client_status)
            .select_related("event", "payment", "performed_by")
...
class ClientVIPView(viewsets.ViewSet):
    ...
    def my_points(self, request):
        ...
        client_status = VIPService.get_or_create_client_status(request.user)
        transactions = VIPPointTransaction.objects.filter(client_vip_status=client_status).order_by("-created_at")[:20]
    ...
    def redeem_benefit(self, request):
        ...
        try:
            redemption = VIPRedemptionService.redeem_benefit(client=request.user, benefit=benefit, event=event)
            ...
            return Response(
                {"detail": "Unable to redeem benefit. Please try again or contact support."},
