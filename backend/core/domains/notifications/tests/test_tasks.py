...
class NotificationTasksTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="test@example.com", password="testpass123", first_name="Test", last_name="User", role="ADMIN"
        )
    ...
    def test_create_notification_async_success(self):
        ...
        result = create_notification_async(
            recipient_id=self.user.id, notification_type_code="TEST_NOTIFICATION", context=context
        )
    ...
    def test_create_notification_async_invalid_type(self):
        ...
        result = create_notification_async(
            recipient_id=self.user.id, notification_type_code="INVALID_TYPE", context={"title": "Test"}
        )
    ...
    def test_create_notification_async_rate_limited(self, mock_rate_limit):
        ...
        result = create_notification_async(
            recipient_id=self.user.id, notification_type_code="TEST_NOTIFICATION", context={"title": "Test"}
        )
    ...
    def test_bulk_create_notifications_async(self):
        ...
        user2 = User.objects.create_user(email="test2@example.com", password="testpass123", role="CLIENT")

        recipient_ids = [self.user.id, user2.id]
        context = {"title": "Bulk Test", "message": "Bulk message"}

        results = bulk_create_notifications_async(
            recipient_ids=recipient_ids, notification_type_code="TEST_NOTIFICATION", context=context
        )
    ...
    def test_update_notification_metrics(self):
        ...
        update_notification_metrics(
            notification_type_code=notification_type_code, delivery_methods=delivery_methods, success=True
        )
        ...
        update_notification_metrics(
            notification_type_code=notification_type_code, delivery_methods=[], success=False, error="Test error"
        )
...
class NotificationTasksIntegrationTestCase(TestCase):
    ...
    def test_error_handling_in_async_task(self):
        ...
        result = create_notification_async(recipient_id=self.user.id, notification_type_code="ERROR_TEST", context={})
