...
class NotificationSecurityServiceTestCase(TestCase):
    ...
    def test_validate_context_data(self):
        ...
        context = {"nested": {"level1": {"level2": "<script>evil</script>content"}}}
        ...
        # Test lists
        context = {"list_data": ["<script>evil</script>", "safe", 123, True]}
...
class NotificationRateLimiterTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="ratelimit@example.com", password="testpass123", role="ADMIN")
        cache.clear()
    ...
    def test_creation_limit_allowed(self):
        ...
        can_create, message = NotificationRateLimiter.check_creation_limit(
            user_id=self.user.id, notification_type_code="TEST_TYPE"
        )
    ...
    def test_creation_limit_spam_protection(self):
        ...
        NotificationRateLimiter.record_creation(
            user_id=self.user.id, notification_type_code=notification_type_code, title=title
        )

        # Try to create the same notification again (should be blocked)
        can_create, message = NotificationRateLimiter.check_creation_limit(
            user_id=self.user.id, notification_type_code=notification_type_code
        )
    ...
    def test_bulk_limit_allowed(self):
        ...
        can_bulk, message = NotificationRateLimiter.check_bulk_limit(user_id=self.user.id, recipient_count=50)
    ...
    def test_record_bulk_creation(self):
        ...
        NotificationRateLimiter.record_bulk_creation(user_id=self.user.id, recipient_count=recipient_count)
...
class NotificationContentValidatorTestCase(TestCase):
    ...
    def test_validate_excessive_capitalization(self):
        ...
        data_with_caps = {"title": "URGENT ALERT MESSAGE!!!", "content": "Normal content", "action_url": "/valid/path"}
