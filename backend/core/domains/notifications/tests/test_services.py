...
class TestNotificationService:
    ...
    def test_get_notifications_filtered_by_type(self, user, notification_type):
        ...
        filtered = NotificationService.get_notifications(user, notification_type="SERVICE_TEST")
    ...
    def test_cleanup_old_notifications(self, user, notification_type):
        ...
        Notification.objects.filter(id=old_notif.id).update(created_at=timezone.now() - timedelta(days=100))
...
class TestNotificationStatsService:
    ...
    def user(self):
        """Create a test user"""
        return User.objects.create_user(email="statsuser@example.com", password="testpass123", role="CLIENT")
...
class TestNotificationDigestService:
    ...
    def user(self):
        ...
        user = User.objects.create_user(
            email="digestuser@example.com", password="testpass123", first_name="Digest", last_name="User", role="CLIENT"
        )
    ...
    def unread_notifications(self, user, notification_type):
        ...
        for i in range(5):
            ...
            Notification.objects.filter(id=n.id).update(created_at=now - timedelta(minutes=30))
            n.refresh_from_db()
            notifications.append(n)
        return notifications
...
class TestPushNotificationService:
    ...
    def test_cleanup_inactive_tokens(self, user):
        ...
        DevicePushToken.objects.filter(id=old_token.id).update(updated_at=timezone.now() - timedelta(days=100))
