...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=1,
            help='Number of hours to check for recent activity'
        )
        parser.add_argument(
            '--detailed',
            action='store_true',
            help='Show detailed metrics and statistics'
        )
        parser.add_argument(
            '--alerts-only',
            action='store_true',
            help='Only show alerts and issues'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                status_style(
                    f'{status_icon} Overall Health: {health["status"].upper()} '
            ...
            if not alerts_only:
                ...
                self.stdout.write(f'   • Active Types: {health.get("active_notification_types", 0)}/{health.get("total_notification_types", 0)}')
                
                if health.get("failure_rate", 0) > 0:
                    self.stdout.write(
                        self.style.WARNING(
                            f'   • Failure Rate: {health["failure_rate"]:.1f}%'
                        )
                    )
        ...
        if health['status'] in ['excellent', 'good'] and not alerts:
            self.stdout.write(
                self.style.SUCCESS('✅ Notification system is healthy')
            )
        elif health['status'] == 'warning' or (alerts and not any(a.get('severity') == 'critical' for a in alerts)):
            self.stdout.write(
                self.style.WARNING('⚠️ Notification system needs attention')
            )
        else:
            self.stdout.write(
                self.style.ERROR('🚨 Notification system requires immediate attention')
            )
        ...
        try:
            ...
            for method, rates in delivery_stats.get('delivery_rates', {}).items():
                if rates['attempted'] > 0:
                    self.stdout.write(f'      • {method.upper()}: {rates["rate"]:.1f}% ({rates["successful"]}/{rates["attempted"]})')
            ...
            self.stdout.write(f'   👥 User Engagement:')
            ...
            self.stdout.write(
                self.style.WARNING(f'   ⚠️ Could not load detailed metrics: {str(e)}')
            )