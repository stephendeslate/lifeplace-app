# backend/core/domains/notifications/management/commands/create_notification_types.py
from django.core.management.base import BaseCommand
from core.domains.notifications.models import NotificationType


class Command(BaseCommand):
    help = 'Create default notification types for the system'
...
<p>Thank you for choosing {{ site_name }}!</p>
                ''',
                'default_sms_template': 'Great news! Your {{ event_name }} on {{ event_date }} is confirmed. Track progress at {{ site_name }}',
                'supports_email': True,
                'supports_sms': True,
            },
    ...
    <li><strong>Role:</strong> {{ user_role }}</li>
        ...
        for type_data in notification_types:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'Created notification type: {type_data["name"]}')
                )
            else:
                # Update existing notification type with new data
                ...
                updated_count += 1
                self.stdout.write(
                    self.style.WARNING(f'Updated notification type: {type_data["name"]}')
                )
        
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully created {created_count} and updated {updated_count} notification types'
            )
        )