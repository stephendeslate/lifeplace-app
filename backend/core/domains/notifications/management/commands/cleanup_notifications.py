...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if cleanup_count > 0:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'{"Would delete" if dry_run else "Deleted"} {cleanup_count} old notifications'
                    )
                )
            ...
            if total_actions == 0:
                self.stdout.write(
                    self.style.SUCCESS('No notifications needed cleanup.')
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
            ...
            logger.error(f'Notification cleanup command failed: {str(e)}', exc_info=True)
    ...
    def _auto_mark_old_as_read(self, days, dry_run, batch_size):
        ...
        while True:
            with transaction.atomic():
                ...
                batch_count = Notification.objects.filter(
    ...
    def _cleanup_old_notifications(self, days, dry_run, batch_size):
        ...
        while True:
            with transaction.atomic():
                ...
                batch_count, _ = Notification.objects.filter(
    ...
    def _cleanup_empty_digests(self, dry_run):
        ...
        empty_digests = NotificationDigest.objects.filter(
            created_at__lt=cutoff_date,
