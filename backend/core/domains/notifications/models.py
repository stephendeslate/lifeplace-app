...
class NotificationPreference(BaseModel):
    ...
    marketing_email = models.BooleanField(
        default=False, help_text="User has explicitly consented to receive marketing emails"
    )
    marketing_sms = models.BooleanField(
        default=False, help_text="User has explicitly consented to receive marketing SMS"
    )
    marketing_in_app = models.BooleanField(default=True)
    marketing_push = models.BooleanField(
        default=False, help_text="User has explicitly consented to receive marketing push notifications"
    )
    ...
    disabled_types = models.ManyToManyField(
        NotificationType, blank=True, related_name="users_disabled", help_text="Specific notification types to disable"
    )
...
class Notification(BaseModel):
    ...
    event = models.ForeignKey(
        "events.Event", on_delete=models.CASCADE, null=True, blank=True, related_name="notifications"
    )
    ...
    def add_delivery_method(self, method, success=True, error=None):
        ...
        self.delivery_attempts[method].append(
            {"timestamp": timezone.now().isoformat(), "success": success, "error": error}
        )
...
class DevicePushToken(BaseModel):
    ...
    token = models.CharField(max_length=255, db_index=True, help_text="Expo push token (ExponentPushToken[xxx])")

    # Device identification
    device_id = models.CharField(max_length=255, blank=True, help_text="Unique device identifier for token management")
    ...
    device_name = models.CharField(
        max_length=255, blank=True, help_text="User-friendly device name (e.g., 'John's iPhone')"
    )

    # Status tracking
    is_active = models.BooleanField(default=True, help_text="Set to False when DeviceNotRegisteredError is received")
    last_used_at = models.DateTimeField(
        null=True, blank=True, help_text="Last time a push was successfully sent to this device"
    )
    failure_count = models.PositiveIntegerField(
        default=0, help_text="Consecutive failure count for automatic deactivation"
    )

    # App context
    app_version = models.CharField(max_length=50, blank=True, help_text="App version when token was registered")
