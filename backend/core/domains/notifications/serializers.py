...
class NotificationPreferenceSerializer(serializers.ModelSerializer):
    ...
    disabled_types = serializers.PrimaryKeyRelatedField(
        many=True, queryset=NotificationType.objects.filter(is_active=True), required=False
    )
    disabled_types_details = NotificationTypeSerializer(source="disabled_types", many=True, read_only=True)
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["id", "delivered_via", "delivery_attempts", "is_expired", "created_at", "updated_at"]
...
class NotificationDigestSerializer(serializers.ModelSerializer):
    ...
    def get_notifications_preview(self, obj):
        ...
        return [{"id": n.id, "title": n.title, "type": n.notification_type.name} for n in notifications]


class NotificationBulkActionSerializer(serializers.Serializer):
    """Serializer for bulk notification actions"""

    notification_ids = serializers.ListField(child=serializers.IntegerField(), required=True, min_length=1)
    action = serializers.ChoiceField(choices=["mark_read", "mark_unread", "delete"], required=True)
    ...
    def validate_notification_ids(self, value):
        ...
        user_notification_ids = set(
            Notification.objects.filter(recipient=request.user, id__in=value).values_list("id", flat=True)
        )
...
class CreateNotificationSerializer(serializers.Serializer):
    ...
    recipient_ids = serializers.ListField(child=serializers.IntegerField(), required=True)
    ...
    force_delivery_methods = serializers.ListField(
        child=serializers.ChoiceField(choices=["email", "sms", "in_app"]), required=False, default=list
    )
...
class DevicePushTokenSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["id", "is_active", "last_used_at", "failure_count", "created_at", "updated_at"]
...
class RegisterPushTokenSerializer(serializers.Serializer):
    ...
    token = serializers.CharField(
        max_length=255, required=True, help_text="Expo push token (ExponentPushToken[xxx] or ExpoPushToken[xxx])"
    )
    device_type = serializers.ChoiceField(choices=["ios", "android", "web"], default="ios", required=False)
    device_id = serializers.CharField(
        max_length=255, required=False, allow_blank=True, help_text="Unique device identifier for token management"
    )
    device_name = serializers.CharField(
        max_length=255, required=False, allow_blank=True, help_text="User-friendly device name (e.g., 'John's iPhone')"
    )
    app_version = serializers.CharField(
        max_length=50, required=False, allow_blank=True, help_text="App version when token was registered"
    )
...
class UnregisterPushTokenSerializer(serializers.Serializer):
    ...
    token = serializers.CharField(max_length=255, required=False, help_text="Expo push token to unregister")
    device_id = serializers.CharField(
        max_length=255, required=False, help_text="Device ID to unregister all tokens for"
    )

    def validate(self, data):
        """Ensure at least one identifier is provided"""
        if not data.get("token") and not data.get("device_id"):
            raise serializers.ValidationError("Either 'token' or 'device_id' must be provided")
        return data
...
class TestPushNotificationSerializer(serializers.Serializer):
    ...
    title = serializers.CharField(max_length=255, default="Test Notification", required=False)
    body = serializers.CharField(
        max_length=500, default="This is a test push notification from LifePlace.", required=False
    )
    device_id = serializers.CharField(
        max_length=255, required=False, allow_blank=True, help_text="Specific device to send test to (optional)"
    )
