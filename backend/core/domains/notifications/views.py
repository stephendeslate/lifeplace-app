...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("notification_type", "recipient", "event", "client").order_by("-created_at")
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if notification.recipient != request.user and request.user.role != "ADMIN":
            return Response(
                {"detail": "You do not have permission to access this notification."}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not request.user.is_staff:
            return Response(
                {"detail": "You do not have permission to create notifications."}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not request.user.is_staff:
            return Response(
                {"detail": "You do not have permission to update notifications."}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if notification.recipient != request.user and not request.user.is_staff:
            return Response(
                {"detail": "You do not have permission to delete this notification."}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def bulk_action(self, request):
        ...
        try:
            count = NotificationService.bulk_action(
                request.user.id, serializer.validated_data["notification_ids"], serializer.validated_data["action"]
            )
    ...
    def unread(self, request):
        ...
        try:
            ...
            notifications = NotificationService.get_notifications(request.user, is_read=False, limit=limit)
    ...
    def create_notification(self, request):
        ...
        if request.user.role != "ADMIN":
            return Response(
                {"detail": "Only administrators can create notifications."}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        try:
            ...
            can_bulk, bulk_message = NotificationRateLimiter.check_bulk_limit(
                user_id=request.user.id, recipient_count=len(recipient_ids)
            )
            ...
            NotificationRateLimiter.record_bulk_creation(user_id=request.user.id, recipient_count=len(recipients))
    ...
    def clear_metrics_cache(self, request):
        ...
        try:
            ...
            return Response({"message": "Metrics cache cleared successfully", "cleared_at": timezone.now().isoformat()})
...
class NotificationTypeViewSet(viewsets.ModelViewSet):
    ...
    def categories(self, request):
        ...
        categories = [
            {"value": choice[0], "label": choice[1]} for choice in NotificationType._meta.get_field("category").choices
        ]
    ...
    def priorities(self, request):
        ...
        priorities = [
            {"value": choice[0], "label": choice[1]} for choice in NotificationType._meta.get_field("priority").choices
        ]
...
class NotificationPreferenceViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response({"detail": "Notification preferences cannot be deleted."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update_preferences(self, request):
        ...
        try:
            preferences = NotificationService.update_user_preferences(request.user.id, request.data)
            serializer = self.get_serializer(preferences)
            return Response(serializer.data)
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def reset_to_defaults(self, request):
        ...
        try:
            ...
            preferences = NotificationService.update_user_preferences(request.user.id, default_data)

            serializer = self.get_serializer(preferences)
            return Response({"message": "Preferences reset to defaults", "preferences": serializer.data})
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class DevicePushTokenViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return DevicePushToken.objects.filter(user=self.request.user).order_by("-created_at")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(DevicePushTokenSerializer(push_token).data, status=status.HTTP_201_CREATED)

        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response(
                {"detail": f"Failed to register push token: {e!s}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response({"message": "Push token unregistered successfully"}, status=status.HTTP_200_OK)
    ...
    def unregister(self, request):
        ...
        try:
            ...
            return Response({"message": f"Unregistered {count} push token(s)", "count": count})
    ...
    def my_devices(self, request):
        ...
        return Response({"devices": serializer.data, "count": tokens.count()})
    ...
    def test_push(self, request):
        ...
        try:
            ...
            if device_id:
                token = DevicePushToken.objects.filter(user=request.user, device_id=device_id, is_active=True).first()
                ...
                result = PushNotificationService.send_push_notification(
                    push_token=token.token, title=title, body=body, data={"test": True}
                )
            ...
            result = PushNotificationService.send_push_to_user(
                user_id=request.user.id, title=title, body=body, data={"test": True}
            )
            ...
            return Response(
                {"detail": f"Failed to send test notification: {e!s}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
