...
def connect_domain_signals():
    ...
    try:
        ...
        logger.warning(f"Could not import NotificationService for domain signals: {e!s}")
    except Exception as e:
        logger.error(f"Error connecting domain signals: {e!s}")
    ...
    try:
                ...
                if created:
                    ...
                    for admin in admin_users:
                        try:
                            ...
                            logger.error(f"Failed to create event notification for admin {admin.email}: {e!s}")
                ...
                # Check for status changes
                    ...
                    if instance.status == "CONFIRMED":
                            ...
                            try:
                                NotificationService.create_notification(
                                    ...
                                    context={
                                        ...
                                        or f"{getattr(instance, 'event_type', 'Event')}",
                                        "client_name": instance.client.get_display_name(),
                                        "event_date": instance.start_date.strftime("%B %d, %Y"),
                                        "action_url": f"/events/{instance.id}",
                                    },
                                ...
                                logger.error(f"Failed to create event confirmation notification: {e!s}")
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create client event confirmation notification: {e!s}")
                        ...
                        try:
                            ...
                            logger.error(f"Failed to create event completion notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in event_notifications signal: {e!s}")
        ...
        def workflow_stage_notifications(sender, instance, created, **kwargs):
            ...
            try:
                ...
                if (
                    instance._previous_stage_id != current_stage_id
                    and hasattr(instance, "current_stage")
                        ...
                        try:
                            NotificationService.create_notification(
                                ...
                                context={
                                    ...
                                    "workflow_name": getattr(instance.workflow_template, "name", "Custom Workflow")
                                    if hasattr(instance, "workflow_template") and instance.workflow_template
                            ...
                            logger.error(f"Failed to create workflow stage notification: {e!s}")

                    # Notify client if stage is client-visible
                    if hasattr(instance.current_stage, "is_client_visible") and getattr(
                        instance.current_stage, "is_client_visible", False
                    ):
                        try:
                            NotificationService.create_notification(
                                recipient=instance.client,
                            ...
                            logger.error(f"Failed to create client workflow notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in workflow_stage_notifications signal: {e!s}")

    except LookupError:
        logger.info("Event model not found, skipping event signal connections")
    except Exception as e:
        logger.error(f"Failed to connect event signals: {e!s}")
...
def _connect_task_signals(NotificationService):
    ...
    try:
            ...
            try:
                    ...
                    try:
                        NotificationService.create_notification(
                            ...
                            context={
                                ...
                                or f"{getattr(instance.event, 'event_type', 'Event')}",
                                "due_date": instance.due_date.strftime("%B %d, %Y at %I:%M %p")
                        ...
                        logger.error(f"Failed to create task assignment notification: {e!s}")
                ...
                # Check for status changes
                    ...
                    if instance.status == "COMPLETED":
                        ...
                        if (
                            hasattr(instance, "is_visible_to_client")
                            and instance.is_visible_to_client
                            and instance.event.client
                        ):
                            try:
                                NotificationService.create_notification(
                                ...
                                logger.error(f"Failed to create task completion notification: {e!s}")
                        ...
                        for admin in admin_users:
                            try:
                                NotificationService.create_notification(
                                    ...
                                    context={
                                        "task_title": instance.title,
                                        "assigned_to": instance.assigned_to.get_display_name()
                                ...
                                logger.error(f"Failed to create admin task completion notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in task_notifications signal: {e!s}")
        ...
        logger.error(f"Failed to connect task signals: {e!s}")
...
def _connect_contract_signals(NotificationService):
    ...
    try:
            ...
            try:
                if created:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to create contract sent notification: {e!s}")
                    ...
                    if instance.status == "SIGNED":
                        ...
                        for admin in admin_users:
                            try:
                                ...
                                logger.error(f"Failed to create contract signed notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in contract_notifications signal: {e!s}")
        ...
        logger.error(f"Failed to connect contract signals: {e!s}")
...
def _connect_payment_signals(NotificationService):
    ...
    try:
                        ...
                        if instance.status == "COMPLETED":
                            ...
                            for admin in admin_users:
                                try:
                                    ...
                                    logger.error(f"Failed to create payment received notification: {e!s}")
                            ...
                            try:
                                ...
                                logger.error(f"Failed to create payment confirmation notification: {e!s}")
                            ...
                            try:
                                NotificationService.create_notification(
                                    ...
                                    delivery_methods=[
                                        ...
                                        "sms",
                                        "in_app",
                                    ],  # Force all methods for payment failures
                                )
                            except Exception as e:
                                logger.error(f"Failed to create payment failure notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in payment_notifications signal: {e!s}")
        ...
        logger.error(f"Failed to connect payment signals: {e!s}")
            ...
            try:
                if created and instance.category == "MANUAL" and instance.client:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to create message received notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in communication_notifications signal: {e!s}")

    except LookupError:
        logger.info("CommunicationRecord model not found, skipping communication signal connections")
    except Exception as e:
        logger.error(f"Failed to connect communication signals: {e!s}")
...
def _connect_client_invitation_signals(NotificationService):
    ...
    try:
                ...
                if created:
                    ...
                    for admin in admin_users:
                        try:
                            ...
                            logger.error(f"Failed to create client invitation notification: {e!s}")

                # Check for acceptance
                elif (
                    hasattr(instance, "_previous_is_accepted")
                    and not instance._previous_is_accepted
                    ...
                    for admin in admin_users:
                        try:
                            ...
                            logger.error(f"Failed to create invitation acceptance notification: {e!s}")

            except Exception as e:
                logger.error(f"Error in client_invitation_notifications signal: {e!s}")
        ...
        logger.error(f"Failed to connect client invitation signals: {e!s}")
    ...
    try:
            ...
            try:
                ...
                for admin in admin_users:
                    try:
                        ...
                        logger.error(f"Failed to create support inquiry notification for admin {admin.email}: {e!s}")
            except Exception as e:
                logger.error(f"Error in support_thread_notifications signal: {e!s}")
        ...
        def support_message_notifications(sender, instance, created, **kwargs):
            ...
            try:
                ...
                logger.error(f"Failed to create support reply notification: {e!s}")
        ...
        def support_status_notifications(sender, instance, created, **kwargs):
            ...
            if previous_status and previous_status != "resolved" and instance.status == "resolved":
                try:
                    ...
                    logger.error(f"Failed to create support resolved notification: {e!s}")
        ...
        logger.error(f"Failed to connect support signals: {e!s}")
