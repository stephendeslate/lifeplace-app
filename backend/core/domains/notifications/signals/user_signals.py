...
def create_notification_preferences(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.error(f"Failed to create notification preferences for {instance.email}: {e!s}")
            ...
            if instance.role == "CLIENT":
                ...
                for admin in admin_users:
                    try:
                        ...
                        logger.warning(f"Notification rate-limited for admin {admin.email}: {e!s}")
                    except Exception as e:
                        logger.error(f"Failed to create client notification for admin {admin.email}: {e!s}")

            elif instance.role == "ADMIN":
                logger.info(f"ADMIN created signal fired for user: {instance.id} - {instance.email}")

                # Notify other admins about new admin
                other_admins = User.objects.filter(role="ADMIN", is_active=True).exclude(id=instance.id)
                    ...
                    try:
                        NotificationService.create_notification(
                            ...
                            context={
                                ...
                                "action_url": "/settings/account/admin-users",
                            },
                        )
                    except InvalidNotificationDataException as e:
                        logger.warning(f"Notification rate-limited for admin {admin.email}: {e!s}")
                    except Exception as e:
                        logger.error(f"Failed to create admin notification for {admin.email}: {e!s}")
                    ...
                    try:
                        NotificationService.create_notification(
                            ...
                            context={
                                ...
                                "action_url": "/settings/account/admin-users"
                                if instance.role == "ADMIN"
                                else f"/clients/{instance.id}",
                            },
                        )
                    except InvalidNotificationDataException as e:
                        logger.warning(f"Notification rate-limited for status change: {e!s}")
                    except Exception as e:
                        logger.error(f"Failed to create user status notification: {e!s}")

    except ImportError as e:
        logger.warning(f"Could not import NotificationService: {e!s}")
    except Exception as e:
        logger.error(f"Error in user_notifications signal: {e!s}")
