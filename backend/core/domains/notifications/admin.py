...
class NotificationTypeAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'colored_badge', 'supports_email', 'supports_sms', 
        'is_active', 'is_system', 'created_at'
    ]
    list_filter = [
        'category', 'priority', 'is_active', 'is_system', 
        'supports_email', 'supports_sms', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Configuration', {
            'fields': (
                'is_active', 'is_system', 'supports_email', 
                'supports_sms', 'auto_read_after_days'
            )
        }),
        ('Timestamps', {
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'title', 'recipient_info', 'notification_type',
        'delivery_status', 'read_status', 'created_at'
    ]
    list_filter = [
        'notification_type__category', 'notification_type__priority',
        'is_read', 'is_expired', 'delivered_via', 'created_at',
        'notification_type'
    ]
    search_fields = [
        'title', 'content', 'recipient__email', 
        'recipient__first_name', 'recipient__last_name',
        'client__email', 'client__first_name', 'client__last_name'
    ]
    readonly_fields = [
        'created_at', 'updated_at', 'read_at', 
        'delivered_via', 'delivery_attempts'
    ]
    date_hierarchy = 'created_at'
    
    fieldsets = (
        ('Notification Details', {
            'fields': ('recipient', 'notification_type', 'title', 'content', 'action_url')
        }),
        ('Context & Relations', {
            'fields': ('event', 'client', 'context_data'),
            'classes': ('collapse',)
        }),
        ('Status', {
        ...
        ('Delivery Information', {
            'fields': ('delivered_via', 'delivery_attempts'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
    ...
    def recipient_info(self, obj):
        ...
        if obj.recipient:
            return format_html(
                ...
                obj.recipient.get_display_name(),
                obj.recipient.email
            )
        return 'N/A'
    ...
    def delivery_status(self, obj):
        ...
        if not obj.delivered_via:
            return format_html(
                '<span style="color: #d32f2f;">Not Delivered</span>'
            )
    ...
    def read_status(self, obj):
        ...
        if obj.is_read:
            return format_html(
                '<span style="color: #2e7d32;">✓ Read</span><br>'
                '<small>{}</small>',
            ...
            return format_html(
                '<span style="color: #ed6c02;">● Unread</span>'
            )
    read_status.short_description = 'Status'
    
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'recipient', 'notification_type', 'event', 'client'
        ).order_by('-created_at')
    ...
    def mark_as_read(self, request, queryset):
        ...
        self.message_user(
            request, 
            f'Successfully marked {updated} notifications as read.'
        )
    ...
    def mark_as_unread(self, request, queryset):
        ...
        self.message_user(
            request, 
            f'Successfully marked {updated} notifications as unread.'
        )
...
class NotificationPreferenceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Category Preferences - Email', {
            'fields': (
                'system_email', 'event_email', 'task_email', 'payment_email',
                'client_email', 'contract_email', 'workflow_email', 'communication_email'
            ),
            'classes': ('collapse',)
        }),
        ('Category Preferences - SMS', {
            'fields': (
                'system_sms', 'event_sms', 'task_sms', 'payment_sms',
                'client_sms', 'contract_sms', 'workflow_sms', 'communication_sms'
            ),
            'classes': ('collapse',)
        }),
        ('Category Preferences - In-App', {
            'fields': (
                'system_in_app', 'event_in_app', 'task_in_app', 'payment_in_app',
                'client_in_app', 'contract_in_app', 'workflow_in_app', 'communication_in_app'
            ),
            'classes': ('collapse',)
        }),
        ('Advanced Settings', {
    ...
    def disabled_types_count(self, obj):
        ...
        return format_html(
            '<span style="color: #2e7d32;">All enabled</span>'
        )
...
class NotificationDigestAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'user_info', 'frequency', 'period_display',
        'notification_count', 'delivery_status', 'created_at'
    ]
    list_filter = [
        'frequency', 'is_sent', 'created_at', 'sent_at'
    ]
    search_fields = [
        'user__email', 'user__first_name', 'user__last_name'
    ]
    readonly_fields = [
        'created_at', 'updated_at', 'sent_at', 'notification_count'
    ]
    date_hierarchy = 'created_at'
    ...
    fieldsets = (
        ('Digest Information', {
            'fields': ('user', 'frequency', 'period_start', 'period_end', 'notification_count')
        }),
        ('Delivery Status', {
            'fields': ('is_sent', 'sent_at', 'delivery_methods')
        }),
        ('Notifications', {
    ...
    def delivery_status(self, obj):
        ...
        if obj.is_sent:
            ...
            return format_html(
                '<span style="color: #2e7d32;">✓ Sent</span><br>'
                '<small>via {}</small>',
                methods.title()
            )
        else:
            return format_html(
                '<span style="color: #ed6c02;">● Pending</span>'
            )
