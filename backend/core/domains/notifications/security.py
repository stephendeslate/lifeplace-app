...
class NotificationSecurityService:
    ...
    @classmethod
    def validate_action_url(cls, url: str) -> Optional[str]:
...
class NotificationRateLimiter:
    ...
    @classmethod
    def check_bulk_limit(cls, user_id: int, recipient_count: int) -> tuple[bool, Optional[str]]:
...
class NotificationThrottle(UserRateThrottle):
    ...
    def get_cache_key(self, request, view):
        ...
        return self.cache_format % {
            'scope': self.scope,
            'ident': ident + ':' + getattr(view, 'action', 'default')
        }
...
class NotificationAdminThrottle(UserRateThrottle):
    ...
    def allow_request(self, request, view):
        ...
        if request.user and request.user.is_authenticated and request.user.is_staff:
            ...
            self.rate = getattr(settings, 'REST_FRAMEWORK', {}).get('DEFAULT_THROTTLE_RATES', {}).get('notifications_admin', '500/hour')
