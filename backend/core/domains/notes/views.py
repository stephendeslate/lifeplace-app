...
class NoteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Note.objects.select_related("created_by", "content_type").order_by("-created_at")
    ...
    def perform_update(self, serializer):
        ...
        data = serializer.validated_data.copy()
        note = NoteService.update_note(self.kwargs["pk"], self.request.user, data)
        serializer.instance = note
    ...
    def for_object(self, request):
        ...
        if not content_type_model or not object_id:
            return Response(
                {"detail": "content_type and object_id are required parameters"}, status=status.HTTP_400_BAD_REQUEST
            )

        notes = NoteService.get_notes_for_object(content_type_model, object_id, self.request.user)
