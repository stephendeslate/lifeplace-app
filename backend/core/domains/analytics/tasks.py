...
def snapshot_daily_kpis(self, date_str=None):
    ...
    try:
        ...
        prev_snapshot = DailyKPISnapshot.objects.filter(
            date__lt=date
        ).order_by('-date').first()
        ...
        if prev_snapshot:
            if prev_snapshot.total_revenue > 0:
                revenue_change_pct = (
                    (today_revenue - prev_snapshot.total_revenue) /
                    prev_snapshot.total_revenue * 100
                )
            if prev_snapshot.total_bookings > 0:
                bookings_change_pct = Decimal(str(
                    (today_bookings - prev_snapshot.total_bookings) /
                    prev_snapshot.total_bookings * 100
                ))
...
def backfill_daily_kpi_snapshots(self, start_date_str, end_date_str):
    ...
    logger.info(
        f"KPI backfill complete: {len(results['success'])} success, "
