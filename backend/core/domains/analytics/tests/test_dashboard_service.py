...
class TestDashboardServiceKPISummary:
    ...
    @freeze_time("2024-06-15 12:00:00")
    def test_kpi_summary_event_revenue_from_completed_events(self, event_factory, payment_factory, user_factory):
        ...
        payment_factory(event=confirmed_event, completed=True, amount=Decimal("500.00"))
    ...
    @freeze_time("2024-06-15 12:00:00")
    def test_kpi_summary_revenue_trend_calculation(self, event_factory, payment_factory, user_factory):
        ...
        payment_factory(event=prev_event, completed=True, amount=Decimal("1000.00"))

        # Create event in current period with payment (higher revenue)
        current_event = event_factory(
            client=client, status="COMPLETED", start_date=now - timedelta(days=10), end_date=now - timedelta(days=9)
        )
        payment_factory(event=current_event, completed=True, amount=Decimal("2000.00"))
    ...
    def test_kpi_summary_with_zero_previous_revenue_no_division_error(
        ...
        payment_factory(event=current_event, completed=True, amount=Decimal("1000.00"))
    ...
    def test_kpi_summary_avg_booking_value_calculation(self, event_factory, payment_factory, user_factory):
...
class TestDashboardServiceConversionTracking:
    ...
    def test_kpi_summary_conversion_rate_calculation(self, event_factory):
        ...
        BookingSession.objects.create(
            booking_flow=flow, session_id=uuid.uuid4(), expires_at=expires, is_completed=False
        )
        BookingSession.objects.create(
            booking_flow=flow, session_id=uuid.uuid4(), expires_at=expires, is_completed=False
        )
...
class TestDashboardServiceEdgeCases:
    ...
    def test_kpi_summary_with_multiple_payments_per_event(self, event_factory, payment_factory, user_factory):
    ...
    def test_kpi_summary_excludes_failed_payments(self, event_factory, payment_factory, user_factory):
