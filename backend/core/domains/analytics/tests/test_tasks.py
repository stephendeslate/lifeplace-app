...
class TestUpdateAllBookingFlowAnalyticsTask:
    ...
    def test_task_updates_active_flows(self, event_type_factory):
        ...
        flow1 = BookingFlow.objects.create(name="Test Flow 1", is_active=True, event_type=event_type1)
        flow2 = BookingFlow.objects.create(name="Test Flow 2", is_active=True, event_type=event_type2)
    ...
    def test_task_defaults_to_yesterday(self):
        ...
        analytics = BookingFlowAnalytics.objects.filter(booking_flow=flow, date=yesterday)
    ...
    def test_task_with_custom_date(self):
        ...
        analytics = BookingFlowAnalytics.objects.filter(booking_flow=flow, date="2024-06-10")
        assert analytics.exists()
        assert flow.id in result["success"]
    ...
    def test_task_partial_failure(self, event_type_factory):
        ...
        flow1 = BookingFlow.objects.create(name="Flow 1", is_active=True, event_type=event_type1)
        flow2 = BookingFlow.objects.create(name="Flow 2", is_active=True, event_type=event_type2)
        flow3 = BookingFlow.objects.create(name="Flow 3", is_active=True, event_type=event_type3)
        ...
        def mock_update(flow_id, date):
            ...
            analytics, _ = BookingFlowAnalytics.objects.get_or_create(
                booking_flow=flow, date=date, defaults={"total_sessions": 0}
            )
            return analytics
...
class TestBackfillBookingFlowAnalyticsTask:
    ...
    def test_backfill_single_day(self):
        ...
        assert BookingFlowAnalytics.objects.filter(booking_flow=flow, date="2024-06-10").exists()
    ...
    def test_backfill_date_range(self):
        ...
        analytics_count = BookingFlowAnalytics.objects.filter(
            booking_flow=flow, date__range=("2024-06-01", "2024-06-05")
        ).count()
    ...
    def test_backfill_handles_nonexistent_flow(self):
        ...
        result = backfill_booking_flow_analytics(nonexistent_flow_id, "2024-06-01", "2024-06-03")
    ...
    def test_backfill_handles_partial_failures(self):
        ...
        def mock_update(flow_id, date):
            ...
            analytics, _ = BookingFlowAnalytics.objects.get_or_create(
                booking_flow_id=flow_id, date=date, defaults={"total_sessions": 0}
            )
            return analytics
    ...
    def test_backfill_updates_existing_analytics(self):
        ...
        BookingFlowAnalytics.objects.create(
            booking_flow=flow, date="2024-06-10", total_sessions=5, completed_bookings=2
        )
        ...
        count = BookingFlowAnalytics.objects.filter(booking_flow=flow, date="2024-06-10").count()
        assert count == 1
...
class TestCacheDailyKPIsTask:
    ...
    def test_task_handles_service_errors_gracefully(self):
        ...
        with patch(
            "core.domains.analytics.services.DashboardService.get_kpi_summary", side_effect=mock_get_kpi_summary
    ...
    def test_task_calculates_correct_date_ranges(self):
        ...
        def capture_kpi_summary(start_date, end_date):
            captured_calls.append({"start_date": start_date, "end_date": end_date})
            return {"total_bookings": 0}

        with patch("core.domains.analytics.services.DashboardService.get_kpi_summary", side_effect=capture_kpi_summary):
    ...
    def test_task_caches_actual_kpi_data(self):
        ...
        mock_kpi_data = {"total_bookings": 42, "event_revenue": 50000.0, "conversion_rate": 25.0}

        with patch("core.domains.analytics.services.DashboardService.get_kpi_summary", return_value=mock_kpi_data):
            with patch("django.core.cache.cache.set") as mock_cache_set:
                cache_daily_kpis()
...
class TestTaskIntegration:
    ...
    def test_update_analytics_creates_correct_metrics(self, user_factory):
        ...
        from core.domains.bookingflow.models import BookingFlow, BookingFlowAnalytics, BookingSession
        ...
        yesterday = (timezone.now() - timedelta(days=1)).date()
        yesterday_dt = timezone.make_aware(timezone.datetime.combine(yesterday, timezone.datetime.min.time()))
        ...
        for i in range(2):
            ...
            BookingSession.objects.filter(booking_flow=flow, is_completed=True).update(
                created_at=yesterday_dt + timedelta(hours=i)
            )

        BookingSession.objects.create(
            session_id=uuid.uuid4(), booking_flow=flow, is_abandoned=True, expires_at=yesterday_dt + timedelta(hours=24)
        )
        BookingSession.objects.filter(booking_flow=flow, is_abandoned=True).update(
            created_at=yesterday_dt + timedelta(hours=3)
        )
        ...
        BookingSession.objects.filter(booking_flow=flow, is_completed=False, is_abandoned=False).update(
            created_at=yesterday_dt + timedelta(hours=4)
        )
        ...
        analytics = BookingFlowAnalytics.objects.get(booking_flow=flow, date=yesterday)
    ...
    def test_backfill_with_varying_session_data(self):
        ...
        for date_str in dates:
            ...
            date_dt = timezone.make_aware(timezone.datetime.combine(date_obj, timezone.datetime.min.time()))

            session = BookingSession.objects.create(
                session_id=uuid.uuid4(), booking_flow=flow, is_completed=True, expires_at=date_dt + timedelta(hours=24)
            )
        ...
        for date_str in dates:
            analytics = BookingFlowAnalytics.objects.get(booking_flow=flow, date=date_str)
