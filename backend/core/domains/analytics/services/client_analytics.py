...
class ClientAnalyticsService:
    ...
    def get_client_dashboard(client_id, start_date=None, end_date=None):
        ...
        # Get client's events
        events = Event.objects.filter(client_id=client_id, created_at__range=(start_date, end_date))

        total_events = events.count()
        upcoming_events = events.filter(start_date__gte=timezone.now(), status__in=["CONFIRMED", "LEAD"]).count()
        ...
        total_spent = payments.filter(status="COMPLETED").aggregate(total=Sum("amount"))["total"] or Decimal("0")

        pending_payments = payments.filter(status="PENDING").aggregate(total=Sum("amount"))["total"] or Decimal("0")

        # Overdue payments
        today = timezone.now().date()
        overdue = payments.filter(status="PENDING", due_date__lt=today).aggregate(
            count=Count("id"), amount=Sum("amount")
        )

        # Upcoming payments (next 30 days)
        upcoming_payments = payments.filter(
            status="PENDING", due_date__gte=today, due_date__lte=today + timedelta(days=30)
    ...
    def get_client_event_history(client_id, limit=10):
        ...
        events = (
            Event.objects.filter(client_id=client_id)
            .select_related("event_type", "venue")
    ...
    def get_client_spending_trends(client_id, months=12):
        ...
        # Get client's events
        event_ids = list(
            Event.objects.filter(client_id=client_id, created_at__range=(start_date, end_date)).values_list(
                "id", flat=True
            )
        )
        ...
        monthly = (
            Payment.objects.filter(event_id__in=event_ids, status="COMPLETED", completed_at__isnull=False)
            .annotate(month=TruncMonth("completed_at"))
            .values("month")
            .annotate(amount=Sum("amount"), payment_count=Count("id"))
            .order_by("month")
        )
    ...
    def get_client_upcoming_deadlines(client_id, days=30):
        ...
        # Get client's events
        event_ids = list(Event.objects.filter(client_id=client_id).values_list("id", flat=True))
        ...
        upcoming_payments = (
            Payment.objects.filter(
                event_id__in=event_ids, status="PENDING", due_date__gte=today.date(), due_date__lte=end_date.date()
            )
        ...
        upcoming_events = (
            Event.objects.filter(
                client_id=client_id, start_date__gte=today, start_date__lte=end_date, status__in=["CONFIRMED", "LEAD"]
            )
            .select_related("event_type", "venue")
            .order_by("start_date")
        )
        ...
        pending_contracts = Contract.objects.filter(
            event_id__in=event_ids, status="SENT", expires_at__gte=today, expires_at__lte=end_date
        ).select_related("event")
