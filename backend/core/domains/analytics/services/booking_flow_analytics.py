...
class BookingFlowIntegrationService:
    ...
    def get_funnel_analysis(start_date, end_date, flow_id=None):
        ...
        """
        from core.domains.bookingflow.models import BookingFlow, BookingFlowStep, BookingSession
        ...
        sessions = BookingSession.objects.filter(booking_flow__in=flows, created_at__range=(start_date, end_date))
        ...
        all_steps = (
            BookingFlowStep.objects.filter(booking_flow__in=flows, is_enabled=True)
            .values("step_type")
            .annotate(min_order=Count("order"))
            .order_by("step_type")
        )
        ...
        for step_info in all_steps:
            ...
            step_ids = list(
                BookingFlowStep.objects.filter(
                    booking_flow__in=flows, step_type=step_type, is_enabled=True
                ).values_list("id", flat=True)
            )

            # Count sessions that completed this step
            completed = sessions.filter(completed_steps__id__in=step_ids).distinct().count()
            ...
            reached = sessions.filter(current_step__order__gte=order).distinct().count()
    ...
    def get_flow_performance_summary(start_date, end_date):
        ...
        for flow in flows:
            sessions = BookingSession.objects.filter(booking_flow=flow, created_at__range=(start_date, end_date))
            ...
            if completed_event_ids:
                revenue = Payment.objects.filter(event_id__in=completed_event_ids, status="COMPLETED").aggregate(
                    total=Sum("amount")
                )["total"] or Decimal("0")
    ...
    def get_abandonment_analysis(start_date, end_date, flow_id=None):
        ...
        sessions = BookingSession.objects.filter(is_abandoned=True, created_at__range=(start_date, end_date))
        ...
        for step_type in step_abandonment:
            step_abandonment[step_type]["percentage"] = (
                round((step_abandonment[step_type]["count"] / total_abandoned) * 100, 1) if total_abandoned > 0 else 0
            )

        # Sort by order
        sorted_abandonment = sorted(step_abandonment.items(), key=lambda x: x[1]["order"])
        ...
        return [
            {
                ...
                "conversion_rate": round((d["completed_sessions"] / d["total_sessions"]) * 100, 1)
