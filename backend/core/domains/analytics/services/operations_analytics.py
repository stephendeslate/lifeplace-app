...
class OperationsAnalyticsService:
    ...
    def get_venue_usage(start_date, end_date):
        ...
        results = (
            Event.objects.filter(
                start_date__range=(start_date, end_date), status__in=["CONFIRMED", "COMPLETED"], venue__isnull=False
            )
            .values("venue__id", "venue__name", "venue__code")
            .annotate(
    ...
    def get_calendar_utilization(start_date, end_date):
        ...
        # By month
        monthly = (
            Event.objects.filter(start_date__range=(start_date, end_date), status__in=["CONFIRMED", "COMPLETED"])
            .annotate(month=ExtractMonth("start_date"))
            .values("month")
            .annotate(booking_count=Count("id"), total_revenue=Sum("total_price"))
            .order_by("month")
        )
        ...
        daily = (
            Event.objects.filter(start_date__range=(start_date, end_date), status__in=["CONFIRMED", "COMPLETED"])
            .annotate(day_of_week=ExtractWeekDay("start_date"))
            .values("day_of_week")
