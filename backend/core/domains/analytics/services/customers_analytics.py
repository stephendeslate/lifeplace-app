...
class CustomersAnalyticsService:
        ...
        return [
            {
                ...
                "conversion_rate": round(
                    (r["confirmed_count"] + completed_by_source.get(r["lead_source"] or "", 0)) / r["lead_count"] * 100,
                    1,
                )
                if r["lead_count"] > 0
                else 0,
    ...
    def get_conversion_rate(start_date, end_date):
        ...
        events.filter(status="CANCELLED").count()

        # Completed events use end_date
        completed = Event.objects.filter(status="COMPLETED", end_date__range=(start_date, end_date)).count()
    ...
    def get_customer_growth(start_date, end_date):
        ...
        results = (
            User.objects.filter(role="CLIENT", date_joined__range=(start_date, end_date))
