...
def client_dashboard(request):
    ...
    if request.user.role != "CLIENT":
        return Response({"detail": "This endpoint is only available for clients"}, status=status.HTTP_403_FORBIDDEN)

    start_date, end_date = parse_date_range(request)
    data = ClientAnalyticsService.get_client_dashboard(request.user.id, start_date, end_date)
