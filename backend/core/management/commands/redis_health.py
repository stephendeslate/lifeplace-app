...
from core.utils.redis_monitoring import RedisAlertManager, RedisHealthMonitor
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--memory", action="store_true", help="Include memory usage details")
        parser.add_argument("--performance", action="store_true", help="Include performance metrics")
        parser.add_argument("--cleanup", action="store_true", help="Run cleanup operations")
        ...
        if options["format"] == "json":
            ...
            for db_name, db_data in health_data["databases"].items():
                ...
                if db_data.get("status") == "healthy":
                    self.stdout.write("  ✅ Status: Healthy")
