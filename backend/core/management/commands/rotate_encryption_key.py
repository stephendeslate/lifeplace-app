...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not encrypted_models:
            self.stdout.write(
                self.style.WARNING('No models with encrypted fields found.')
            )
            return
        ...
        for model_info in encrypted_models:
            ...
            rotated_count = self._rotate_model_keys(
                model_class, 
                encrypted_fields, 
                batch_size, 
                dry_run
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Rotated {rotated_count} records in {model_class._meta.label}"
                )
            )
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"\n🔍 DRY RUN COMPLETE: Would rotate {total_rotated} records across "
                    f"{len(encrypted_models)} models"
                )
            )
        else:
            self.stdout.write(
    ...
    def _rotate_model_keys(self, model_class: Type, encrypted_fields: List[str], 
            ...
            with transaction.atomic():
                for record in batch:
                    try:
                        ...
                        logger.error(
                            f"Failed to process {model_class._meta.label} record {record.pk}: {e}"
                        )
                        continue
