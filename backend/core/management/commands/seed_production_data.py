...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force overwrite existing data (deletes and re-creates)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
        ...
        parser.add_argument(
            '--skip',
            type=str,
            help='Skip specific groups (comma-separated)'
        )
        parser.add_argument(
            '--fixtures-dir',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        for group_name, fixtures in self.FIXTURE_GROUPS:
            ...
            for fixture_file, model_path in fixtures:
                loaded, skipped = self._load_fixture(
                    fixture_file,
                    model_path,
                    force,
                    dry_run
    ...
    def _load_fixture(self, fixture_file, model_path, force, dry_run):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'  Loaded: {fixture_file} ({record_count} records)')
            )
