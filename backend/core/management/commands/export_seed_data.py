...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--output", type=str, default="core/fixtures", help="Output directory for fixtures (relative to backend/)"
        )
    ...
    def handle(self, *args, **options):
        ...
        for output_file, model_path, filters in self.EXPORT_CONFIG:
            try:
                records = self._export_model(base_output_path, output_file, model_path, filters, dry_run)
                total_records += records
                if records > 0:
                    exported_files += 1
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"  Error exporting {model_path}: {e}"))
    ...
    def _export_model(self, base_path, output_file, model_path, filters, dry_run):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"  Skipped {model_path}: Model not found"))
            return 0
